import { loadState, saveState, deleteStateStorage, migrate, ensureProfile, computeLevelFromXp, difficultyD, rankLabel, todayISO, formatDateDMY, addNotification, uuid, RANKS } from "./state.js";
import { h, clear, toast, modal, placeholderPage, tip } from "./ui.js";
import { preloadAudio, playBgm, wireSfx, playSfx, setVolumes, setVibrationEnabled, isVibrationEnabled } from "./audio.js";
import { getLang, applyLang, setLang, tr } from "./i18n.js";
import {
  dailyResetIfNeeded, seasonCheckAndReset, newDailyGroup, canAddMoreCardsToday, addCard,
  applyDailyCardProgress, dueCardsForToday, applyReturnChoice, applyReturnChoiceToCardIds,
  computeRewardsForGame, applyRatingDelta, recordAttendance, reconnectStreak, computeAvatarLayers,
  tasksResetIfNeeded, tasksOnPlayGameFinished, tasksOnPlayLessonFinished, getTasksView, claimTaskReward, hasUnclaimedTaskRewards,
  ensureMasteryPath, masteryMarkDueOpened, masteryMarkPlayFinished, masteryMarkTrainingFinished, masteryClaimDailyReward,
  normalizeText
} from "./logic.js";
const app = document.getElementById("app");
const bgEl = document.getElementById("bg"); // kept but not used in modern UI
const USE_VANTA = false;

// === Rewarded Ads (AdMob) ===
// This app uses Rewarded Ads in the Shop to grant optional rewards.
// We ONLY grant rewards on the official 'admob.rewarded.reward' event to prevent fraud.
const DM_ADMOB_APP_ID = "ca-app-pub-4124078609590433~5831605973";
const DM_ADMOB_REWARDED_UNIT = "ca-app-pub-4124078609590433/6186829199";

const dmAds = (()=>{
  let _started = false;
  let _rewarded = null;
  let _ready = false;
  let _inFlight = false;
  let _pending = null; // { kind: string, startedAt:number }
  let _lastErr = "";

  function supported(){
    return (typeof window !== "undefined" && !!window.cordova && typeof window.admob !== "undefined");
  }

  function isReady(){ return !!_ready; }
  function isInFlight(){ return !!_inFlight; }
  function lastError(){ return _lastErr || ""; }

  async function _ensureRewarded(){
    if(!_rewarded) _rewarded = new admob.RewardedAd({ adUnitId: DM_ADMOB_REWARDED_UNIT });
    return _rewarded;
  }

  async function start(){
    if(_started) return true;
    _started = true;

    if(!supported()) return false;

    // Install listeners once
    try{
      document.addEventListener("admob.rewarded.load", ()=>{ _ready = true; _lastErr = ""; }, false);
      document.addEventListener("admob.rewarded.loadfail", (e)=>{ _ready = false; _lastErr = (e && e.error) ? String(e.error) : "loadfail"; }, false);
      document.addEventListener("admob.rewarded.show", ()=>{ /* no-op */ }, false);
      document.addEventListener("admob.rewarded.showfail", (e)=>{ _inFlight = false; _lastErr = (e && e.error) ? String(e.error) : "showfail"; }, false);

      document.addEventListener("admob.rewarded.reward", ()=>{
        // reward is granted ONLY here
        try{ _applyPendingReward(); }catch(_e){}
      }, false);

      document.addEventListener("admob.rewarded.dismiss", async ()=>{
        _inFlight = false;
        _pending = null; // if user dismissed, do not count
        // load next ad
        try{ await load(); }catch(_e){}
      }, false);
    }catch(_e){}

    try{
      await admob.start();
    }catch(e){
      _lastErr = String(e || "admob_start_failed");
      return false;
    }

    // Prime first ad
    try{ await load(); }catch(_e){}
    return true;
  }

  async function load(){
    if(!supported()) return false;
    try{
      const r = await _ensureRewarded();
      _ready = false;
      await r.load();
      _ready = true;
      _lastErr = "";
      return true;
    }catch(e){
      _ready = false;
      _lastErr = String(e || "rewarded_load_failed");
      return false;
    }
  }

  function canStart(kind){
    // Check daily limits for fuel/skip
    const econ = state?.economy || {};
    if(kind === "fuel1" && Number(econ.dailyRewardedFuelClaimed||0) >= 1) return false;
    if(kind === "skip1" && Number(econ.dailyRewardedSkipClaimed||0) >= 1) return false;
    if(kind === "unlockDueToday" && String(econ.dueTodayUnlockedIso||"") === todayISO()) return false;
    return true;
  }

  async function show(kind){
    // Kind: gold100 | help1 | fuel1 | skip1 | unlockDueToday
    if(!supported()) {
      toast((getLang()==="en") ? "Ads are not available on this device." : "الإعلانات غير متوفرة على هذا الجهاز.");
      return false;
    }
    if(_inFlight) return false;

    if(!canStart(kind)){
      toast((getLang()==="en") ? "Daily limit reached." : "تم بلوغ الحد اليومي.");
      return false;
    }

    // Ensure sdk + rewarded prepared
    await start();

    // If ad is not ready, try one load first
    if(!_ready){
      const ok = await load();
      if(!ok){
        toast((getLang()==="en") ? "No ad available now. Try later." : "لا يوجد إعلان متاح الآن. جرّب لاحقًا.");
        return false;
      }
    }

    try{
      const r = await _ensureRewarded();
      _pending = { kind, startedAt: Date.now() };
      _inFlight = true;
      await r.show();
      return true;
    }catch(e){
      _inFlight = false;
      _pending = null;
      _lastErr = String(e || "rewarded_show_failed");
      toast((getLang()==="en") ? "Ad failed to show." : "تعذر عرض الإعلان.");
      // Attempt re-load for next time
      try{ await load(); }catch(_e){}
      return false;
    }
  }

  function _applyPendingReward(){
    if(!_pending || !_pending.kind) return;

    // Anti-fraud: require a pending user-initiated action.
    const kind = _pending.kind;
    _pending = null;

    // Apply reward
    try{
      const econ = state.economy || (state.economy = {});
      const isEn = (getLang()==="en");
      const addGold = (amt)=>{
        amt = Math.max(0, Number(amt||0));
        econ.gold = Math.max(0, Number(econ.gold||0) + amt);
        econ.dailyGoldEarned = Math.max(0, Number(econ.dailyGoldEarned||0) + amt);
        econ.totalGoldEarned = Math.max(0, Number(econ.totalGoldEarned||0) + amt);
      };

      if(kind === "gold100"){
        addGold(100);
        toast(isEn ? "+100 gold." : "+100 ذهب.");
      }
      else if(kind === "help1"){
        econ.helps = Math.max(0, Number(econ.helps||0) + 1);
        toast(isEn ? "+1 help." : "+1 مساعدة.");
      }
      else if(kind === "unlockDueToday"){
        // Unlock "Due Today" pack for this day (one time per day)
        econ.dueTodayUnlockedIso = todayISO();
        econ.dueTodayUnlockAt = Date.now();
        toast(isEn ? "Unlocked for today." : "تم فتح الرزمة لليوم.");
      }
      else if(kind === "fuel1"){
        if(Number(econ.dailyRewardedFuelClaimed||0) >= 1) {
          toast(isEn ? "Daily limit reached." : "تم بلوغ الحد اليومي.");
        } else {
          econ.dailyRewardedFuelProgress = Math.max(0, Number(econ.dailyRewardedFuelProgress||0) + 1);
          const p = Number(econ.dailyRewardedFuelProgress||0);
          if(p >= 2){
            econ.dailyRewardedFuelProgress = 0;
            econ.dailyRewardedFuelClaimed = 1;
            econ.fuel = Math.max(0, Number(econ.fuel||0) + 1);
            toast(isEn ? "+1 fuel." : "+1 وقود.");
          } else {
            toast(isEn ? `Progress: ${p}/2` : `التقدم: ${p}/2`);
          }
        }
      }
      else if(kind === "skip1"){
        if(Number(econ.dailyRewardedSkipClaimed||0) >= 1) {
          toast(isEn ? "Daily limit reached." : "تم بلوغ الحد اليومي.");
        } else {
          econ.dailyRewardedSkipProgress = Math.max(0, Number(econ.dailyRewardedSkipProgress||0) + 1);
          const p = Number(econ.dailyRewardedSkipProgress||0);
          if(p >= 3){
            econ.dailyRewardedSkipProgress = 0;
            econ.dailyRewardedSkipClaimed = 1;
            econ.skips = Math.max(0, Number(econ.skips||0) + 1);
            toast(isEn ? "+1 skip." : "+1 تخطي.");
          } else {
            toast(isEn ? `Progress: ${p}/3` : `التقدم: ${p}/3`);
          }
        }
      }

      saveState(state);
      syncTopbar();
      // refresh current route (updates shop UI counts/disable)
      try{ render(); }catch(_e){}
    }catch(_e){}
  }

  return {
    supported,
    start,
    load,
    show,
    isReady,
    isInFlight,
    lastError,
    canStart,
  };
})();


// Timers that need cleanup on route changes
let _leaderboardTick = null;

// --- Invariant: do not persist an incomplete "today" daily group (must be >=4 cards) ---
function removeCardIdsFromState(ids){
  if(!Array.isArray(ids) || !ids.length) return;
  const idSet = new Set(ids);

  // remove from byId
  for(const id of ids){
    try{ delete state.cards.byId[id]; }catch{}
  }

  // remove from order
  try{ state.cards.order = (state.cards.order||[]).filter(id=> !idSet.has(id)); }catch{}

  // remove from daily groups
  try{
    for(const iso of Object.keys(state.cards.dailyGroups||{})){
      const dg = state.cards.dailyGroups[iso];
      if(!dg?.cardIds) continue;
      dg.cardIds = dg.cardIds.filter(id=> !idSet.has(id));
      if(dg.cardIds.length === 0) delete state.cards.dailyGroups[iso];
    }
  }catch{}

  // remove from ignored/completed lists
  try{ state.cards.ignored = (state.cards.ignored||[]).filter(id=> !idSet.has(id)); }catch{}
  try{ state.cards.completed = (state.cards.completed||[]).filter(id=> !idSet.has(id)); }catch{}

  // remove from custom packs
  try{
    for(const p of (state.library.customPacks||[])){
      if(!p?.cardIds) continue;
      p.cardIds = p.cardIds.filter(id=> !idSet.has(id));
    }
  }catch{}

  // remove from lesson caches
  try{
    for(const iso of Object.keys(state.attendance.lessonCache||{})){
      const entry = state.attendance.lessonCache[iso];
      const list = Array.isArray(entry) ? entry : entry?.cardIds;
      if(!Array.isArray(list)) continue;
      const next = list.filter(id=> !idSet.has(id));
      if(Array.isArray(entry)) state.attendance.lessonCache[iso] = next;
      else state.attendance.lessonCache[iso] = { ...(entry||{}), cardIds: next };
    }
  }catch{}
}

function purgeIncompleteTodayGroup(){
  try{
    const iso = todayISO();
    const dg = state?.cards?.dailyGroups?.[iso];
    const ids = Array.isArray(dg?.cardIds) ? dg.cardIds.slice() : [];
    if(ids.length > 0 && ids.length < 4){
      removeCardIdsFromState(ids);
      try{ delete state.cards.dailyGroups[iso]; }catch{}
    }
  }catch(_){}
}

let state = migrate(loadState() || null);
purgeIncompleteTodayGroup();
saveState(state);

// --- Theme (Light / Dark) ---
function getTheme(){
  const t = (localStorage.getItem("dm_theme") || "light").toLowerCase();
  return (t === "dark") ? "dark" : "light";
}
function applyTheme(theme){
  const t = (theme === "dark") ? "dark" : "light";
  localStorage.setItem("dm_theme", t);
  document.documentElement.dataset.theme = t;
}
// Apply immediately (also set in index.html to avoid flash)
applyTheme(getTheme());
let vibOn = (localStorage.getItem("dm_vibrate") ?? "1") !== "0";
let darkOn = (getTheme() === "dark");

// --- Card back skin (applies to all card surfaces via CSS variable) ---
function getSelectedCardBack(){
  try{
    const lib = state?.library?.cardBacks;
    if(!lib) return null;
    const items = lib.items || [];
    const sel = lib.selected;
    return items.find(x=>x.id===sel) || items[0] || null;
  }catch(_){ return null; }
}

function applyCardBackSkin(){
  try{
    const it = getSelectedCardBack();
    const src = (it && it.src) ? String(it.src) : "";
    const root = document.documentElement;
    root.style.setProperty("--dm-cardback-image", src ? `url("${src}")` : "none");
  }catch(_){ }
}

// Static chrome labels live in index.html (outside the routed app). Keep them in sync with dm_lang.
function syncStaticChromeI18n(){
  try{
    const lblStreak = document.getElementById("lblStreak");
    if(lblStreak) lblStreak.textContent = tr("الحماس");

    const lblNotifications = document.getElementById("lblNotifications");
    if(lblNotifications) lblNotifications.textContent = tr("الإشعارات");

    const lblHelp = document.getElementById("lblHelp");
    if(lblHelp) lblHelp.textContent = tr("التعليمات");

    const lblSettings = document.getElementById("lblSettings");
    if(lblSettings) lblSettings.textContent = tr("الإعدادات");

    const btnProfile = document.getElementById("btnProfile");
    if(btnProfile) btnProfile.setAttribute("aria-label", tr("الملف الشخصي"));

    const goldTag = document.getElementById("goldTag");
    if(goldTag) goldTag.setAttribute("aria-label", tr("الذهب"));
  }catch(_){ }
}

// تعليمات الصفحة (زر ? في الشريط العلوي)
let currentRoute = "home";
const HELP_TEXT = {
  login: { ar: "أدخل اسم مستخدم واختر الجنس، ثم ابدأ.", en: "Enter a username, choose gender, then start." },
  home: { ar: `نصيحة مهمة: قبل أن تضع وقتك في التطبيق، صدِّر بياناتك من «الإعدادات» عبر «تصدير بياناتي»، واحتفظ بملف النسخ الاحتياطي لتستطيع استيراده لاحقًا عند تغيير الجهاز أو حذف بيانات المتصفح.

آلية البطاقات: أي بطاقة تُضيفها اليوم تُحفظ ضمن مجموعة تاريخ اليوم، وتبدأ بالظهور في الألعاب ابتداءً من الغد ثم وفق جدول المستويات المتفق عليه.

لديك اقتراح أو ملاحظة؟ أرسلها إلى بريد الدعم الموجود داخل «الإعدادات».`, en: `Important tip: before you invest your time, export your data from “Settings” using “Export my data”, and keep the backup file so you can import it later if you change devices or clear your browser data.

How cards work: any card you add today is saved inside today’s daily group, and it starts appearing in games from tomorrow, then continues according to the agreed level schedule.

Have a suggestion or a note? Send it to the support email shown inside “Settings”.` },
  add: { ar: "اكتب الكلمة الأجنبية في الحقل الأول، وترجمتها في الثاني، وتلميحًا في الثالث. الحد الأدنى 4 بطاقات والحد الأقصى 8. يمكنك شراء بطاقتين إضافيتين من المتجر.", en: "Put the foreign word in the first field, its meaning in the second, and a memorable hint in the third. Minimum 4 cards, maximum 8. You can buy 2 extra cards from the shop." },
  library: { ar: "افتح أي رزمة ثم استخدم «تحديد» لتنفيذ الإجراءات على عدة بطاقات. «تدريب» يتطلب 4 بطاقات فأكثر.", en: "Open any pack, then use “Select” to run actions on multiple cards. “Training” requires at least 4 cards." },
  shop: { ar: "اشترِ الأدوات والتخصيص. العناصر المشتراة تختفي من المتجر وتظهر ضمن مكتبة الملف الشخصي.", en: "Buy power-ups and cosmetics. Purchased items disappear from the shop and appear in your Profile library." },
  profile: { ar: "غيّر الأفاتار/الخلفية/الإطار من قسم التخصيص. العناصر المملوكة تظهر ويمكن تفعيلها بزر «استخدام».", en: "Change avatar/background/frame from Customize. Owned items can be applied with “Use”." },
  settings: { ar: "تحكم بالصوت. استخدم «تصدير بياناتي» و«استيراد بياناتي» للحفظ الاحتياطي.", en: "Control audio. Use “Export my data” and “Import my data” for backups." },
  notifications: { ar: "تُعلَّم التنبيهات كمقروءة عند فتح الصفحة. استلام الجوائز يتطلب النقر عليها.", en: "Notifications are marked as read when you open the page. Claiming rewards requires tapping them." },
  streak: { ar: "يتتبع الالتزام اليومي. يمكن وصل أيام الانقطاع باستخدام الوقود.", en: "Tracks your daily streak. You can reconnect missed days using Fuel." },
  leaderboard: { ar: "تصنيف محلي على هذا الجهاز. الموسم يُعاد ضبطه كل 100 يوم وفق خوارزمية التصفير.", en: "Local leaderboard on this device. The season resets every 100 days using the reset algorithm." },
};

function openHelp(){
  const lang = getLang();
  const txtObj = HELP_TEXT[currentRoute];
  const txt = (txtObj && typeof txtObj === "object") ? (txtObj[lang] || txtObj.ar) : (txtObj || "لا توجد تعليمات لهذه الصفحة.");
  const body = h("div",{},
    h("div",{class:"subtle", style:"white-space:pre-wrap; line-height:1.8;"}, txt)
  );
  modal(tr("التعليمات"), body, [ { label: tr("إغلاق"), kind:"primary", sfx:"naker", onClick:(close)=> close() } ]);
}

// -------- النسخ الاحتياطي الأسبوعي (تذكير كل سبت) --------
function backupDateKey(d=new Date()){
  const dd = String(d.getDate()).padStart(2,"0");
  const mm = String(d.getMonth()+1).padStart(2,"0");
  const yy = String(d.getFullYear()).slice(-2);
  return `${dd}-${mm}-${yy}`;
}

function defaultBackupFileName(){
  return `DeckMastery_${backupDateKey()}_Backup.json`;
}

function downloadFullBackup(fileName=defaultBackupFileName()){
  const isEn = getLang()==="en";
  const payload = {
    _app:"DeckMastery",
    _type:"backup",
    _v:1,
    exportedAt: new Date().toISOString(),
    storage:{
      deckmastery_v1: state,
      dm_music: localStorage.getItem("dm_music") ?? "0.65",
      dm_sfx: localStorage.getItem("dm_sfx") ?? "0.85",
      dm_vibrate: localStorage.getItem("dm_vibrate") ?? "1",
      dm_theme: localStorage.getItem("dm_theme") ?? "light",
    }
  };

  const jsonText = JSON.stringify(payload, null, 2);

  // Cordova/Monaca APK: تنزيل الويب عبر Blob + a.download قد لا يعمل داخل WebView.
  // ملاحظة مهمة (Android حديث): المسار externalDataDirectory ينتهي داخل Android/data وهو غير مرئي للمستخدم.
  // لذلك نحفظ الملف في cacheDirectory ثم نعرض المشاركة مباشرة ليتمكن المستخدم من حفظه في Downloads/Drive/... إلخ.
  const hasCordova = (typeof window !== "undefined") && !!window.cordova;
  const hasFilePlugin = hasCordova && !!(window.cordova.file) && (typeof window.resolveLocalFileSystemURL === "function");

  if(hasFilePlugin){
    const social = (window.plugins && window.plugins.socialsharing) || null;
    const hasSocialSharing = !!social && (typeof social.shareWithOptions === "function" || typeof social.share === "function");

    // على بعض بيئات Cordova/Android يرجع FileEntry.toURL() رابط cdvfile:// وليس file://،
    // وPlugin المشاركة قد يتجاهله فيصبح الناتج "نص فقط". لذلك نضمن مسار قابل للمشاركة.
    const resolveShareableUrl = (fileEntry)=> new Promise((resolve)=>{
      try{
        if(!fileEntry) return resolve("");
        const tryUrl = (fileEntry.nativeURL)
          || (typeof fileEntry.toNativeURL === "function" ? fileEntry.toNativeURL() : "")
          || (typeof fileEntry.toURL === "function" ? fileEntry.toURL() : "")
          || "";
        const u = String(tryUrl||"");
        if(!u) return resolve("");
        // إذا كان cdvfile:// حاول تحويله لملف حقيقي عبر resolveLocalFileSystemURL
        if(u.startsWith("cdvfile://") && typeof window.resolveLocalFileSystemURL === "function"){
          window.resolveLocalFileSystemURL(u, (e)=>{
            const nu = String((e && (e.nativeURL || (typeof e.toNativeURL === "function" && e.toNativeURL()) || (typeof e.toURL === "function" && e.toURL()))) || u);
            resolve(nu);
          }, ()=> resolve(u));
          return;
        }
        resolve(u);
      }catch(_){ resolve(""); }
    });

    // fallback قوي: مشاركة كـ base64 ملف (يعمل حتى لو مسار الملف لم يُقبل)
    const toBase64Utf8 = (str)=>{
      try{ return btoa(unescape(encodeURIComponent(String(str||"")))); }
      catch(_){ return ""; }
    };

    const shareFileOrBase64 = async ({fileUrl, fileName, jsonText, message})=>{
      if(!hasSocialSharing) return false;
      const safeUrl = String(fileUrl||"");
      const msg = String(message||"");
      const subj = String(fileName||"");

      const doShareWithOptions = (files)=>{
        if(typeof social.shareWithOptions === "function"){
          social.shareWithOptions({ message: msg, subject: subj, files }, ()=>{}, ()=>{});
          return true;
        }
        if(typeof social.share === "function"){
          // signature: (message, subject, file, url, success, error)
          social.share(msg, subj, files && files[0], null, ()=>{}, ()=>{});
          return true;
        }
        return false;
      };

      // 1) حاول مشاركة الملف
      if(safeUrl){
        try{ return doShareWithOptions([safeUrl]); }
        catch(_){ /* ignore */ }
      }

      // 2) fallback: base64 file attachment
      const b64 = toBase64Utf8(jsonText);
      if(!b64) return false;
      try{ return doShareWithOptions([`base64:${subj}//${b64}`]); }
      catch(_){ return false; }
    };

    // Android 11+ يمنع غالبًا وصول المستخدم إلى Android/data.
    // الحل الأذكى:
    // 1) حاول حفظ الملف داخل Downloads إن أمكن (ليكون مرئيًا فورًا في تطبيق الملفات).
    // 2) إذا فشل (قيود scoped storage/صلاحيات)، احفظه في cache ثم افتح نافذة المشاركة ليختار المستخدم مكان الحفظ.
    const candidates = [];
    try{
      // غالبًا متاح على كثير من الأجهزة/البُنى.
      if(window.cordova.file.externalRootDirectory) candidates.push(window.cordova.file.externalRootDirectory + "Download/");
      if(window.cordova.file.externalStorageDirectory) candidates.push(window.cordova.file.externalStorageDirectory + "Download/");
    }catch(_){ /* ignore */ }
    // fallback داخل التطبيق (غير مرئي للمستخدم) لكنه يصلح للمشاركة
    candidates.push(
      window.cordova.file.cacheDirectory
      || window.cordova.file.externalCacheDirectory
      || window.cordova.file.dataDirectory
    );

    const writeFile = (dir, name, content)=> new Promise((resolve, reject)=>{
      window.resolveLocalFileSystemURL(dir, (dirEntry)=>{
        dirEntry.getFile(name, {create:true, exclusive:false}, (fileEntry)=>{
          fileEntry.createWriter((writer)=>{
            writer.onwriteend = ()=> resolve(fileEntry);
            writer.onerror = (e)=> reject(e);
            const blob = new Blob([content], {type:"application/json"});
            writer.write(blob);
          }, reject);
        }, reject);
      }, reject);
    });

    const writeFirstAvailable = async ()=>{
      let lastErr = null;
      for(const dir of candidates){
        if(!dir) continue;
        try{ return { fileEntry: await writeFile(dir, fileName, jsonText), dir }; }
        catch(e){ lastErr = e; }
      }
      throw lastErr || new Error("write failed");
    };

    writeFirstAvailable()
      .then(({fileEntry, dir})=>{
        localStorage.setItem("dm_last_backup_filename", fileName);
        // احتفظ بالمسار للاستفادة لاحقًا (بدون تغيير أي زر آخر الآن)
        const pRaw = (fileEntry && ((typeof fileEntry.toURL === "function" && fileEntry.toURL()) || fileEntry.nativeURL)) || "";
        // حوّل cdvfile:// إن وُجد إلى مسار file:// قابل للمشاركة
        resolveShareableUrl(fileEntry).then((p)=>{
          if(p) localStorage.setItem("dm_last_backup_path", p);

          const looksLikeDownloads = (()=>{
            const d = String(dir||"");
            const u = String(p||"");
            return /Download\/?$/i.test(d) || /\/Download\//i.test(u) || /\/Downloads\//i.test(u);
          })();

          // إذا حفظنا فعلاً داخل Downloads، نكتفي برسالة قصيرة (ولا نُجبر المستخدم على المشاركة).
          if(looksLikeDownloads){
            toast(isEn ? "Backup saved to Downloads." : "تم حفظ النسخة الاحتياطية داخل Downloads.");
            return;
          }

          // وإلا (غالبًا تم الحفظ في cache)، افتح نافذة المشاركة ليحفظها المستخدم أينما يريد.
          if(hasSocialSharing){
            modal(isEn ? "Backup created" : "تم إنشاء النسخة الاحتياطية", h("div",{class:"modal-body"},
              h("div",{style:"line-height:1.75"},
                h("div",{}, isEn
                  ? "Android may hide app folders (Android/data). To save the backup, use Share and choose Downloads/Files/Drive."
                  : "بسبب قيود أندرويد الحديثة قد لا تظهر مجلدات التطبيق (Android/data). لحفظ النسخة اختر «مشاركة» ثم اختر Files/Downloads أو Drive."),
                h("div",{style:"margin-top:8px; opacity:.9; word-break:break-all"}, p || pRaw)
              )
            ), [
              { label: isEn ? "Share" : "مشاركة", kind:"primary", sfx:"tap_primary", onClick:(close)=>{
                  close();
                  shareFileOrBase64({ fileUrl: p, fileName, jsonText, message: isEn ? "DeckMastery backup" : "نسخة احتياطية DeckMastery" });
              }},
              { label: isEn ? "OK" : "حسنًا", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() }
            ]);
          } else {
            toast(isEn ? "Backup created. Share plugin not found." : "تم إنشاء النسخة الاحتياطية، لكن إضافة المشاركة غير متوفرة.");
          }
        });
      })
      .catch(()=>{
        // إذا فشلنا لأي سبب، جرّب مسار الويب كـ fallback.
        try{
          const blob = new Blob([jsonText], {type:"application/json"});
          const url = URL.createObjectURL(blob);
          const a = document.createElement("a");
          a.href = url;
          a.download = fileName;
          document.body.appendChild(a);
          a.click();
          a.remove();
          setTimeout(()=>URL.revokeObjectURL(url), 5000);
          localStorage.setItem("dm_last_backup_filename", fileName);
        }catch(e){
          toast(isEn ? "Export failed." : "فشل التصدير.");
        }
      });

    return;
  }

  // Cordova لكن بدون File plugin: أعطِ رسالة واضحة بدل فشل صامت.
  if(hasCordova && !hasFilePlugin){
    toast(isEn ? "To export in APK, add cordova-plugin-file." : "للتصدير داخل APK أضف cordova-plugin-file.");
    // لا نُعيد هنا لأن مسار الويب قد يفشل داخل WebView ويُربك المستخدم.
    return;
  }

  // Web: المسار الأصلي
  const blob = new Blob([jsonText], {type:"application/json"});
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = fileName;
  document.body.appendChild(a);
  a.click();
  a.remove();
  setTimeout(()=>URL.revokeObjectURL(url), 5000);
  localStorage.setItem("dm_last_backup_filename", fileName);
}

function maybeShowSaturdayBackupReminder(){
  try{
    if(!state.profile) return;
    // لا نُقاطع جلسات اللعب/التدريب
    if(["play","training"].includes(currentRoute)) return;

    const now = new Date();
    const isSaturday = now.getDay() === 6; // Saturday
    if(!isSaturday) return;

    const todayKey = backupDateKey(now);
    const shownKey = localStorage.getItem("dm_backup_reminder_saturday_last") || "";
    if(shownKey === todayKey) return;

    // سجّل أنه ظهر اليوم (مرة واحدة فقط)
    localStorage.setItem("dm_backup_reminder_saturday_last", todayKey);

    const body = h("div",{},
      h("div",{class:"subtle", style:"white-space:pre-wrap; line-height:1.9;"},
`هذا تذكيرٌ أسبوعيٌّ لطيف.

لتحمي تقدّمك من ضياع البيانات، ننصحك بتنزيل نسخة احتياطية على جهازك.

النسخة تشمل كل شيء: مستوى الحساب، الذهب، التصنيف، جميع الرزم والبطاقات والمجموعات اليومية، وكل المقتنيات التي اشتريتها من المتجر.`)
    );

    modal("نسخة احتياطية أسبوعية", body, [
      {
        label:"تنزيل الآن",
        kind:"primary",
        sfx:"naker",
        onClick:(close)=>{
          const prev = localStorage.getItem("dm_last_backup_filename");
          const name = defaultBackupFileName();
          downloadFullBackup(name);
          close();

          // رسالة ثانية: نصيحة حذف النسخة القديمة (يدويًا)
          const oldName = (prev && prev !== name) ? prev : "";
          const tipBody = h("div",{},
            h("div",{class:"subtle", style:"white-space:pre-wrap; line-height:1.9;"},
`للمحافظة على مساحة جهازك، احذف النسخ القديمة إن لم تعد تحتاجها.
${oldName ? `\nآخر نسخة يُنصح بحذفها: ${oldName}` : ""}`)
          );
          modal("تنبيه مساحة التخزين", tipBody, [
            { label:"حسنًا", kind:"primary", sfx:"naker", onClick:(c)=>c() }
          ]);
        }
      },
      {
        label:"لاحقًا",
        kind:"ghost",
        sfx:"naker",
        onClick:(close)=> close()
      }
    ]);
  }catch{}
}


window.addEventListener("hashchange", render);
window.addEventListener("pointerdown", async ()=> {
  // unlock audio
  preloadAudio().catch(()=>{});
}, { once:true, passive:true });

init();

async function init(){
  applyLang();
  syncStaticChromeI18n();
  applyCardBackSkin();
  // icons
  window.lucide?.createIcons?.();

  // preload audio
  preloadAudio().catch(()=>{});

  // route protection: require local profile
  if(!state.profile){
    location.hash = "#/login";
  } else if(!location.hash || location.hash === "#/login"){
    location.hash = "#/home";
  }

  // daily and season checks
  dailyResetIfNeeded(state);
  seasonCheckAndReset(state);
  // tasks reset (daily/weekly)
  tasksResetIfNeeded(state);
  // تطور مستويات البطاقات مع مرور الأيام
  applyDailyCardProgress(state);
  ensureLocalNotifications();

  // apply stored volumes
	  const music = Number(localStorage.getItem("dm_music") ?? "0.65");
	  const sfx = Number(localStorage.getItem("dm_sfx") ?? "0.85");
	  vibOn = (localStorage.getItem("dm_vibrate") ?? "1") !== "0";
	  darkOn = (getTheme() === "dark");
	  setVolumes({ music, sfx });

  // vibration toggle (default ON)
  try{
    if(localStorage.getItem("dm_vibrate") == null) localStorage.setItem("dm_vibrate", "1");
  }catch(_){ }
	  try{
	    setVibrationEnabled(vibOn);
	  }catch(_){ }

    setupHardwareBackExit();

  render();

  // تذكير النسخ الاحتياطي (كل سبت مرة واحدة)
  setTimeout(()=> maybeShowSaturdayBackupReminder(), 350);

  // show after first render
  document.documentElement.classList.remove("boot");
}

function ensureLocalNotifications(){
  // unread dot if exists
  // add expiry reminder for extra cards if any unused (simple: if bought extra and not reached max)
  // keep minimal, local only
  if(!state.profile) return;
  // once per day reminder if not attended and time is late
  const iso = todayISO();
  const attended = !!state.attendance.days[iso]?.completed;
  const hour = new Date().getHours();
  if(!attended && hour >= 20){
    addNotification(state, { type:"reminder", title:"الدرس اليومي", body:"لم يتم تسجيل حضور اليوم.", claimable:false });
  }
  saveState(state);
}

function setBackground(mode){
  if(!USE_VANTA || !bgEl) return;
  // destroy existing vanta
  if(bgEl._vantaEffect){
    try{ bgEl._vantaEffect.destroy(); }catch{}
    bgEl._vantaEffect = null;
  }

  // تحسين أداء الهواتف: Vanta/three.js قد يثقل الجهاز.
  const prefersReduced = window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches;
  const deviceMem = Number(navigator.deviceMemory || 0);
  const cores = Number(navigator.hardwareConcurrency || 0);
  const coarse = window.matchMedia?.("(pointer: coarse)")?.matches;
  const smallScreen = Math.min(window.innerWidth, window.innerHeight) <= 520;
  const lowEnd = prefersReduced || (deviceMem && deviceMem <= 3) || (cores && cores <= 4) || (coarse && smallScreen);

  if(lowEnd){
    // وضع خفيف: بدون Vanta (خلفية ثابتة من CSS)
    bgEl.classList.add("bg-static");
    return;
  }

  bgEl.classList.remove("bg-static");
  const common = {
    el: bgEl,
    mouseControls: true,
    touchControls: true,
    gyroControls: false,
    minHeight: 200,
    minWidth: 200,
    // تقليل العبء
    scale: 1.0,
    scaleMobile: 0.85,
  };

  // تهيئة Vanta بعد الإطار التالي لتجنب تجميد أول تحميل
  requestAnimationFrame(()=>{
    if(mode === "focus"){
      bgEl._vantaEffect = window.VANTA?.CLOUDS?.({ ...common, skyColor:0x0b0a16, cloudColor:0x2a204f, cloudShadowColor:0x0, sunColor:0xffcc66, speed:0.30 });
    } else if(mode === "competitive"){
      bgEl._vantaEffect = window.VANTA?.STARS?.({ ...common, backgroundColor:0x07060a, color:0xffcc66, size:1.05, spacing:22, speed:0.75 });
    } else {
      bgEl._vantaEffect = window.VANTA?.CLOUDS?.({ ...common, skyColor:0x0a0912, cloudColor:0x1f2a56, cloudShadowColor:0x0, sunColor:0xffd28a, speed:0.45 });
    }
  });
}

function syncTopbar(){
  const p = state.profile;
  const econ = state.economy;
  const rating = state.rating;

  document.getElementById("btnProfile").disabled = !p;

  document.getElementById("uiName").textContent = p ? p.username : "DeckMastery";
  document.getElementById("uiRank").textContent = tr(rankLabel(rating.rankKey, rating.subLevel));
  const lvl = computeLevelFromXp(econ.xp);
  document.getElementById("uiLevel").textContent = (getLang()==="en") ? `Level ${lvl.level}` : `المستوى ${lvl.level}`;
  document.getElementById("uiXpFill").style.width = `${Math.round(lvl.pct*100)}%`;
  document.getElementById("uiGold").textContent = String(Math.max(0, Math.floor(econ.gold)));

  document.getElementById("uiStreak").textContent = String(state.streak.current || 0);

  // notifications dot
  const hasUnread = state.notifications.items.some(x=>!x.readAt);
  document.getElementById("uiNotiDot").hidden = !hasUnread;

  // tasks dot (any unclaimed reward)
  const hasTasksReward = hasUnclaimedTaskRewards(state);
  const tDot = document.getElementById("uiTasksDot");
  if(tDot) tDot.hidden = !hasTasksReward;

  // avatar composite
  const stack = document.getElementById("avatarStack");
  stack.innerHTML = "";
  if(p){
    const {frame, bg, av} = computeAvatarLayers(state);
    // frame base
    const frameImg = new Image();
    frameImg.src = frame?.src || "";
    frameImg.className = "layer frame";
    // background (80% of frame, centered)
    const bgImg = new Image();
    bgImg.src = bg?.src || "";
    bgImg.className = "layer bg";
    // avatar (80% of bg, center X, bottom aligned)
    const avImg = new Image();
    avImg.src = av?.src || "";
    avImg.className = "layer av";

    // ملاحظة: موضع الطبقات يدار بالكامل عبر CSS (بدون أي انزياح يمين/يسار).

    stack.append(bgImg, avImg, frameImg);
    // Layering per spec: frame (bottom) -> background -> avatar (top)
  }
}

wireGlobalActions();

function wireGlobalActions(){
  document.getElementById("btnProfile").addEventListener("click", (e)=>{ e.__dm_sfx_ok = true; nav("#/profile"); });
  document.getElementById("btnSettings").addEventListener("click", (e)=>{ e.__dm_sfx_ok = true; nav("#/settings"); });
  document.getElementById("btnNotifications").addEventListener("click", (e)=>{ e.__dm_sfx_ok = true; nav("#/notifications"); });
  document.getElementById("btnStreak").addEventListener("click", (e)=>{ e.__dm_sfx_ok = true; nav("#/streak"); });
  document.getElementById("btnHelp").addEventListener("click", (e)=>{ e.__dm_sfx_ok = true; openHelp(); });
  // تفعيل أصوات اللمس/النقر لشريط الأعلى مرة واحدة
  // Global SFX delegator (covers app + modalRoot)
  wireSfx(document);
}

function nav(hash){
  return navEx(hash);
}

// --- Navigation stack (for in-app Back button)
// We intentionally manage our own stack because some pages previously forced a return to Home.
// This makes "Back" consistently return to the previous page the user came from.
const NAV_STACK_KEY = "dm_nav_stack_v1";
const SESSION_RETURN_KEY = "dm_return_after_session_v1";
const SESSION_ROUTES = new Set(["#/play", "#/training"]); // do not enter via Back
function baseHash(h){
  const s = (h || "").split("?")[0];
  return s || "#/home";
}
function isSessionRoute(h){ return SESSION_ROUTES.has(baseHash(h)); }
function getSessionReturn(){
  try{ return sessionStorage.getItem(SESSION_RETURN_KEY) || ""; }catch(_){ return ""; }
}
function setSessionReturn(h){
  try{ sessionStorage.setItem(SESSION_RETURN_KEY, h); }catch(_){/* ignore */}
}
function lastNonSessionFromStack(st){
  if(!Array.isArray(st)) return "";
  for(let i=st.length-1;i>=0;i--){
    const v = st[i];
    if(v && !isSessionRoute(v)) return v;
  }
  return "";
}

function getNavStack(){
  try{
    const raw = sessionStorage.getItem(NAV_STACK_KEY);
    const arr = raw ? JSON.parse(raw) : [];
    return Array.isArray(arr) ? arr : [];
  }catch(_){
    return [];
  }
}

function setNavStack(arr){
  try{
    const safe = Array.isArray(arr) ? arr.slice(-50) : [];
    sessionStorage.setItem(NAV_STACK_KEY, JSON.stringify(safe));
  }catch(_){/* ignore */}
}

function navEx(hash, opts={}){
  const {
    replace = false,
    noSfx = false,
    fromBack = false,
  } = opts || {};

  // SFX is handled centrally via data-sfx + wireSfx (plays after successful click actions).

  // keep legacy "return from settings" key for compatibility, but it is no longer relied upon
  if(hash === "#/settings"){
    const from = location.hash && location.hash !== "#/settings" ? location.hash : "#/home";
    sessionStorage.setItem("dm_return_from_settings", from);
  }

  // push current hash into stack unless this navigation is triggered by Back.
// IMPORTANT: We never want Back to enter Play/Training, so those routes are excluded from the stack.
const cur = location.hash || "#/home";
const curBase = baseHash(cur);
const nextBase = baseHash(hash);

// When entering a session (play/training), remember where we came from (non-session only)
if(!fromBack && isSessionRoute(nextBase)){
  const st0 = getNavStack();
  const fallback = lastNonSessionFromStack(st0) || "#/home";
  const ret = (!isSessionRoute(curBase) ? cur : fallback);
  setSessionReturn(ret);
}

// Push current page to stack (only if current page is NOT a session route)
if(!fromBack && cur && cur !== hash && !isSessionRoute(curBase)){
  const st = getNavStack();
  if(st.length === 0 || st[st.length-1] !== cur) st.push(cur);
  setNavStack(st);
}
  // navigate
  if(replace){
    // replace current history entry to avoid creating loops when using our custom back stack
    try{ location.replace(hash); }catch(_){ location.hash = hash; }
  }else{
    location.hash = hash;
  }
}

function navBack(){
  // Back should always go one step back *within the app*.
  // Rules:
  // 1) Never enter Play/Training via Back.
  // 2) If we're currently inside Play/Training, Back returns to the page that launched the session.
  const cur = location.hash || "#/home";
  const curBase = baseHash(cur);

  if(isSessionRoute(curBase)){
    const ret = getSessionReturn() || "#/home";
    return navEx(ret, { fromBack:true, replace:true });
  }

  const st = getNavStack();
  while(st.length){
    const prev = st.pop();
    if(!prev) continue;
    if(prev === cur) continue;
    if(isSessionRoute(prev)) continue;
    setNavStack(st);
    return navEx(prev, { fromBack:true, replace:true });
  }
  setNavStack([]);
  return navEx("#/home", { fromBack:true, replace:true });
}

let _hwBackPrimedAt = 0;
let _hwBackInstalled = false;

function setupHardwareBackExit(){
  if(_hwBackInstalled) return;
  if(!(typeof window !== "undefined" && window.cordova)) return;
  if(!(typeof document !== "undefined" && document.addEventListener)) return;

  _hwBackInstalled = true;
  document.addEventListener("backbutton", (e)=>{
    try{ e.preventDefault?.(); }catch(_){}
    const now = Date.now();
    if(now - _hwBackPrimedAt < 2000){
      try{ navigator?.app?.exitApp?.(); }catch(_){}
      return;
    }
    _hwBackPrimedAt = now;
    const isEn = getLang() === "en";
    toast(isEn ? "Press back again to exit." : "اضغط زر الرجوع مرة ثانية للخروج.");
  }, false);
}

function render(){
  // route protection
  if(!state.profile){
    if(location.hash !== "#/login") location.hash = "#/login";
  }

  // تحديثات يومية/موسمية يجب تطبيقها بغض النظر عن الصفحة
  if(state.profile){
    try{
      dailyResetIfNeeded(state);
      seasonCheckAndReset(state);
      tasksResetIfNeeded(state);
      // keep Mastery Path (and its weekly reward) synced even if user doesn't open the page
      try{ ensureMasteryPath(state); }catch(_){ }

      // show weekly reward toast exactly once
      try{
        const mp = state?.masteryPath;
        if(mp?.week?._justGrantedSkip){
          mp.week._justGrantedSkip = false;
          const isEn = getLang()==="en";
          toast(isEn ? "Weekly reward: +1 skip game." : "مكافأة الأسبوع: +1 تخطي لعبة.");
        }
      }catch(_){ }

      saveState(state);
    }catch(_){/* ignore */}
  }
  syncTopbar();

  const fullHash = (location.hash || "#/home").replace("#/","");
  const parts = fullHash.split("?");
  const route = parts[0];
  const hashParams = new URLSearchParams(parts[1] || "");
  currentRoute = route;

  // هذا الشريط العلوي (الملف الشخصي + الذهب + أزرار أعلى) خاص بالصفحة الرئيسية فقط.
  // بقية الصفحات يجب أن تظهر بدون "قسم الرئيسية العلوي" حتى لا يتكرر في كل مكان.
  const topbar = document.getElementById("topbar");
  const topNav = document.getElementById("topNav");
  const showHomeChrome = (route === "home");
  if(topbar) topbar.style.display = showHomeChrome ? "flex" : "none";
  if(topNav) topNav.style.display = showHomeChrome ? "grid" : "none";




  // إخفاء شريط معلومات الحساب أثناء اللعب/التدريب فقط
  document.body.classList.toggle(
    "in-session",
    route === "play" || route === "training"
  );

  // cleanup any page-specific timers (avoid leaks when navigating)
  try{
    if(_leaderboardTick){ clearInterval(_leaderboardTick); _leaderboardTick = null; }
  }catch(_){ }

  clear(app);

  // background + bgm per route
  // Music rules:
  // - Outside gameplay: khalfia.mp3
  // - Inside gameplay sessions (play/training): tahadi.mp3
  if(route.startsWith("play") || route.startsWith("training")) {
    setBackground("focus");
    playBgm("session").catch(()=>{});
  } else if(route.startsWith("add")) {
    setBackground("focus");
    playBgm("main").catch(()=>{});
  } else {
    // profile/settings/shop/library/notifications/home/login...
    setBackground(route.startsWith("leaderboard") || route.startsWith("profile") ? "competitive" : "main");
    playBgm("main").catch(()=>{});
  }

  if(route === "login") renderLogin();
  else if(route === "home") renderHome();
  else if(route === "add") renderAddCard();
  else if(route === "play") renderPlay();
  else if(route === "training") renderTraining();
  else if(route === "library") renderLibrary();
  else if(route === "shop") renderShop();
  else if(route === "profile") renderProfile();
  else if(route === "settings") renderSettings();
  else if(route === "notifications") renderNotifications();
  else if(route === "streak") renderStreak();
  else if(route === "friends") renderComingSoonPage({
    title: (getLang()==="en") ? "Friends" : "الأصدقاء",
    message: (getLang()==="en") ? "This page is not ready yet. It will be available in a future update." : "هذه الصفحة قيد التطوير وستتوفر في تحديث لاحق."
  });
  else if(route === "leaderboard") renderLeaderboard();
  else if(route === "tasks") renderTasks();
  else if(route === "mastery") renderMastery();

  window.lucide?.createIcons?.();

  // Some badge elements exist only on specific routes (e.g., tasks dot on Home).
  // Re-sync after rendering the route.
  try{ syncTopbar(); }catch(_){ }
}


// Unified "Coming Soon" page (used for placeholder routes like friends/tasks/mastery/leaderboard)
function renderComingSoonPage({ title, message, icon="fas fa-hourglass-half" } = {}){
  const isEn = getLang()==="en";
  const dir = (document.documentElement?.dir || "rtl").toLowerCase();
  const backIcon = (dir === "ltr") ? "fas fa-arrow-left" : "fas fa-arrow-right";

  const header = h("header",{class:"page-header"},
    h("div",{class:"profile-card", "data-sfx":"naker", onClick: ()=> nav("#/home")},
      h("div",{class:"login-badge", ariaHidden:"true"}, h("i",{class: icon, ariaHidden:"true"})),
      h("div",{class:"profile-text"},
        h("div",{class:"profile-name"}, title || (isEn?"Coming soon":"قريبًا")),
        h("div",{class:"user-meta"}, h("span",{class:"meta-pill"}, isEn?"Under development":"قيد التطوير"))
      )
    ),
    h("div",{class:"gold-tag", "aria-label": isEn?"Gold":"الذهب"},
      h("i",{class:"fas fa-coins"}),
      h("span",{class:"gold-val"}, String(Math.max(0, Math.floor(state.economy.gold||0))))
    )
  );

  const body = h("section",{class:"panel glass gold"},
    h("div",{class:"empty-state"},
      h("div",{class:"empty-icon"}, h("i",{class: icon, ariaHidden:"true"})),
      h("div",{class:"subtle empty-msg", style:"white-space:pre-wrap;line-height:1.9;"}, message || (isEn?"This feature is under development.":"هذه الميزة قيد التطوير.")),
      h("div",{class:"row", style:"gap:10px;justify-content:center;margin-top:14px;flex-wrap:wrap;"},
        h("button",{class:"btn primary", "data-sfx":"tap_primary", onClick: ()=> nav("#/home")}, isEn?"Home":"الرئيسية"),
        h("button",{class:"btn", "data-sfx":"tap_secondary", onClick: ()=> navBack()}, isEn?"Back":"رجوع")
      )
    )
  );

  const backBtn = h("button",{class:"fixed-back-btn btn-active", "data-sfx":"naker", onClick: ()=> navBack()},
    h("i",{class: backIcon, ariaHidden:"true"}),
    isEn?"Back":"رجوع"
  );

  const wrap = h("div",{class:"packs-wrap"}, header, h("div",{class:"packs-scroll", style:"display:flex;align-items:center;justify-content:center;"}, body), backBtn);
  app.appendChild(wrap);
}

// Consistent empty state for sessions (play/training)
function renderSessionEmpty({ title, message, icon="fas fa-circle-info", primary=null, secondary=null } = {}){
  const dir = (document.documentElement?.dir || "rtl").toLowerCase();
  const backIcon = (dir === "ltr") ? "fas fa-arrow-left" : "fas fa-arrow-right";

  const head = h("div", { class: "panel-title" },
    h("div", { class: "row", style: "gap:10px;align-items:center;flex-wrap:nowrap;" },
      h("button", { class: "icon-btn", "data-sfx": "tap_secondary", ariaLabel: "رجوع", onClick: () => navBack() },
        h("i", { class: backIcon, ariaHidden: "true" })
      ),
      h("h2", { class: "h1" }, title || "")
    ),
    h("div", {})
  );

  const body = h("div", { class: "empty-state" },
    h("div", { class: "empty-icon" }, h("i", { class: icon, ariaHidden: "true" })),
    h("div", { class: "subtle empty-msg", style: "white-space:pre-wrap;line-height:1.9;" }, message || "")
  );

  const actions = [];
  if(primary){
    actions.push(h("button", { class: "btn primary", "data-sfx": primary.sfx || "tap_primary", onClick: primary.onClick }, primary.label));
  }
  if(secondary){
    actions.push(h("button", { class: `btn ${secondary.kind || "ghost"}`, "data-sfx": secondary.sfx || "tap_secondary", onClick: secondary.onClick }, secondary.label));
  }

  const panel = h("section", { class: "glass gold panel session-empty" },
    head,
    h("div", { class: "hr" }),
    body,
    actions.length ? h("div", { class: "hr" }) : null,
    actions.length ? h("div", { class: "action-stack" }, ...actions) : null,
  );

  app.appendChild(panel);
}

function renderLogin(){
  clear(app);

  let selectedGender = "male";

  const setLanguage = (lang) => {
    setLang(lang);
    applyLang();
    syncStaticChromeI18n();
    // Re-render to reflect translations + direction immediately
    renderLogin();
  };

  const uInput = h("input", {
    id: "u",
    type: "text",
    class: "modern-input login-username",
    style: "padding-left: 40px;",
    dir: "auto",
    maxlength: "30",
    placeholder: (getLang() === "en") ? "e.g. Wisam_123" : "مثل: Wisam_123",
    inputmode: "text",
    autocapitalize: "off",
    autocomplete: "nickname",
    spellcheck: "false",
  });

  const setGenderUI = (value, btn) => {
    selectedGender = value;
    const grid = btn?.parentElement;
    grid?.querySelectorAll?.(".gender-btn")?.forEach((b) => {
      b.classList.toggle("selected", b === btn);
      b.setAttribute("aria-pressed", b === btn ? "true" : "false");
    });
  };

  const genderBtn = (value, label, iconClass) =>
    h(
      "button",
      {
        class: `gender-btn ${value === selectedGender ? "selected" : ""}`,
        type: "button",
        "data-sfx": "tap_secondary",
        "aria-pressed": value === selectedGender ? "true" : "false",
        onClick: (e) => setGenderUI(value, e.currentTarget),
      },
      h("i", { class: iconClass, ariaHidden: "true" }),
      h("span", {}, label)
    );

  const start = () => {
    const u = (uInput.value || "").trim();
    if (!/^[A-Za-z0-9_]{3,30}$/.test(u)) {
      toast(tr("اسم غير صالح."));
      return;
    }
    ensureProfile(state, { username: u, gender: selectedGender });
    saveState(state);
    nav("#/home");
  };

  // Header like the new UI (but without real profile)
  const header = h(
    "header",
    { class: "page-header" },
    h(
      "div",
      { class: "profile-card login-header-card", role: "heading", "aria-level": "1" },
      h("div", { class: "login-badge", ariaHidden: "true" }, h("i", { class: "fas fa-user-plus" })),
      h(
        "div",
        { class: "profile-text" },
        h("div", { class: "profile-name" }, "DeckMastery"),
        h("div", { class: "user-meta" }, h("span", { class: "meta-pill" }, tr("البدء")))
      )
    ),
    h("div", { class: "gold-tag", title: tr("الحساب") }, h("i", { class: "fas fa-lock" }), h("span", {}, tr("محلي")))
  );

  const hero = h(
    "div",
    { class: "login-hero glass" },
    h("div", { class: "auth-mark", ariaHidden: "true" }, h("i", { class: "fas fa-wand-magic-sparkles" })),
    h("div", { class: "subtle", style: "text-align:center;white-space:pre-wrap;line-height:1.7;" }, tr(HELP_TEXT.login[getLang()] ? HELP_TEXT.login[getLang()] : HELP_TEXT.login.ar))
  );

  const langSwitch = (() => {
    const lang = getLang();
    return h(
      "div",
      { class: "lang-switch" },
      h(
        "button",
        {
          class: `btn lang-btn ${lang === "ar" ? "selected" : ""}`,
          type: "button",
          "data-sfx": "tap_secondary",
          onClick: () => setLanguage("ar"),
        },
        "العربية"
      ),
      h(
        "button",
        {
          class: `btn lang-btn ${lang === "en" ? "selected" : ""}`,
          type: "button",
          "data-sfx": "tap_secondary",
          onClick: () => setLanguage("en"),
        },
        "English"
      )
    );
  })();

  const form = h(
    "section",
    { class: "panel glass gold login-card" },
    hero,
    langSwitch,
    h(
      "div",
      { class: "field" },
      h("label", {}, tr("الاسم المستعار")),
      h("div",{class:"search-box login-userbox", style:"margin-bottom:0;"}, uInput, h("i",{class:"fas fa-user", ariaHidden:"true"})),
      h("div", { class: "subtle", style: "font-size:0.85rem;" }, "A-Z / 0-9 / _ — 3..30")
    ),
    h(
      "div",
      { class: "field" },
      h("label", {}, tr("الجنس")),
      h("div", { class: "gender-grid" },
        genderBtn("male", tr("ذكر"), "fas fa-mars"),
        genderBtn("female", tr("أنثى"), "fas fa-venus"),
        genderBtn("other", tr("آخر"), "fas fa-transgender")
      )
    ),
    h(
      "button",
      { class: "btn primary auth-start", type: "button", "data-sfx": "tap_primary", onClick: start },
      h("i", { class: "fas fa-play", ariaHidden: "true" }),
      tr("بدء")
    )
  );

  const page = h("div", { class: "login-page" }, header, form);
  app.appendChild(page);

  // Enter => start
  uInput.addEventListener("keydown", (ev) => {
    if (ev.key === "Enter") start();
  });
}



function renderHome(){
  // absence popup if pending
  if(state.attendance.missedPending?.length){
    const items = (state.attendance.missedPending || [])
      .filter(x=> x && x.iso)
      .slice()
      .sort((a,b)=> String(a.iso).localeCompare(String(b.iso)));

    // Fallback for old states (no cardIds): use all frozen cards.
    const allFrozenIds = state.cards.order.filter(id=> state.cards.byId[id]?.frozen);

    // Per-day choice state (default: continue)
    const choiceByIso = new Map(items.map(it=> [it.iso, "continue"]));
    const btnRefs = new Map(); // iso -> {continue, restart, ignore}

    const setChoice = (iso, choice) => {
      choiceByIso.set(iso, choice);
      const refs = btnRefs.get(iso);
      if(!refs) return;
      refs.continue.classList.toggle("selected", choice === "continue");
      refs.restart.classList.toggle("selected", choice === "restart");
      refs.ignore.classList.toggle("selected", choice === "ignore");
    };

    const dayBlock = (it) => {
      const title = state.cards.dailyGroups[it.iso]?.titleDMY || formatDateDMY(it.iso);
      const count = (Array.isArray(it.cardIds) && it.cardIds.length) ? it.cardIds.length : allFrozenIds.length;

      const bContinue = h("button", { class:"btn small", "data-sfx":"tap_secondary", onClick:()=> { setChoice(it.iso, "continue"); } }, "استمرار");
      const bRestart  = h("button", { class:"btn small", "data-sfx":"tap_secondary", onClick:()=> { setChoice(it.iso, "restart"); } }, "إعادة");
      const bIgnore   = h("button", { class:"btn small danger", "data-sfx":"tagahl", onClick:()=> { setChoice(it.iso, "ignore"); } }, "تجاهل");

      btnRefs.set(it.iso, { continue: bContinue, restart: bRestart, ignore: bIgnore });
      // apply default selection styling
      setChoice(it.iso, choiceByIso.get(it.iso) || "continue");

      return h("div", { class:"abs-lesson" },
        h("div", { class:"abs-lesson-head" },
          h("div", { class:"abs-lesson-title" }, title),
          h("div", { class:"meta-pill" }, tr("بطاقات"), ": ", String(count || 0))
        ),
        h("div", { class:"row abs-choices" }, bContinue, bRestart, bIgnore)
      );
    };

    const body = h("div", { class:"abs-wrap" },
      h("div", { class:"subtle" }, tr("لديك دروس فائتة. اختر الإجراء لكل درس.")),
      ...items.map(dayBlock),
      h("div", { class:"hr" }),
      h("div", { class:"subtle" }, tr("إجراءات للكل")),
      h("div", { class:"row abs-bulk" },
        h("button", { class:"btn small", "data-sfx":"tap_secondary", onClick:()=> {
          for(const it of items) setChoice(it.iso, "continue");
        } }, "استمرار بالكل"),
        h("button", { class:"btn small", "data-sfx":"tap_secondary", onClick:()=> {
          for(const it of items) setChoice(it.iso, "restart");
        } }, "إعادة الكل"),
        h("button", { class:"btn small danger", "data-sfx":"tagahl", onClick:()=> {
          for(const it of items) setChoice(it.iso, "ignore");
        } }, "تجاهل الكل")
      )
    );

    modal(tr("العودة بعد الغياب"), body, [
      { label: tr("إلغاء"), kind:"ghost", sfx:"naker", onClick:(close)=> close() },
      { label: tr("حفظ"), kind:"primary", sfx:"tap_primary", onClick:(close)=> {
        for(const it of items){
          const choice = choiceByIso.get(it.iso) || "continue";
          const ids = (Array.isArray(it.cardIds) && it.cardIds.length) ? it.cardIds : allFrozenIds;
          applyReturnChoiceToCardIds(state, ids, choice);
        }
        state.attendance.missedPending = [];
        saveState(state);
        toast(tr("تم."));
        close();
      }}
    ]);
  }

  clear(app);

  const hero = h("div",{class:"main-hero"},
    h("button",{class:"play-master btn-active", "data-sfx":"play", ariaLabel:"ابدأ اللعب", onClick:()=> { location.hash = "#/play"; }},
      h("i",{class:"fas fa-play"})
    ),
    h("button",{class:"nav-btn btn-active cta-add-card", "data-sfx":"tap_primary", onClick:()=> nav("#/add")},
      h("i",{class:"fas fa-plus"}), "إضافة بطاقة"
    )
  );

  const menu = h("div",{class:"menu-grid"},
    h("div",{class:"menu-item btn-active", "data-sfx":"naker", onClick:()=> nav("#/library")}, h("i",{class:"fas fa-box-open"}), "رزم البطاقات"),
    h("div",{class:"menu-item btn-active", "data-sfx":"naker", onClick:()=> nav("#/shop")}, h("i",{class:"fas fa-shopping-cart"}), "المتجر"),
    h("div",{class:"menu-item btn-active", "data-sfx":"naker", onClick:()=> nav("#/leaderboard")}, h("i",{class:"fas fa-trophy"}), "التصنيف"),
    h("div",{class:"menu-item btn-active", "data-sfx":"naker", onClick:()=> nav("#/tasks")},
      h("i",{class:"fas fa-tasks"}),
      h("span",{class:"dot", id:"uiTasksDot", hidden:true, style:"top:10px; left:12px;"}),
      "المهمات"
    ),
    h("div",{class:"menu-item btn-active", "data-sfx":"naker", onClick:()=> nav("#/mastery")}, h("i",{class:"fas fa-road"}), "طريق الإتقان"),
    h("div",{class:"menu-item btn-active", "data-sfx":"naker", onClick:()=> nav("#/friends")}, h("i",{class:"fas fa-users"}), "الأصدقاء")
  );

  const panel = h("section",{class:"panel glass gold"},
    h("div",{class:"panel-title"},
      h("h2",{class:"h1"}, "الرئيسية"),
      h("span",{class:"subtle"}, formatDateDMY(todayISO()))
    ),
    hero,
    menu
  );

  app.appendChild(panel);
  
  // ---- First-run beta notice (show once per profile) ----
  if (state.profile && !state.profile.betaNoticeShown) {
    // mark as shown immediately (so it never repeats)
    state.profile.betaNoticeShown = true;
    saveState(state);

    const body = document.createElement("div");

    const ar = document.createElement("div");
    ar.setAttribute("dir", "rtl");
    ar.style.lineHeight = "1.7";
    ar.textContent =
      "تنبيه: التطبيق جديد وما زال تحت الاختبار. إذا واجهت أي مشكلة أو لديك اقتراح مفيد لتحسين التطبيق، ستجد بريد الدعم داخل الإعدادات ويمكنك مراسلتنا عليه.";

    const hr = document.createElement("div");
    hr.className = "hr";

    const en = document.createElement("div");
    en.setAttribute("dir", "ltr");
    en.style.lineHeight = "1.7";
    en.textContent =
      "Notice: This app is new and still under testing. If you face any issue or have a useful suggestion to improve the app, you can find the support email in Settings and contact us.";

    body.append(ar, hr, en);

    modal("تنبيه / Notice", body, [
      {
        label: "حسنًا / OK",
        kind: "primary",
        sfx: "tap_primary",
        onClick: (close) => close(),
      },
    ]);
  }
  // -------------------------------------------------------

  // apply background mode (disabled in modern UI)
  setBackground(state.mode || "normal");

  // mark daily reset
  dailyResetIfNeeded(state);
  seasonCheckAndReset(state);
  saveState(state);

  // SFX is delegated globally via wireSfx(document)
}


function renderAddCard(){
  const g = newDailyGroup(state);
  const isEn = getLang()==="en";

  // Track how many cards were added in this visit (session).
  // IMPORTANT: the minimum rule (4 cards) applies to the whole "today" group,
  // not just this session.
  let sessionAdded = 0;

  // Draft cards are kept in memory only until the minimum (4) is reached for today.
  // They are NOT persisted to LocalStorage.
  let draftCards = [];

  // Remove card ids everywhere they could exist inside the local state.
  // This is used to rollback today's draft cards if the user exits before reaching 4.
  function removeCardIds(ids){
    if(!Array.isArray(ids) || !ids.length) return;
    const idSet = new Set(ids);

    // remove from byId
    for(const id of ids){
      delete state.cards.byId[id];
    }

    // remove from order
    state.cards.order = (state.cards.order||[]).filter(id=> !idSet.has(id));

    // remove from daily groups
    for(const iso of Object.keys(state.cards.dailyGroups||{})){
      const dg = state.cards.dailyGroups[iso];
      if(!dg?.cardIds) continue;
      dg.cardIds = dg.cardIds.filter(id=> !idSet.has(id));
      // if group becomes empty, drop it to keep UI clean
      if(dg.cardIds.length === 0) delete state.cards.dailyGroups[iso];
    }

    // remove from ignored/completed lists
    state.cards.ignored = (state.cards.ignored||[]).filter(id=> !idSet.has(id));
    state.cards.completed = (state.cards.completed||[]).filter(id=> !idSet.has(id));

    // remove from custom packs
    for(const p of (state.library.customPacks||[])){
      if(!p?.cardIds) continue;
      p.cardIds = p.cardIds.filter(id=> !idSet.has(id));
    }

    // remove from lesson caches
    for(const iso of Object.keys(state.attendance.lessonCache||{})){
      const entry = state.attendance.lessonCache[iso];
      const list = Array.isArray(entry) ? entry : entry?.cardIds;
      if(!Array.isArray(list)) continue;
      const next = list.filter(id=> !idSet.has(id));
      if(Array.isArray(entry)) state.attendance.lessonCache[iso] = next;
      else state.attendance.lessonCache[iso] = { ...(entry||{}), cardIds: next };
    }
  }

  // inputs
  // Add-card inputs: Enter should move between fields, and on the 3rd field it should add the card and focus the first field.
  const a = h("input",{type:"text", maxlength:"45", class:"modern-input", id:"original-text", dir:"auto", enterkeyhint:"next", autocomplete:"off", placeholder: isEn?"Original text...":"النص الأصلي...", dir:"auto"});
  const b = h("input",{type:"text", maxlength:"45", class:"modern-input", id:"translation-text", dir:"auto", enterkeyhint:"next", autocomplete:"off", placeholder: isEn?"Translation...":"الترجمة...", dir:"auto"});
  const hint = h("input",{type:"text", maxlength:"60", class:"modern-input", id:"hint-text", dir:"auto", enterkeyhint:"done", autocomplete:"off", placeholder: isEn?"Hint (required)...":"التلميح...", dir:"auto"});

  const counter = h("div",{class:"subtle add-counter", id:"counter"}, "");
  const addedToday = h("div",{class:"add-today"}, "");
  const maxNote = h("div",{class:"subtle", id:"maxNote"}, "");

  function todayGroupCount(){
    const iso = todayISO();
    const dg = state.cards.dailyGroups?.[iso];
    return Array.isArray(dg?.cardIds) ? dg.cardIds.length : 0;
  }

  function effectiveTodayCount(){
    return todayGroupCount() + (draftCards?.length || 0);
  }

  function todayGroupIds(){
    const iso = todayISO();
    const dg = state.cards.dailyGroups?.[iso];
    return Array.isArray(dg?.cardIds) ? [...dg.cardIds] : [];
  }

  function update(){

const st = canAddMoreCardsToday(state);
const saved = todayGroupCount();
const draft = draftCards.length;
const effective = saved + draft;

const effectiveCount = (st.count || 0) + draft;
const can = effectiveCount < (st.max || 0);

const cur = Math.max(1, effective + 1);
counter.textContent = (isEn)
  ? `Card ${cur} of 4 (min)`
  : `البطاقة ${cur} من 4 (الحد الأدنى)`;
addedToday.innerHTML = (isEn)
  ? `Added today: <b style="color:var(--accent-color)">${effectiveCount}</b> / ${st.max}`
  : `عدد المضاف اليوم: <b style="color:var(--accent-color)">${effectiveCount}</b> / ${st.max}`;

btnAdd.disabled = !can;
btnAdd.style.opacity = can ? "1" : "0.6";
if(!can){
  maxNote.textContent = isEn ? "Daily limit reached." : "تم بلوغ الحد اليومي.";
} else {
  maxNote.textContent = "";
}

// finish availability (depends on today's total, including drafts)
const canFinish = effective >= 4;
// Keep the button clickable so it can show the "minimum 4" message + Ekmal.wav.
btnFinish.disabled = false;
btnFinish.setAttribute("aria-disabled", canFinish ? "false" : "true");
btnFinish.style.opacity = canFinish ? "1" : "0.65";
  }

  function commit(){

const A = a.value.trim();
const B = b.value.trim();
const H = hint.value.trim();
if(!A || !B || !H){
  toast(isEn?"Missing fields.":"يرجى تعبئة الحقول الأساسية.");
  // Focus the first missing field for faster keyboard flow
  if(!A) a.focus({preventScroll:true});
  else if(!B) b.focus({preventScroll:true});
  else hint.focus({preventScroll:true});
  return false;
}

const st = canAddMoreCardsToday(state);
const effectiveCount = (st.count || 0) + (draftCards?.length || 0);
if(effectiveCount >= (st.max || 0)){
  toast(isEn?"Daily limit reached.":"تم بلوغ الحد.");
  return false;
}

// If today's minimum (4) is already satisfied in saved data, we can persist immediately.
const saved = todayGroupCount();
if(saved >= 4){
  addCard(state, {a:A, b:B, hint:H});
  saveState(state);
}else{
  // Stage in memory only (NOT persisted) until reaching 4 cards total for today.
  draftCards.push({a:A, b:B, hint:H});
  // Once the minimum is reached, commit all drafts at once.
  if(saved + draftCards.length >= 4){
    const list = draftCards.slice();
    draftCards = [];
    for(const d of list){
      addCard(state, {a:d.a, b:d.b, hint:d.hint});
    }
    saveState(state);
  }
}

sessionAdded += 1;
a.value=""; b.value=""; hint.value="";
update();
// Ready for the next card
a.focus({preventScroll:true});
return true;
  }

  // Keyboard flow (desktop + mobile): Enter moves to next input, and on hint it submits and prepares next card.
  a.addEventListener("keydown", (e)=>{
    if(e.key === "Enter"){
      e.preventDefault();
      b.focus({preventScroll:true});
    }
  });
  b.addEventListener("keydown", (e)=>{
    if(e.key === "Enter"){
      e.preventDefault();
      hint.focus({preventScroll:true});
    }
  });
  hint.addEventListener("keydown", (e)=>{
    if(e.key === "Enter"){
      e.preventDefault();
      commit();
    }
  });

  // header like the prototype
  const headerCardStack = h("div",{class:"avatar-stack", ariaHidden:"true"});
  try{
    const {frame, bg, av} = computeAvatarLayers(state);
    const bgImg = new Image(); bgImg.src = bg?.src || ""; bgImg.className = "layer bg";
    const avImg = new Image(); avImg.src = av?.src || ""; avImg.className = "layer av";
    const frameImg = new Image(); frameImg.src = frame?.src || ""; frameImg.className = "layer frame";
    // Layering per spec: frame (bottom) -> background -> avatar (top)
    headerCardStack.append(frameImg, bgImg, avImg);
  }catch{}

  const header = h("header",{class:"page-header"},
    h("div",{class:"profile-card", "data-sfx":"naker", onClick: ()=> nav("#/profile")},
      headerCardStack,
      h("div",{class:"profile-text"},
        h("div",{class:"profile-name"}, isEn?"Add Card":"إضافة بطاقة"),
        h("div",{class:"user-meta"}, isEn?"Prepare a new card":"تجهيز بطاقة جديدة")
      )
    ),
    h("div",{class:"gold-tag"}, h("i",{class:"fas fa-coins"}), h("span",{class:"gold-val"}, String(Math.max(0, Math.floor(state.economy.gold||0)))))
  );

  const infoRow = h("div",{class:"card-info-row"},
    h("span",{}, isEn?"Date:":"التاريخ:", " ", h("b",{}, g.titleDMY)),
    h("span",{class:"info-link", "data-sfx":"tap_secondary", onClick: (e)=> { e.preventDefault?.(); openHelp(); }},
      h("i",{class:"fas fa-info-circle"}), " ", isEn?"Instructions":"تعليمات"
    )
  );

  // actions (requested layout: 3 buttons in one row, Exit below)
  const btnAdd = h("button",{class:"btn-wide btn-active accent", "data-sfx":"hefth", onClick: ()=> commit()},
    h("i",{class:"fas fa-save"}),
    isEn?"Save":"حفظ"
  );

  const btnFinish = h("button",{class:"btn-wide btn-active neutral", "data-sfx":"enhaa", onClick: ()=> {
    // Dynamic SFX:
    // - before reaching 4 cards => Ekmal.wav
    // - after reaching 4 cards  => Enhaa.wav
    if(todayGroupCount() < 4){
      btnFinish.setAttribute("data-sfx", "ekmal");
      toast(isEn?"Minimum is 4 cards.":"الحد الأدنى 4 بطاقات.");
      return true;
    }
    btnFinish.setAttribute("data-sfx", "enhaa");
    nav("#/home");
    return true;
  }},
    h("i",{class:"fas fa-check"}),
    isEn?"Finish":"إنهاء"
  );

  const btnShop = h("button",{class:"btn-wide btn-active gold", "data-sfx":"naker", onClick: ()=> nav("#/shop")},
    h("i",{class:"fas fa-shopping-cart"}),
    isEn?"Shop":"المتجر"
  );

  const btnExit = h("button",{class:"btn-wide btn-active dark", "data-sfx":"tahther", onClick: ()=> {
    const anyInputs = !!(a.value||b.value||hint.value);
    const tCount = effectiveTodayCount();

    // Nothing to lose
    if(!anyInputs && tCount===0){ nav("#/home"); return; }

    // If today's group is still below the minimum, exiting must rollback ALL today's cards.
    if(tCount > 0 && tCount < 4){
      const body = h("div",{},
        h("div",{class:"subtle"}, isEn
          ? `You added ${tCount} card(s) today. Minimum is 4. If you exit now, today's cards will be deleted.`
          : `أضفت اليوم ${tCount} بطاقة فقط. الحد الأدنى 4. إذا خرجت الآن سيتم حذف بطاقات اليوم.`
        )
      );
      modal(isEn?"Confirm":"تأكيد", body, [
        { label: isEn?"Continue":"متابعة الإضافة", kind:"ghost", onClick:(close)=> close(), sfx:"tap_secondary" },
        { label: isEn?"Delete & exit":"حذف وخروج", kind:"danger", onClick:(close)=> {
            // clear in-memory drafts
            draftCards = [];
            // rollback today's draft cards
            const ids = todayGroupIds();
            removeCardIds(ids);
            // also drop today's group entry (clean)
            try{ delete state.cards.dailyGroups[todayISO()]; }catch{}
            saveState(state);
            a.value=""; b.value=""; hint.value=""; sessionAdded=0;
            close();
            nav("#/home");
          }, sfx:"hathef" },
      ]);
      return;
    }

    // Otherwise: keep saved cards, discard only unsaved inputs
    if(!anyInputs){ nav("#/home"); return; }
    const body = h("div",{}, h("div",{class:"subtle"}, isEn?"Discard unsaved text and exit?":"حذف النص غير المحفوظ والخروج؟"));
    modal(isEn?"Confirm":"تأكيد", body, [
      { label: isEn?"Cancel":"إلغاء", kind:"ghost", onClick:(close)=> close(), sfx:"tap_secondary" },
      { label: isEn?"Discard":"حذف وخروج", kind:"danger", onClick:(close)=> { a.value=""; b.value=""; hint.value=""; close(); nav("#/home"); }, sfx:"hathef" },
    ]);
  }},
    h("i",{class:"fas fa-sign-out-alt"}),
    isEn?"Exit":"خروج"
  );

  const inputs = h("div",{class:"input-group"}, a, b, hint);
  const row = h("div",{class:"action-row"}, btnAdd, btnShop, btnFinish);
  const actions = h("div",{class:"action-stack add-actions"}, row, btnExit, maxNote);

  const wrap = h("div",{class:"add-wrap"},
    header,
    infoRow,
    addedToday,
    inputs,
    counter,
    actions
  );

  update();
  app.appendChild(wrap);
}

function renderPlay(){
  const today = todayISO();
  let due = null;

  // تثبيت بطاقات اليوم حتى لو تغيّرت مستوياتها بعد التقييم
  const cached = state.attendance.lessonCache?.[today];
  const cachedIds = Array.isArray(cached) ? cached : (cached?.cardIds);
  if(Array.isArray(cachedIds) && cachedIds.length){
    due = cachedIds
      .map(id=> state.cards.byId[id])
      .filter(c=> c && !c.ignored && !c.completed && !c.frozen);
    if(due.length === 0) due = null;
  }
  if(!due){
    due = dueCardsForToday(state).filter(c=>!c.frozen);
    state.attendance.lessonCache = state.attendance.lessonCache || {};
    state.attendance.lessonCache[today] = { cardIds: due.map(c=>c.id) };
    saveState(state);
  }
  if(due.length === 0){
    const totalCards = Array.isArray(state?.cards?.order) ? state.cards.order.length : Object.keys(state?.cards?.byId||{}).length;
    const msg = (totalCards <= 0)
      ? "لا توجد بطاقات بعد.\nأضف 4 بطاقات على الأقل لتبدأ."
      : "لا توجد بطاقات لليوم.\nأضف بطاقات جديدة أو عد غدًا.";
    renderSessionEmpty({
      title: "اللعب",
      message: msg,
      icon: "fas fa-layer-group",
      primary: { label: "إضافة بطاقة", sfx:"tap_primary", onClick: ()=> nav("#/add") },
      secondary: { label: "رجوع", kind:"ghost", sfx:"tap_secondary", onClick: ()=> navBack() },
    });
    return;
  }

  // Create session (pending rewards)
  const session = {
    startedAt: performance.now(),
    pending: { xp:0, gold:0, rating:0 },
    perGame: [],
    usedHelps: 0,
    usedSkips: 0,
    completed: false,
    cards: shuffle([...due]),
  };

  // pick 30% for anagram/typing without overlap
  const n30 = Math.max(1, Math.floor(session.cards.length*0.30));
  const anagramSet = new Set(session.cards.slice(0, n30).map(c=>c.id));
  const typingSet = new Set(session.cards.slice(n30, n30*2).map(c=>c.id));

  const games = shuffle(["match","memory","anagram","typing"]);
  let gameIndex = 0;

  const panel = h("section",{class:"glass gold panel"});
  const header = h("div",{class:"panel-title"},
    h("h2",{class:"h1"}, "اللعب"),
    h("div",{class:"hud-metrics"},
      h("span",{class:"subtle", id:"timer"}, "00:00"),
      h("span",{class:"subtle", id:"remain"}, "")
    )
  );
  panel.appendChild(header);

  const stage = h("div",{class:"no-lams"});
  panel.appendChild(stage);
  app.appendChild(panel);

  // header widgets: help/skip/settings (with owned counts)
  const helpBadge = h("span",{class:"icon-badge", ariaLabel:"عدد المساعدة"}, String(state.economy.helps ?? 0));
  const skipBadge = h("span",{class:"icon-badge", ariaLabel:"عدد التخطي"}, String(state.economy.skips ?? 0));

  const helpBtn = h(
    "button",
    { class:"icon-btn", "data-sfx":"musaade", onClick: ()=> onHelp() },
    h("i",{class:"fas fa-wand-magic-sparkles", ariaHidden:"true"}),
    helpBadge
  );
  const skipBtn = h(
    "button",
    { class:"icon-btn", "data-sfx":"takhate", onClick: ()=> onSkip() },
    h("i",{class:"fas fa-forward", ariaHidden:"true"}),
    skipBadge
  );
  const logBtn = h("button",{class:"icon-btn", "data-sfx":"tap_secondary", onClick: ()=> showHelpLog()}, h("i",{class:"fas fa-scroll", ariaHidden:"true"}));
  const setBtn = h("button",{class:"icon-btn", "data-sfx":"naker", onClick: ()=> openSettingsOverlay()}, h("i",{class:"fas fa-sliders", ariaHidden:"true"}));
  header.appendChild(h("div",{class:"top-right"}, helpBtn, logBtn, skipBtn, setBtn));

  function refreshOwnedCounters(){
    helpBadge.textContent = String(state.economy.helps ?? 0);
    skipBadge.textContent = String(state.economy.skips ?? 0);
  }

  window.lucide?.createIcons?.();

  // مؤشر عدد البطاقات المتبقية داخل اللعبة الحالية (مثال: 7/20)
  const remainEl = header.querySelector("#remain");
  function setRemain(done, total){
    if(!remainEl) return;
    const t = Math.max(0, Number(total || 0));
    const d = Math.max(0, Number(done || 0));
    const r = Math.max(0, t - d);
    remainEl.textContent = t ? `${r}/${t}` : "";
  }

  // timer
  let timerId = setInterval(()=>{
    const t = Math.floor((performance.now()-session.startedAt)/1000);
    const mm = String(Math.floor(t/60)).padStart(2,"0");
    const ss = String(t%60).padStart(2,"0");
    header.querySelector("#timer").textContent = `${mm}:${ss}`;
  }, 250);

  // help log (per current game only)
  let helpLog = [];
  let helpSeq = 0;
  let helpHandler = null;
  function openSettingsOverlay(){
    const isEn = getLang()==="en";
    const music = Number(localStorage.getItem("dm_music") ?? "0.65");
    const sfx = Number(localStorage.getItem("dm_sfx") ?? "0.85");

    const m = h("input",{type:"range", min:"0", max:"1", step:"0.01", value:String(music)});
    const s = h("input",{type:"range", min:"0", max:"1", step:"0.01", value:String(sfx)});

    // Vibration toggle
    const vibIcon = h("i",{class: vibOn ? "fas fa-toggle-on" : "fas fa-toggle-off"});
    const vibTxt  = h("span",{class:"vib-txt"}, vibOn ? (isEn?"On":"شغال") : (isEn?"Off":"متوقف"));
    const vibBtn  = h("button",{type:"button", class:`vibrate-pill ${vibOn?"on":"off"} btn-active`, "data-sfx":"tap_secondary", onClick: ()=>{
      vibOn = !vibOn;
      setVibrationEnabled(vibOn);
      vibBtn.classList.toggle("on", vibOn);
      vibBtn.classList.toggle("off", !vibOn);
      vibIcon.className = vibOn ? "fas fa-toggle-on" : "fas fa-toggle-off";
      vibTxt.textContent = vibOn ? (isEn?"On":"شغال") : (isEn?"Off":"متوقف");
      playSfx("tap_secondary");
    }}, vibIcon, vibTxt);

    // Theme toggle
    const thIcon = h("i",{class: darkOn ? "fas fa-moon" : "fas fa-sun"});
    const thTxt  = h("span",{}, darkOn ? (isEn?"Dark":"داكن") : (isEn?"Light":"فاتح"));
    const thBtn  = h("button",{type:"button", class:`theme-pill ${darkOn?"on":"off"} btn-active`, "data-sfx":"tap_secondary", onClick: ()=>{
      darkOn = !darkOn;
      applyTheme(darkOn ? "dark" : "light");
      thBtn.classList.toggle("on", darkOn);
      thBtn.classList.toggle("off", !darkOn);
      thIcon.className = darkOn ? "fas fa-moon" : "fas fa-sun";
      thTxt.textContent = darkOn ? (isEn?"Dark":"داكن") : (isEn?"Light":"فاتح");
      playSfx("tap_secondary");
    }}, thIcon, thTxt);

    const body = h("div",{class:"settings-list"},
      h("div",{class:"setting-item"},
        h("div",{class:"setting-label"}, h("i",{class:"fas fa-music"}), isEn?"Music volume":"مستوى الموسيقى"),
        m
      ),
      h("div",{class:"setting-item"},
        h("div",{class:"setting-label"}, h("i",{class:"fas fa-volume-up"}), isEn?"Interaction volume":"مستوى التفاعلات"),
        s
      ),
      h("div",{class:"setting-item"},
        h("div",{class:"setting-row"},
          h("div",{class:"setting-label"}, h("i",{class:"fas fa-mobile-screen-button"}), isEn?"Tap vibration":"اهتزاز اللمس"),
          vibBtn
        )
      ),
      h("div",{class:"setting-item"},
        h("div",{class:"setting-row"},
          h("div",{class:"setting-label"}, h("i",{class:"fas fa-circle-half-stroke"}), isEn?"Theme":"المظهر"),
          thBtn
        )
      ),
      h("div",{class:"support-email"}, isEn?"Full settings are on Home.":"ملاحظة: الإعدادات الكاملة من الصفحة الرئيسية.")
    );

    modal(isEn?"Quick Settings":"إعدادات سريعة", body, [
      { label:isEn?"Back":"رجوع", kind:"primary", sfx:"tap_secondary", onClick:(close)=> close() },
    ]);

    m.addEventListener("input", ()=>{
      localStorage.setItem("dm_music", m.value);
      setVolumes({music:Number(m.value), sfx:Number(s.value)});
    }, {passive:true});
    s.addEventListener("input", ()=>{
      localStorage.setItem("dm_sfx", s.value);
      setVolumes({music:Number(m.value), sfx:Number(s.value)});
    }, {passive:true});
  }

  function onHelp(){
    const isEn = getLang()==="en";

    // must have a help handler for the current game
    if(!helpHandler || typeof helpHandler.apply !== "function"){
      toast(isEn ? "No help available here." : "لا تتوفر مساعدة هنا.");
      return false;
    }
    if(typeof helpHandler.canApply === "function"){
      // Backward compatible: canApply may return boolean OR {ok:boolean, message?:string}
      const ca = helpHandler.canApply();
      if(ca && typeof ca === "object"){
        if(!ca.ok){
          toast(String(ca.message ?? (isEn ? "No applicable help right now." : "لا توجد مساعدة ممكنة الآن.")));
          return false;
        }
      } else if(!ca){
        toast(isEn ? "No applicable help right now." : "لا توجد مساعدة ممكنة الآن.");
        return false;
      }
    }

    if(state.economy.helps <= 0){
      // No help available => shake + message (no sound)
      try{
        if(window.gsap && typeof gsap.fromTo === "function"){
          gsap.fromTo(helpBtn, {x:0}, {x:8, duration:0.08, yoyo:true, repeat:3});
        } else if(helpBtn?.animate){
          helpBtn.animate(
            [{transform:"translateX(0px)"},{transform:"translateX(8px)"},{transform:"translateX(0px)"}],
            {duration:240, iterations:3, easing:"ease-in-out"}
          );
        }
      }catch{}
      toast(isEn ? "No help left." : "لا توجد مساعدة.");
      return false;
    }

    // Apply first; only consume a help if apply succeeded.
    // Backward compatible: apply may return {noop:true, message?:string} to indicate "no consumption".
    let entryRaw;
    try{
      entryRaw = helpHandler.apply();
    }catch(e){
      toast(isEn ? "Help failed." : "فشلت المساعدة.");
      return false;
    }

    // If handler indicates "noop", do not consume help.
    if(entryRaw && typeof entryRaw === "object" && entryRaw.noop){
      toast(String(entryRaw.message ?? (isEn ? "No applicable help right now." : "لا توجد مساعدة ممكنة الآن.")));
      return false;
    }

    // If handler indicates "noop", do not consume help.
    if(entryRaw && typeof entryRaw === "object" && entryRaw.noop){
      toast(String(entryRaw.message ?? (isEn ? "No applicable help right now." : "لا توجد مساعدة ممكنة الآن.")));
      return false;
    }

    // consume
    state.economy.helps -= 1;
    session.usedHelps += 1;
    saveState(state);
    refreshOwnedCounters();

    // normalize entry for per-game replay
    const entry = (entryRaw && typeof entryRaw === "object" && ("payload" in entryRaw || "title" in entryRaw))
      ? entryRaw
      : { title: isEn ? "Help used" : "مساعدة مستخدمة", payload: String(entryRaw ?? "") };

    entry.id = entry.id ?? (++helpSeq);
    helpLog.push(entry);

    toast(isEn ? "Help used." : "تم استخدام مساعدة.");
    return true;
  }

  function onSkip(){
    if(state.economy.skips <= 0){
      try{
        if(window.gsap && typeof gsap.fromTo === "function"){
          gsap.fromTo(skipBtn, {x:0}, {x:-8, duration:0.08, yoyo:true, repeat:3});
        } else if(skipBtn?.animate){
          skipBtn.animate(
            [{transform:"translateX(0px)"},{transform:"translateX(-8px)"},{transform:"translateX(0px)"}],
            {duration:240, iterations:3, easing:"ease-in-out"}
          );
        }
      }catch{}
      toast("لا يوجد تخطي.");
      return false;
    }
    const body = h("div",{}, h("div",{class:"subtle"}, "تأكيد التخطي؟"));
    modal("تخطي", body, [
      { label:"إلغاء", kind:"ghost", sfx:"naker", onClick:(close)=> close() },
      { label:"تأكيد", kind:"primary", sfx:"none", onClick:(close)=> {
        state.economy.skips -= 1;
        session.usedSkips += 1;
        saveState(state);
        refreshOwnedCounters();
        close();
        // treat current game as perfect and jump forward
        finishGame({ skipped:true, perfect:true });
        return true;
      }},
    ]);
    return true;
  }

  function showHelpLog(){
    const isEn = getLang()==="en";

    if(!helpLog.length){
      modal(isEn ? "Help log" : "سجل المساعدات",
            h("div",{class:"subtle"}, isEn ? "No help used in this game." : "لم يتم استخدام مساعدة في هذه اللعبة."),
            [{ label:isEn ? "Close" : "إغلاق", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close() }]);
      return;
    }

    const list = h("div",{class:"settings-list"});
    helpLog.forEach((entry, idx)=>{
      const title = entry?.title || (isEn ? `Help #${idx+1}` : `مساعدة #${idx+1}`);
      const b = h("button",{class:"btn-wide btn-active", type:"button", "data-sfx":"tap_secondary"});
      b.textContent = `${idx+1}) ${title}`;
      b.addEventListener("click", ()=>{
        try{
          if(helpHandler && typeof helpHandler.replay === "function"){
            helpHandler.replay(entry);
          }else{
            modal(title,
                  h("div",{class:"subtle", style:"white-space:pre-wrap; font-family: ui-monospace, SFMono-Regular, Menlo, monospace;"},
                    (typeof entry?.payload === "string" ? entry.payload : JSON.stringify(entry?.payload ?? {}, null, 2))
                  ),
                  [{ label:isEn ? "Close" : "إغلاق", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close() }]);
          }
        }catch{
          toast(isEn ? "Could not replay help." : "تعذّر إعادة عرض المساعدة.");
        }
      }, {passive:true});
      list.appendChild(b);
    });

    modal(isEn ? "Help log" : "سجل المساعدات", list, [
      { label:isEn ? "Close" : "إغلاق", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close() }
    ]);
  }

  function nextStepPopup(){

    const body = h("div",{}, h("div",{class:"subtle"}, "هل تود الاستمرار؟"));
    modal("استمرار", body, [
      { label:"اللعبة التالية", kind:"primary", sfx:"tap_primary", onClick:(close)=> { close(); gameIndex++; runGame(); } },
      { label:"إنهاء الجلسة", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> { close(); runRating(); } },
    ]);
  }

  function runGame(){
    clear(stage);
    window.lucide?.createIcons?.();
    helpLog = [];
    helpSeq = 0;
    helpHandler = null;
    if(gameIndex >= games.length){
      runRating();
      return;
    }
    const gk = games[gameIndex];

    if(gk === "match") renderMatch();
    if(gk === "memory") renderMemory();
    if(gk === "anagram") renderAnagram();
    if(gk === "typing") renderTyping();
  }

  function finishGame(result){
    // compute rewards for this game
    const avgLevel = Math.round(session.cards.reduce((s,c)=>s+c.level,0)/session.cards.length);
    const priceExtraCard = 100;
    const usedHelp = result.usedHelp || false;
    const base = computeRewardsForGame({
      gameKey: result.gameKey,
      pairsOrCards: result.pairsOrCards,
      successCount: result.successCount,
      wrongCount: result.wrongCount,
      avgSec: result.avgSec,
      usedHelp,
      levelAvg: avgLevel,
      rankKey: state.rating.rankKey,
      subLevel: state.rating.subLevel,
      priceExtraCard
    });

    // Gold tuning: reduce Play rewards by 50% (gold only)
    const playGoldMult = 0.5;
    const tunedGold = Math.max(0, Math.round((base.gold || 0) * playGoldMult));
    // clamp daily gold cap for lessons: 300
    const dailyCap = 300;
    const remaining = Math.max(0, dailyCap - state.economy.dailyGoldEarned);
    const goldGranted = Math.min(tunedGold, remaining);
    state.economy.dailyGoldEarned += goldGranted;

    session.pending.xp += base.xp;
    session.pending.gold += goldGranted;
    session.pending.rating += base.rating;
    session.perGame.push({ ...base, gold: goldGranted, gameKey: result.gameKey });

    // tasks progress (Play only)
    try{
      tasksOnPlayGameFinished(state, {
        gameKey: result.gameKey,
        wrongCount: result.wrongCount || 0,
        usedHelp: !!usedHelp,
        skipped: !!result.skipped,
      });
      saveState(state);
    }catch(_){/* ignore */}

    // popup flow
    if(gameIndex < games.length-1){
      nextStepPopup();
    } else {
      runRating();
    }
  }

  // Game 1: matching
  function countNonSpace(str){
  return String(str ?? "").replace(/\s+/g, "").length;
}

// Reveal next 20% letters (spaces are preserved and not counted)
function build20Mask(answer, enteredCount){
  const s = String(answer ?? "");
  const letterIdx = [];
  for(let i=0;i<s.length;i++){
    if(s[i] !== " ") letterIdx.push(i);
  }
  const totalLetters = letterIdx.length;
  const reveal = Math.max(1, Math.ceil(totalLetters * 0.20));

  const start = Math.min(Math.max(0, enteredCount), totalLetters);
  const end = Math.min(totalLetters, start + reveal);

  const revealSet = new Set(letterIdx.slice(start, end));
  const out = [];
  for(let i=0;i<s.length;i++){
    if(s[i] === " "){
      out.push(" ");
    }else{
      out.push(revealSet.has(i) ? s[i] : "_");
    }
  }
  return out.join("");
}

// Smart 20% hint: start from the first wrong letter position (ignoring spaces, case-insensitive).
// Returns null if the attempt already matches the answer (so help isn't wasted).
// === Unified normalization (MUST be used in check + hint) ===

function clampHintBudget(L){
  const len = Math.max(0, Number(L||0));
  const raw = Math.ceil(0.2 * len);
  return Math.min(4, Math.max(1, raw));
}

// Levenshtein alignment from attempt -> answer, returns ops plus match indices.
// Strings are expected to be ALREADY normalized.
function alignAttemptToAnswer(attempt, answer){
  const T = Array.from(String(attempt ?? ""));
  const A = Array.from(String(answer ?? ""));
  const n = T.length, m = A.length;

  // DP table (n+1)*(m+1)
  const dp = Array.from({length:n+1}, ()=> new Array(m+1).fill(0));
  for(let i=0;i<=n;i++) dp[i][0] = i;
  for(let j=0;j<=m;j++) dp[0][j] = j;
  for(let i=1;i<=n;i++){
    for(let j=1;j<=m;j++){
      const cost = (T[i-1] === A[j-1]) ? 0 : 1;
      const del = dp[i-1][j] + 1;      // delete from attempt
      const ins = dp[i][j-1] + 1;      // insert into attempt (i.e., take from answer)
      const sub = dp[i-1][j-1] + cost; // replace/match
      dp[i][j] = Math.min(del, ins, sub);
    }
  }

  const opsRev = [];
  let i = n, j = m;
  while(i>0 || j>0){
    if(i>0 && j>0){
      const cost = (T[i-1] === A[j-1]) ? 0 : 1;
      if(dp[i][j] === dp[i-1][j-1] + cost){
        opsRev.push({
          type: cost===0 ? "match" : "replace",
          tIndex: i-1,
          aIndex: j-1,
          tChar: T[i-1],
          aChar: A[j-1],
        });
        i--; j--; continue;
      }
    }
    if(j>0 && dp[i][j] === dp[i][j-1] + 1){
      opsRev.push({
        type: "insert",
        tIndex: i,
        aIndex: j-1,
        tChar: "",
        aChar: A[j-1],
      });
      j--; continue;
    }
    if(i>0){
      opsRev.push({
        type: "delete",
        tIndex: i-1,
        aIndex: j, // position *between* answer chars
        tChar: T[i-1],
        aChar: "",
      });
      i--; continue;
    }
  }
  const ops = opsRev.reverse();

  const matchedA = new Set();
  for(const op of ops){
    if(op.type === "match" && typeof op.aIndex === "number") matchedA.add(op.aIndex);
  }
  return { ops, matchedA, A };
}

function pickSpreadPositions(sortedPositions, k){
  const arr = Array.from(sortedPositions||[]);
  if(k <= 0) return [];
  if(arr.length <= k) return arr.slice();
  if(k === 1) return [arr[Math.floor((arr.length-1)/2)]];
  const out = [];
  const step = (arr.length - 1) / (k - 1);
  const used = new Set();
  for(let i=0;i<k;i++){
    const idx = Math.round(i * step);
    used.add(idx);
  }
  for(const idx of used){
    out.push(arr[idx]);
  }
  // ensure correct length (handle rounding collisions)
  out.sort((a,b)=> arr.indexOf(a)-arr.indexOf(b));
  if(out.length < k){
    for(const p of arr){
      if(out.length >= k) break;
      if(!out.includes(p)) out.push(p);
    }
  }
  return out.slice(0, k);
}

// Typing hint: fair distribution over error segments (alignment-based) and never wastes a help.
// Returns {mask, newReveals:number[]} or null if already correct.
function buildTypingHintMask(answerNorm, attemptNorm, alreadyRevealed){
  const A = String(answerNorm ?? "");
  const T = String(attemptNorm ?? "");
  if(T === A) return null;

  const AChars = Array.from(A);
  const L = AChars.length;
  const budget = clampHintBudget(L);
  const revealed = alreadyRevealed instanceof Set ? alreadyRevealed : new Set();

  // Alignment
  const { ops, matchedA } = alignAttemptToAnswer(T, A);

  // Build error segments as contiguous non-match ops.
  const segments = [];
  let cur = null;
  const toAIndex = (op)=>{
    if(!L) return null;
    if(op.type === "match" || op.type === "replace" || op.type === "insert"){
      return Math.min(Math.max(0, op.aIndex), L-1);
    }
    // delete: tie to the current answer position
    const pos = Math.min(Math.max(0, op.aIndex), L);
    return Math.min(pos, L-1);
  };

  for(const op of ops){
    if(op.type === "match"){
      if(cur){ segments.push(cur); cur = null; }
      continue;
    }
    if(!cur) cur = { idxSet: new Set(), min: Infinity, max: -Infinity };
    const ai = toAIndex(op);
    if(typeof ai === "number" && ai >= 0 && ai < L){
      // Never spend reveals on spaces (they are always visible).
      if(AChars[ai] !== " "){
        cur.idxSet.add(ai);
        if(ai < cur.min) cur.min = ai;
        if(ai > cur.max) cur.max = ai;
      }
    }
  }
  if(cur) segments.push(cur);

  // If alignment produces no segments (rare), fall back to spreading over the whole answer.
  if(!segments.length){
    const all = [];
    for(let i=0;i<L;i++) if(AChars[i] !== " ") all.push(i);
    const candidates = all.filter(i=>!revealed.has(i));
    const pick = pickSpreadPositions(candidates, Math.min(budget, candidates.length));
    const newReveals = pick;
    const revealAll = new Set([...revealed, ...newReveals]);
    const mask = AChars.map((ch,i)=> ch === " " ? " " : (matchedA.has(i) || revealAll.has(i) ? ch : "_")).join("");
    return { mask, newReveals };
  }

  // Convert segments to sorted candidate arrays (excluding already revealed)
  const segs = segments.map(s=>{
    const arr = Array.from(s.idxSet).sort((a,b)=>a-b).filter(i=>!revealed.has(i));
    return { arr, min:s.min, max:s.max };
  }).filter(s=>s.arr.length>0);

  if(!segs.length){
    // nothing new to reveal
    const revealAll = new Set([...revealed]);
    const mask = AChars.map((ch,i)=> ch === " " ? " " : (matchedA.has(i) || revealAll.has(i) ? ch : "_")).join("");
    return { mask, newReveals: [] };
  }

  const pickFromSeg = (seg)=>{
    const mid = (seg.min + seg.max) / 2;
    let best = seg.arr[0];
    let bestD = Math.abs(best - mid);
    for(const v of seg.arr){
      const d = Math.abs(v - mid);
      if(d < bestD){ bestD = d; best = v; }
    }
    return best;
  };

  let remaining = budget;
  const chosen = [];

  if(segs.length === 1){
    // Single error segment: spread within it (more useful for empty attempt / one big mismatch)
    const pick = pickSpreadPositions(segs[0].arr, Math.min(remaining, segs[0].arr.length));
    chosen.push(...pick);
    remaining -= pick.length;
  } else {
    // Multiple segments: first give each one char
    for(const seg of segs){
      if(remaining <= 0) break;
      const p = pickFromSeg(seg);
      chosen.push(p);
      seg.arr = seg.arr.filter(x=>x!==p);
      remaining -= 1;
    }
    // Distribute the rest to larger segments
    while(remaining > 0){
      segs.sort((a,b)=> b.arr.length - a.arr.length);
      const seg = segs.find(s=>s.arr.length>0);
      if(!seg) break;
      const p = pickFromSeg(seg);
      chosen.push(p);
      seg.arr = seg.arr.filter(x=>x!==p);
      remaining -= 1;
    }
  }

  const newReveals = chosen.filter(i=>!revealed.has(i));
  const revealAll = new Set([...revealed, ...newReveals]);
  const mask = AChars.map((ch,i)=> ch === " " ? " " : (matchedA.has(i) || revealAll.has(i) ? ch : "_")).join("");
  return { mask, newReveals };
}

function renderMatch(){
    const cards = session.cards;
    const left = shuffle(cards.map(c=>({ id:c.id, type:"a", text:c.a })));
    const right = shuffle(cards.map(c=>({ id:c.id, type:"b", text:c.b })));
    let sel = null;
    let selBtn = null;
    let correct = 0, wrong = 0;
    const matchedIds = new Set();
    let lastT = performance.now();
    const times = [];

    setRemain(0, cards.length);

    const wrap = h("div",{class:"match-wrap"});
    const colA = h("div",{class:"match-col"});
    const colB = h("div",{class:"match-col"});
    wrap.append(colA, colB);

    const makeBtn = (item)=> {
      const b = h("button",{class:"btn match-choice", "data-sfx":"ekhtiar"}, item.text);
      b.dataset.cardid = item.id;
      b.dataset.type = item.type;
      b.addEventListener("click", ()=> pick(item, b), {passive:true});
      return b;
    };

    left.forEach(it=> colA.appendChild(makeBtn(it)));
    right.forEach(it=> colB.appendChild(makeBtn(it)));

    stage.appendChild(h("div",{class:"subtle"}, "التوصيل"));
    stage.appendChild(h("div",{class:"hr"}));
    stage.appendChild(wrap);
// HELP (20%): reveal ONE pair as popup (card + answer). Logged per this game, replayable.
helpHandler = {
  canApply: ()=> {
    const remaining = session.cards.filter(c=>!matchedIds.has(c.id));
    return remaining.length > 0;
  },
  apply: ()=> {
    const isEn = getLang()==="en";
    const remaining = session.cards.filter(c=>!matchedIds.has(c.id));
    if(!remaining.length) throw new Error("No remaining pairs");

    // Always reveal exactly one pair (minimum help)
    const c = remaining[0];

    const body = h("div",{},
      h("div",{class:"subtle"}, isEn ? "Card & Answer" : "البطاقة والجواب"),
      h("div",{class:"hr"}),
      h("div",{style:"font-weight:800; font-size:18px; margin:8px 0 2px;"}, c.a),
      h("div",{class:"subtle", style:"margin:0 0 6px;"}, "→"),
      h("div",{style:"font-weight:800; font-size:18px; margin:2px 0 0;"}, c.b),
    );

    modal(isEn ? "Help" : "مساعدة", body, [
      {label:isEn?"OK":"حسناً", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close()}
    ]);

    return {
      title: isEn ? `Match: revealed 1 pair` : `التوصيل: عرض زوج واحد`,
      payload: { id: c.id, a: c.a, b: c.b }
    };
  },
  replay: (entry)=> {
    const isEn = getLang()==="en";
    const p = entry?.payload || {};
    const body = h("div",{},
      h("div",{class:"subtle"}, isEn ? "Card & Answer" : "البطاقة والجواب"),
      h("div",{class:"hr"}),
      h("div",{style:"font-weight:800; font-size:18px; margin:8px 0 2px;"}, p.a ?? ""),
      h("div",{class:"subtle", style:"margin:0 0 6px;"}, "→"),
      h("div",{style:"font-weight:800; font-size:18px; margin:2px 0 0;"}, p.b ?? ""),
    );
    modal(isEn ? "Help" : "مساعدة", body, [
      {label:isEn?"OK":"حسناً", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close()}
    ]);
  }
};



    function pick(item, btn){
      const now = performance.now();
      const dt = (now-lastT)/1000;
      lastT = now;
      times.push(dt);

      // اختيار/إلغاء اختيار البطاقة الأولى
      if(!sel){
        sel = item;
        selBtn = btn;
        btn.classList.add("selected");
        try{ playSfx("ekhtiar"); }catch{}
        return;
      }
      if(sel.id === item.id && sel.type === item.type){
        selBtn?.classList.remove("selected");
        sel = null;
        selBtn = null;
        try{ playSfx("ekhtiar"); }catch{}
        return;
      }

      const x = sel;
      const bx = selBtn;
      sel = null;
      selBtn = null;

      const y = item;

      bx?.classList.remove("selected");
      btn.classList.remove("selected");

      const ok = x.id === y.id && x.type !== y.type;
      if(ok){
        correct += 1;
        matchedIds.add(x.id);
        playSfx("saheh");
        setRemain(correct, cards.length);
        // remove matched pair buttons (حسب المعرف)
        const aBtn = colA.querySelector(`button[data-cardid="${x.id}"][data-type="a"]`);
        const bBtn = colB.querySelector(`button[data-cardid="${x.id}"][data-type="b"]`);
        aBtn?.remove();
        bBtn?.remove();
      } else {
        wrong += 1;
        playSfx("khata");
        if(vibOn && navigator.vibrate) navigator.vibrate(40);

        [bx, btn].forEach((el)=>{
          if(!el) return;
          el.classList.add("wrong");
          try{
            if(window.gsap && typeof gsap.fromTo === "function"){
              gsap.fromTo(el, {x:0}, {x:10, duration:0.06, yoyo:true, repeat:3});
            } else if(el.animate){
              el.animate(
                [{transform:"translateX(0px)"},{transform:"translateX(10px)"},{transform:"translateX(0px)"}],
                {duration:240, iterations:2, easing:"ease-in-out"}
              );
            }
          }catch{}
          setTimeout(()=> el.classList.remove("wrong"), 450);
        });
      }
      if(colA.children.length === 0){
        const avgSec = average(times);
        finishGame({ gameKey:"match", pairsOrCards: correct, successCount: correct, wrongCount: wrong, avgSec, usedHelp: session.usedHelps>0 });
      }
    }
  }

  // Game 2: memory
  function renderMemory(){
    const cs = session.cards;
    const items = shuffle([
      ...cs.map(c=>({ id:c.id, kind:"a", text:c.a })),
      ...cs.map(c=>({ id:c.id, kind:"b", text:c.b })),
    ]);

    let revealed = [];
    const matched = new Set();
    let correct = 0;
    let lastSuccessT = performance.now();
    const times = [];

    const grid = h("div",{class:"memory-grid"});
    const tiles = [];

    setRemain(0, cs.length);

    const open = new Set(items.map((_,i)=>i));

    items.forEach((it, idx)=>{
      const tile = h("div",{class:"memory-tile open"});
      const inner = h("div",{class:"memory-inner"});
      const front = h("div",{class:"memory-face memory-front"}, "✦");
      const back  = h("div",{class:"memory-face memory-back"}, it.text);
      inner.append(front, back);
      tile.appendChild(inner);

      // show all at start for a few seconds
      tile.classList.add("flipped");

      tile.addEventListener("click", ()=> flip(idx, tile), {passive:true});
      tiles[idx] = tile;
      grid.appendChild(tile);
    });

    stage.appendChild(h("div",{class:"subtle"}, "قلب البطاقات"));
    stage.appendChild(h("div",{class:"hr"}));
    stage.appendChild(grid);
// HELP (20%): show ONE matching pair visually (focus + flip), replayable from log.
// We dim other tiles briefly so user can clearly see the two matching cards.
helpHandler = {
  canApply: ()=> {
    const m = new Map();
    items.forEach((it, i)=>{
      if(matched.has(i)) return;
      const arr = m.get(it.id) || [];
      arr.push(i);
      m.set(it.id, arr);
    });
    for(const arr of m.values()){
      if(arr.length >= 2) return true;
    }
    return false;
  },
  apply: ()=> {
    const isEn = getLang()==="en";
    const m = new Map();
    items.forEach((it, i)=>{
      if(matched.has(i)) return;
      const arr = m.get(it.id) || [];
      arr.push(i);
      m.set(it.id, arr);
    });

    let pickId = null;
    let pair = null;
    for(const [id, arr] of m.entries()){
      if(arr.length >= 2){
        pickId = id;
        pair = arr.slice(0,2);
        break;
      }
    }
    if(!pair) throw new Error("No available pair");

    const [i1,i2] = pair;
    const t1 = tiles[i1], t2 = tiles[i2];
    if(!t1 || !t2) throw new Error("Tiles missing");

    const wasFlipped1 = t1.classList.contains("flipped");
    const wasFlipped2 = t2.classList.contains("flipped");

    // lock interactions + dim others
    grid.classList.add("hint-lock","hint-mode");
    tiles.forEach((t)=>{
      if(!t) return;
      t.classList.remove("hint-focus");
    });

    t1.classList.add("flipped","hint-focus","hint-flash");
    t2.classList.add("flipped","hint-focus","hint-flash");

    setTimeout(()=>{
      t1.classList.remove("hint-flash");
      t2.classList.remove("hint-flash");
      if(!wasFlipped1 && !matched.has(i1)) t1.classList.remove("flipped");
      if(!wasFlipped2 && !matched.has(i2)) t2.classList.remove("flipped");
      t1.classList.remove("hint-focus");
      t2.classList.remove("hint-focus");
      grid.classList.remove("hint-lock","hint-mode");
    }, 1600);

    const card = session.cards.find(c=>c.id===pickId) || {};
    return {
      title: isEn ? "Memory: showed a matching pair" : "قلب البطاقات: إظهار زوج مطابق",
      payload: { id: pickId, a: card.a, b: card.b, i1, i2 }
    };
  },
  replay: (entry)=> {
    const p = entry?.payload || {};
    const i1 = p.i1, i2 = p.i2;
    const t1 = tiles?.[i1], t2 = tiles?.[i2];
    if(!t1 || !t2) return;

    const wasFlipped1 = t1.classList.contains("flipped");
    const wasFlipped2 = t2.classList.contains("flipped");

    grid.classList.add("hint-lock","hint-mode");
    tiles.forEach((t)=>{
      if(!t) return;
      t.classList.remove("hint-focus");
    });

    t1.classList.add("flipped","hint-focus","hint-flash");
    t2.classList.add("flipped","hint-focus","hint-flash");

    setTimeout(()=>{
      t1.classList.remove("hint-flash");
      t2.classList.remove("hint-flash");
      if(!wasFlipped1) t1.classList.remove("flipped");
      if(!wasFlipped2) t2.classList.remove("flipped");
      t1.classList.remove("hint-focus");
      t2.classList.remove("hint-focus");
      grid.classList.remove("hint-lock","hint-mode");
    }, 1600);
  }
};



    setTimeout(()=>{
      open.clear();
      tiles.forEach((t)=>{
        if(!t) return;
        t.classList.remove("open","flipped","selected");
      });
    }, 5000);

    function flip(idx, tile){
      if(matched.has(idx)) return;
      if(open.has(idx)) return;

      // allow user to unselect the same revealed tile
      if(revealed.includes(idx)){
        playSfx("ekhtiar");
        revealed = revealed.filter(x=>x!==idx);
        tile.classList.remove("flipped","selected");
        return;
      }
      if(revealed.length >= 2) return;

      if(revealed.length === 0) playSfx("ekhtiar");
      tile.classList.add("flipped","selected");
      revealed.push(idx);

      if(revealed.length === 2){
        const [i1,i2] = revealed;
        const a = items[i1], b = items[i2];
        const ok = a.id === b.id && a.kind !== b.kind;
        if(ok){
          matched.add(i1); matched.add(i2);
          correct += 1;
          const now = performance.now();
          times.push((now-lastSuccessT)/1000);
          lastSuccessT = now;
          playSfx("saheh");
          setRemain(correct, cs.length);

          setTimeout(()=>{
            const t1 = tiles[i1];
            const t2 = tiles[i2];
            if(t1){ t1.classList.add("matched"); t1.classList.remove("selected"); }
            if(t2){ t2.classList.add("matched"); t2.classList.remove("selected"); }
            revealed = [];
            if(correct >= cs.length){
              const avgSec = average(times.length?times:[6]);
              finishGame({ gameKey:"memory", pairsOrCards: correct, successCount: correct, wrongCount: 0, avgSec, usedHelp: session.usedHelps>0 });
            }
          }, 250);
        } else {
          playSfx("khata");
          setTimeout(()=>{
            const t1 = tiles[i1];
            const t2 = tiles[i2];
            if(t1) t1.classList.remove("flipped","selected");
            if(t2) t2.classList.remove("flipped","selected");
            revealed = [];
          }, 800);
        }
      }
    }

  }

  // Game 3: anagram
  function renderAnagram(){
    const pool = session.cards.filter(c=>anagramSet.has(c.id));
    if(pool.length === 0){
      finishGame({ gameKey:"anagram", pairsOrCards: 0, successCount: 0, wrongCount: 0, avgSec: 6, usedHelp:false });
      return;
    }
    let idx = 0;
    let correct = 0, wrong = 0;
    let lastT = performance.now();
    const times = [];

    const card = h("div",{class:"card glass anagram-card"});
    stage.appendChild(h("div",{class:"subtle"}, "ترتيب الحروف"));
    stage.appendChild(h("div",{class:"hr"}));
    stage.appendChild(card);

    function show(){
      setRemain(idx, pool.length);

      if(idx >= pool.length){
        const avgSec = average(times.length?times:[6]);
        finishGame({ gameKey:"anagram", pairsOrCards: pool.length, successCount: correct, wrongCount: wrong, avgSec, usedHelp: session.usedHelps>0 });
        return;
      }

      const c = pool[idx];
      const isEn = getLang()==="en";

      // Unified normalization for check + hint
      const A = normalizeText(c.a);
      const AChars = Array.from(A);
      const spaceMask = AChars.map(ch=>ch===" ");
      const ALetters = AChars.filter(ch=>ch!==" ");
      const L = ALetters.length;

      // No tiles for spaces: spaces are fixed automatically.
      const bankLetters = shuffle(ALetters.slice());

      // Slots correspond to ALetters (spaces removed)
      const slots = Array.from({length:L}, ()=> null);
      const slotBtn = Array.from({length:L}, ()=> null);
      const locked = Array.from({length:L}, ()=> false);
      const stack = []; // {pos, btn}

      const progress = h("div",{class:"subtle", style:"text-align:center;"}, `(${Math.max(0, pool.length-idx)}/${pool.length})`);
      const answer = h("div",{class:"anagram-answer", dir:"auto"}, "");
      const grid = h("div",{class:"anagram-grid"});

      function buildAttemptString(){
        // Merge spaces back in
        const out = [];
        let k = 0;
        for(let i=0;i<AChars.length;i++){
          if(spaceMask[i]){
            out.push(" ");
          } else {
            out.push(slots[k] ?? "");
            k++;
          }
        }
        return out.join("");
      }

      function renderAnswer(){
        clear(answer);
        let k = 0;
        for(let i=0;i<AChars.length;i++){
          if(spaceMask[i]){
            answer.appendChild(h("span",{class:"anagram-space"}, " "));
          } else {
            const ch = slots[k];
            const isLocked = locked[k] && ch === ALetters[k];
            const span = h("span",{class:`anagram-slot ${isLocked?"locked":""}`}, ch ?? "_");
            answer.appendChild(span);
            k++;
          }
        }
      }

      function update(){
        renderAnswer();
        undoBtn.disabled = stack.length === 0;
      }

      function placeLetter(btn){
        // Find next empty, unlocked slot
        let pos = -1;
        for(let i=0;i<L;i++){
          if(!locked[i] && slots[i] === null){ pos = i; break; }
        }
        if(pos < 0) return;
        slots[pos] = btn._ch;
        slotBtn[pos] = btn;
        stack.push({pos, btn});
        btn.disabled = true;
        btn.classList.add("is-used");
        update();
      }

      const undoBtn = h("button",{class:"btn-wide btn-active anagram-undo","data-sfx":"naker", type:"button", disabled:true, onClick: ()=> {
        if(!stack.length) return;
        const last = stack.pop();
        const pos = last.pos;
        if(locked[pos]){ update(); return; }
        slots[pos] = null;
        slotBtn[pos] = null;
        last.btn.disabled = false;
        last.btn.classList.remove("is-used");
        update();
      }}, h("i",{class:"fas fa-rotate-left", ariaHidden:"true"}), isEn?"Undo":"تراجع");

      // Build bank buttons (letters only; spaces are fixed)
      const bankButtons = [];
      bankLetters.forEach((ch)=>{
        const b = h("button",{class:"anagram-letter btn-active","data-sfx":"ekhtiar", type:"button", onClick: ()=> placeLetter(b)}, ch);
        b._ch = ch;
        bankButtons.push(b);
        grid.appendChild(b);
      });

      function computeWrongPositions(){
        const wrong = [];
        for(let i=0;i<L;i++){
          if((slots[i] ?? "") !== ALetters[i]) wrong.push(i);
        }
        return wrong;
      }

      function spreadPickPositions(wrongPositions, k){
        return pickSpreadPositions(wrongPositions, k);
      }

      function lockCorrectAt(pos){
        const target = ALetters[pos];
        // If already correct in place, just lock it.
        if((slots[pos] ?? "") === target){
          if(!locked[pos]){ locked[pos] = true; return true; }
          return false;
        }

        // Prefer taking the correct letter from the unused bank.
        const bankBtn = bankButtons.find(b=>!b.disabled && b._ch === target);
        if(bankBtn){
          // Return current letter (if any) back to bank.
          const oldBtn = slotBtn[pos];
          if(oldBtn){ oldBtn.disabled = false; oldBtn.classList.remove("is-used"); }
          // Remove any stack entry for this pos
          for(let i=stack.length-1;i>=0;i--){ if(stack[i].pos === pos){ stack.splice(i,1); break; } }
          slots[pos] = target;
          slotBtn[pos] = bankBtn;
          bankBtn.disabled = true;
          bankBtn.classList.add("is-used");
          locked[pos] = true;
          return true;
        }

        // Otherwise swap with another (unlocked) position that holds the needed letter.
        let other = -1;
        for(let i=0;i<L;i++){
          if(i === pos) continue;
          if(!locked[i] && slots[i] === target && ALetters[i] !== target){ other = i; break; }
        }
        if(other >= 0){
          const tmpCh = slots[pos];
          const tmpBtn = slotBtn[pos];
          slots[pos] = slots[other];
          slotBtn[pos] = slotBtn[other];
          slots[other] = tmpCh;
          slotBtn[other] = tmpBtn;
          // Update stack positions
          for(const e of stack){
            if(e.pos === pos) e.pos = other;
            else if(e.pos === other) e.pos = pos;
          }
          locked[pos] = true;
          return true;
        }
        return false;
      }

      // HELP: fair 20% budget, distributed over wrong positions. Never consumes if no effect.
      helpHandler = {
        canApply: ()=> {
          const wrong = computeWrongPositions();
          if(wrong.length === 0){
            return { ok:false, message: isEn ? "Your answer is already correct." : "إجابتك صحيحة، لا توجد مساعدة مطلوبة." };
          }
          return { ok:true };
        },
        apply: ()=> {
          const wrong = computeWrongPositions();
          if(wrong.length === 0){
            return { noop:true, message: isEn ? "Your answer is already correct." : "إجابتك صحيحة، لا توجد مساعدة مطلوبة." };
          }
          const budget = clampHintBudget(L);
          const picks = spreadPickPositions(wrong, Math.min(budget, wrong.length));
          let fixed = 0;
          for(const pos of picks){
            if(lockCorrectAt(pos)) fixed += 1;
          }
          update();
          if(fixed <= 0){
            return { noop:true, message: isEn ? "Cannot apply help now." : "لا يمكن تطبيق المساعدة الآن." };
          }

          const after = buildAttemptString();
          const body = h("div",{},
            h("div",{class:"subtle"}, isEn ? `Locked ${fixed} letter(s)` : `تم تثبيت ${fixed} حرف/حروف`),
            h("div",{class:"hr"}),
            h("div",{class:"bidi-plaintext", dir:"auto", style:"white-space:pre; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size:22px; text-align:center; letter-spacing:1px;"}, after)
          );
          modal(isEn ? "Help" : "مساعدة", body, [
            {label:isEn?"OK":"حسناً", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close()}
          ]);

          return { title: (isEn ? "Anagram: lock letters" : "ترتيب الحروف: تثبيت"), payload: { after, fixed } };
        },
        replay: (entry)=> {
          const after = entry?.payload?.after ?? "";
          const fixed = entry?.payload?.fixed ?? 0;
          const body = h("div",{},
            h("div",{class:"subtle"}, isEn ? `Locked ${fixed} letter(s)` : `تم تثبيت ${fixed} حرف/حروف`),
            h("div",{class:"hr"}),
            h("div",{class:"bidi-plaintext", dir:"auto", style:"white-space:pre; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size:22px; text-align:center; letter-spacing:1px;"}, after)
          );
          modal(isEn ? "Help" : "مساعدة", body, [
            {label:isEn?"OK":"حسناً", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close()}
          ]);
        }
      };

      const okBtn = h("button",{class:"btn-wide btn-active anagram-ok","data-sfx":"none", type:"button", onClick: ()=> check(c, buildAttemptString())},
        h("i",{class:"fas fa-check-circle", ariaHidden:"true"}),
        isEn?"OK":"موافق"
      );

      clear(card);
      card.append(progress, answer, grid, h("div",{class:"anagram-actions"}, undoBtn, okBtn));
      update();
    }

    function check(c, answer){
      const now = performance.now();
      times.push((now-lastT)/1000);
      lastT = now;

      const isEn = getLang()==="en";
      if(normalizeText(answer) === normalizeText(c.a)){
        correct += 1;
        playSfx("saheh");
        idx += 1;
        show();
      } else {
        wrong += 1;
        playSfx("khata");
        toast(isEn ? "Wrong" : "خطأ");
        show();
      }
    }

    show();
  }

  // Game 4: typing
    function renderTyping(){
      const pool = session.cards.filter(c=>typingSet.has(c.id) && !anagramSet.has(c.id));
      if(pool.length === 0){
        finishGame({ gameKey:"typing", pairsOrCards: 0, successCount: 0, wrongCount: 0, avgSec: 6, usedHelp:false });
        return;
      }
      let idx = 0;
      let correct = 0, wrong = 0;
      let lastT = performance.now();
      const times = [];

      const card = h("div",{class:"card glass typing-card"});
      stage.appendChild(h("div",{class:"subtle"}, "الكتابة"));
      stage.appendChild(h("div",{class:"hr"}));
      stage.appendChild(card);

      function show(){
        setRemain(idx, pool.length);

        if(idx >= pool.length){
          const avgSec = average(times.length?times:[6]);
          finishGame({ gameKey:"typing", pairsOrCards: pool.length, successCount: correct, wrongCount: wrong, avgSec, usedHelp: session.usedHelps>0 });
          return;
        }

        const c = pool[idx];

        const progress = h("div",{class:"subtle", style:"text-align:center;"}, `(${Math.max(0, pool.length-idx)}/${pool.length})`);
        const prompt = h("div",{class:"typing-prompt"}, c.b || "");
        const hint = h("div",{class:"typing-hint subtle"}, "اكتب الإجابة ثم اضغط موافق");

        const input = h("input",{type:"text", class:"modern-input typing-input", maxlength:"45", placeholder:"اكتب هنا...", dir:"auto", autocapitalize:"none", autocomplete:"off", spellcheck:"false"});

        // --- HELP (fair 20% budget, alignment-based, never wastes a help) ---
        const revealedSet = new Set();
        helpHandler = {
          canApply: ()=> {
            const isEn = getLang()==="en";
            const A = normalizeText(c.a);
            const T = normalizeText(input.value);
            if(T === A){
              return { ok:false, message: (isEn ? "Your answer is correct. No help needed." : "إجابتك صحيحة، لا توجد مساعدة مطلوبة.") };
            }
            const res = buildTypingHintMask(A, T, revealedSet);
            if(!res || !res.newReveals || res.newReveals.length === 0){
              return { ok:false, message: (isEn ? "No applicable help right now." : "لا يمكن تطبيق المساعدة الآن.") };
            }
            return { ok:true };
          },
          apply: ()=> {
            const isEn = getLang()==="en";
            const A = normalizeText(c.a);
            const T = normalizeText(input.value);
            if(T === A){
              return { noop:true, message: (isEn ? "Your answer is correct. No help needed." : "إجابتك صحيحة، لا توجد مساعدة مطلوبة.") };
            }
            const res = buildTypingHintMask(A, T, revealedSet);
            if(!res || !res.newReveals || res.newReveals.length === 0){
              return { noop:true, message: (isEn ? "No applicable help right now." : "لا يمكن تطبيق المساعدة الآن.") };
            }
            for(const i of res.newReveals) revealedSet.add(i);

            const body = h("div",{},
              h("div",{class:"subtle"}, isEn ? "Hint" : "تلميح"),
              h("div",{class:"hr"}),
              h("div",{class:"bidi-plaintext", dir:"auto", style:"white-space:pre; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size:22px; text-align:center; letter-spacing:1px;"}, res.mask)
            );
            modal(isEn ? "Help" : "مساعدة", body, [
              {label:isEn?"OK":"حسناً", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close()}
            ]);

            return { title: (isEn ? "Typing: hint" : "الكتابة: تلميح"), payload: { mask: res.mask } };
          },
          replay: (entry)=> {
            const isEn = getLang()==="en";
            const mask = entry?.payload?.mask ?? "";
            const body = h("div",{},
              h("div",{class:"subtle"}, isEn ? "Hint" : "تلميح"),
              h("div",{class:"hr"}),
              h("div",{class:"bidi-plaintext", dir:"auto", style:"white-space:pre; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size:22px; text-align:center; letter-spacing:1px;"}, mask)
            );
            modal(isEn ? "Help" : "مساعدة", body, [
              {label:isEn?"OK":"حسناً", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close()}
            ]);
          }
        };

        // Enter to submit
        input.addEventListener("keydown", (e)=>{
          if(e.key === "Enter"){
            e.preventDefault();
            check(c, input.value);
          }
        });

        const okBtn = h("button",{class:"btn-wide btn-active typing-ok", "data-sfx":"none", style:"background: var(--accent-color); color: white; margin-top:12px;", onClick: ()=> check(c, input.value)}, 
          h("i",{class:"fas fa-check-circle", ariaHidden:"true"}), 
          "موافق"
        );

        clear(card);
        card.append(progress, prompt, hint, h("div",{class:"field"}, input), okBtn);

        // Focus for speed
        setTimeout(()=> { try{ input.focus(); }catch{} }, 0);
      }

      function check(c, val){
        const now = performance.now();
        times.push((now-lastT)/1000);
        lastT = now;

        const isEn = getLang()==="en";
        const a = normalizeText(val);
        const b = normalizeText(c.a);

        if(a === b){
          correct += 1;
          playSfx("saheh");
          idx += 1;
          show();
        } else {
          wrong += 1;
          playSfx("khata");
          toast(isEn ? "Wrong" : "خطأ");
        }
      }

      show();
    }

    // Rating game
  function runRating(){
    // New: list-based rating (safer + faster) — selections are editable until save
    clear(stage);
    const allCards = session.cards || [];
    const RATE_LEVELS = new Set([6, 8, 10]);
    const cards = allCards.filter(c => c && RATE_LEVELS.has(Number(c.level)));
    const isEn = getLang() === "en";

    const STR = {
      title: isEn ? "Rating" : "التقييم",
      subtitle: cards.length ? (isEn ? "Rate only Level 6/8/10 cards. Nothing is saved until you press Save." : "قيّم فقط البطاقات بمستوى 6 و8 و10. لن يتم حفظ شيء قبل الضغط على زر الحفظ.") : (isEn ? "No cards to rate today." : "لا توجد بطاقات لتقييمها اليوم."),
      text: isEn ? "Text" : "النص",
      meaning: isEn ? "Meaning" : "المعنى",
      easy: isEn ? "Easy" : "سهل",
      medium: isEn ? "Medium" : "متوسط",
      hard: isEn ? "Hard" : "صعب",
      allEasy: isEn ? "All Easy" : "تحديد الكل: سهل",
      allMedium: isEn ? "All Medium" : "تحديد الكل: متوسط",
      allHard: isEn ? "All Hard" : "تحديد الكل: صعب",
      save: isEn ? "Save & Finish" : "حفظ وإنهاء",
      needAll: isEn ? "Please rate all cards (or use the All buttons)." : "يجب تقييم كل البطاقات (أو استخدم أزرار التحديد الكلي)."
    };

    // CSS.escape helper (for safe query selectors)
    const cssEscape = (s)=>{
      try{ return (window.CSS && CSS.escape) ? CSS.escape(String(s)) : String(s); }catch{ return String(s); }
    };

    // draft ratings: cardId -> "easy"|"medium"|"hard"
    const draft = Object.create(null);

    const page = h("section",{class:"rating-page"});
    const head = h("div",{class:"rating-head"},
      h("div",{class:"subtle rating-title"}, STR.title),
      h("div",{class:"subtle rating-subtitle"}, STR.subtitle),
    );

    const list = h("div",{class:"rating-list"});

    // Only rate higher-level cards (6/8/10). If none today, keep the flow (attendance) and show a message.
    if(cards.length === 0){
      const msg = h("div",{class:"glass gold panel center", style:"text-align:center; padding:18px; margin-top:12px;"},
        h("div",{class:"subtle"}, isEn ? "No cards to rate today." : "لا توجد بطاقات لتقييمها اليوم.")
      );
      const btn = h("button",{class:"btn primary rating-save", "data-sfx":"tap_primary", onClick: finalizeLesson}, isEn ? "Finish" : "إنهاء");
      const footOnly = h("div",{class:"rating-footer"}, btn);
      page.append(head, msg, footOnly);
      stage.appendChild(page);
      return;
    }


    function ratingChoice(cardId, key, label, iconCls, sfx){
      return h(
        "button",
        {
          class:`rating-choice ${key} btn-active`,
          "data-sfx": sfx,
          onClick: ()=>{
            draft[cardId] = key;
            const row = document.querySelector(`[data-rate-row="${cssEscape(cardId)}"]`);
            if(row){
              row.querySelectorAll(".rating-choice").forEach(b=> b.classList.remove("selected"));
              const btn = row.querySelector(`.rating-choice.${key}`);
              if(btn) btn.classList.add("selected");
            }
          }
        },
        h("i", { class: iconCls, ariaHidden:"true" }),
        h("div", { class:"rating-label" }, label),
      );
    }

    function rowForCard(c, idx){
      const row = h("div",{class:"rating-item glass gold", "data-rate-row": c.id});

      const top = h("div",{class:"rating-item-top"},
        h("div",{class:"rating-count"}, `${idx+1} / ${cards.length}`),
        h("div",{class:"kbd rating-hint"}, c.hint || "")
      );

      const reveal = h(
        "div",
        { class:"rating-reveal" },
        h(
          "button",
          { class:"btn small btn-active", "data-sfx":"tap_secondary", onClick: ()=> modal(STR.text, h("div",{class:"subtle"}, c.a), []) },
          h("i", { class:"fas fa-file-lines", ariaHidden:"true" }),
          h("span", {}, STR.text)
        ),
        h(
          "button",
          { class:"btn small btn-active", "data-sfx":"tap_secondary", onClick: ()=> modal(STR.meaning, h("div",{class:"subtle"}, c.b), []) },
          h("i", { class:"fas fa-language", ariaHidden:"true" }),
          h("span", {}, STR.meaning)
        ),
      );

      const choices = h(
        "div",
        { class:"rating-choices" },
        ratingChoice(c.id, "easy", STR.easy, "fas fa-smile", "sahel"),
        ratingChoice(c.id, "medium", STR.medium, "fas fa-meh", "wasat"),
        ratingChoice(c.id, "hard", STR.hard, "fas fa-dizzy", "saab"),
      );

      row.append(top, reveal, choices);
      return row;
    }

    cards.forEach((c,i)=> list.appendChild(rowForCard(c,i)));

    const bulk = h("div",{class:"rating-bulk"},
      h("button",{class:"btn small btn-active", "data-sfx":"tap_secondary", onClick: ()=> setAll("easy")}, STR.allEasy),
      h("button",{class:"btn small btn-active", "data-sfx":"tap_secondary", onClick: ()=> setAll("medium")}, STR.allMedium),
      h("button",{class:"btn small btn-active", "data-sfx":"tap_secondary", onClick: ()=> setAll("hard")}, STR.allHard),
    );

    const saveBtn = h("button",{class:"btn primary rating-save", "data-sfx":"tap_primary", onClick: saveAll}, STR.save);
    const foot = h("div",{class:"rating-footer"}, bulk, saveBtn);

    page.append(head, list, foot);
    stage.appendChild(page);

    function setAll(v){
      cards.forEach(c=> { draft[c.id] = v; });
      cards.forEach(c=>{
        const row = document.querySelector(`[data-rate-row="${cssEscape(c.id)}"]`);
        if(!row) return;
        row.querySelectorAll(".rating-choice").forEach(b=> b.classList.remove("selected"));
        const btn = row.querySelector(`.rating-choice.${v}`);
        if(btn) btn.classList.add("selected");
      });
    }

    function firstMissing(){
      for(const c of cards){
        if(!draft[c.id]) return c;
      }
      return null;
    }

    function applyRatingToCard(c, v){
      c.lastRating = v;

      if(v === "hard"){
        c.level = 0;
        c.progressDays = 0;
      } else if(v === "medium"){
        // في لعبة التقييم لا تظهر إلا مستويات: 1/3/6/8/10 (حسب جدول الإتقان الجديد)
        if(c.level === 1 || c.level === 3){
          c.level = 0;
          c.progressDays = 0;
        } else {
          // 6/8/10 → 5
          c.level = 5;
          c.progressDays = 5;
        }
      } else {
        // easy
        // لا تُكمل البطاقة فوراً عند "سهل". الإكمال يتم تلقائياً عند progressDays >= 31
        // داخل منطق التطور اليومي (حتى تظهر البطاقة في اليوم 30 ثم تُصبح مكتملة بعدها).
      }
      c.lastReviewedIso = todayISO();
    }

    function saveAll(){
      if(cards.length === 0){
        finalizeLesson();
        return;
      }

      const miss = firstMissing();
      if(miss){
        toast(STR.needAll);
        try{
          const el = document.querySelector(`[data-rate-row="${cssEscape(miss.id)}"]`);
          if(el) el.scrollIntoView({behavior:"smooth", block:"center"});
        }catch{}
        return;
      }

      let easyCount = 0;
      for(const c of cards){
        const v = draft[c.id];
        if(v === "easy") easyCount += 1;
        applyRatingToCard(c, v);
      }

      // accuracy (apply once)
      state.rating.accuracySeason.total += cards.length;
      state.rating.accuracySeason.correct += easyCount;

      finalizeLesson();
    }
  }


  function finalizeLesson(){
    clearInterval(timerId);
    // apply pending rewards
    state.economy.xp += session.pending.xp;
    state.economy.gold += session.pending.gold;
    // lifetime earned gold (achievements)
    state.economy.totalGoldEarned = Math.max(0, Number(state.economy.totalGoldEarned||0) + Number(session.pending.gold||0));

    const beforeRank = rankLabel(state.rating.rankKey, state.rating.subLevel);
    const beforeLevel = computeLevelFromXp(state.economy.xp - session.pending.xp).level;

    applyRatingDelta(state, session.pending.rating);

    recordAttendance(state);

    // tasks/achievements (Play only)
    try{
      const durationSec = Math.max(0, (performance.now() - session.startedAt) / 1000);
      tasksOnPlayLessonFinished(state, { durationSec, cardsPlayed: session.cards.length });
      try{ masteryMarkPlayFinished(state, { durationSec, cardsPlayed: session.cards.length }); }catch(_){ }
    }catch(_){/* ignore */}

    // notification: reward
    addNotification(state, { type:"reward", title:"تم تسجيل الحضور", body:`+${session.pending.gold} ذهب`, claimable:false });

    saveState(state);
    syncTopbar();

    // reward summary with count-up
    const box = h("div",{class:"glass gold panel center"},
      h("h2",{class:"h1"}, "الجوائز"),
      h("div",{class:"hr"}),
      h("div",{class:"card-list"},
        h("div",{class:"glass gold card"}, h("div",{class:"subtle"}, "XP"), h("div",{style:"font-size:24px;font-weight:900;", id:"xpN"}, "0")),
        h("div",{class:"glass gold card"}, h("div",{class:"subtle"}, "الذهب"), h("div",{style:"font-size:24px;font-weight:900;", id:"gN"}, "0")),
        h("div",{class:"glass gold card"}, h("div",{class:"subtle"}, "التصنيف"), h("div",{style:"font-size:24px;font-weight:900;", id:"rN"}, "0")),
      ),
      h("div",{class:"hr"}),
      h("button",{class:"btn primary", "data-sfx":"tap_primary", onClick: ()=> nav("#/home")}, "عودة")
    );
    clear(app);
    app.appendChild(box);

    // confetti
    try{
      confetti({ particleCount: 120, spread: 65, origin:{y:0.75} });
      playSfx("levelup");
    }catch{}

    // count-up
    const xpEl = document.getElementById("xpN");
    const gEl = document.getElementById("gN");
    const rEl = document.getElementById("rN");
    countUp(xpEl, session.pending.xp);
    setTimeout(()=> countUp(gEl, session.pending.gold), 420);
    setTimeout(()=> countUp(rEl, session.pending.rating), 860);

    // level up notification
    const afterLevel = computeLevelFromXp(state.economy.xp).level;
    if(afterLevel > beforeLevel){
      addNotification(state, { type:"level", title:"ترقية مستوى", body:`المستوى ${afterLevel}`, claimable:false });
      saveState(state);
    }
  }

  runGame();
}

// ===== تدريب (من رزم البطاقات) =====
const TRAINING_KEY = "dm_training_payload_v1";
function setTrainingPayload(payload){
  try{ localStorage.setItem(TRAINING_KEY, JSON.stringify(payload)); }catch{}
}
function getTrainingPayload(){
  try{
    const raw = localStorage.getItem(TRAINING_KEY);
    if(!raw) return null;
    return JSON.parse(raw);
  }catch{ return null; }
}
function clearTrainingPayload(){
  try{ localStorage.removeItem(TRAINING_KEY); }catch{}
}

function renderTraining(){
  const payload = getTrainingPayload();
  const ids = payload?.ids || [];
  const cards = ids.map(id=>state.cards.byId[id]).filter(Boolean);

  if(cards.length < 4){
    clearTrainingPayload();
    renderSessionEmpty({
      title: "تدريب",
      message: "لا توجد بطاقات كافية للتدريب.\nاختر 4 بطاقات على الأقل من رزم البطاقات.",
      icon: "fas fa-dumbbell",
      primary: { label: "رزم البطاقات", sfx:"tap_primary", onClick: ()=> nav("#/library") },
      secondary: { label: "رجوع", kind:"ghost", sfx:"tap_secondary", onClick: ()=> navBack() },
    });
    return;
  }

  // جلسة تدريب: 4 ألعاب فقط (بدون تقييم)، لا حضور، لا تغيير مستويات
  const session = {
    startedAt: performance.now(),
    pending: { xp:0, gold:0, rating:0 },
    perGame: [],
    usedHelps: 0,
    usedSkips: 0,
    cards: shuffle([...cards]),
  };

  // pick 30% for anagram/typing without overlap
  const n30 = Math.max(1, Math.floor(session.cards.length*0.30));
  const anagramSet = new Set(session.cards.slice(0, n30).map(c=>c.id));
  const typingSet = new Set(session.cards.slice(n30, n30*2).map(c=>c.id));

  const games = shuffle(["match","memory","anagram","typing"]);
  let gameIndex = 0;

  const panel = h("section",{class:"glass gold panel"});
  const header = h("div",{class:"panel-title"},
    h("h2",{class:"h1"}, "تدريب"),
    h("div",{class:"hud-metrics"},
      h("span",{class:"subtle", id:"timer"}, "00:00"),
      h("span",{class:"subtle", id:"remain"}, "")
    )
  );
  panel.appendChild(header);

  const stage = h("div",{class:"no-lams"});
  panel.appendChild(stage);
  app.appendChild(panel);

  // header widgets: help/skip/settings (with owned counts)
  const helpBadge = h("span",{class:"icon-badge", ariaLabel:"عدد المساعدة"}, String(state.economy.helps ?? 0));
  const skipBadge = h("span",{class:"icon-badge", ariaLabel:"عدد التخطي"}, String(state.economy.skips ?? 0));

  const helpBtn = h(
    "button",
    { class:"icon-btn", "data-sfx":"musaade", onClick: ()=> onHelp() },
    h("i",{class:"fas fa-wand-magic-sparkles", ariaHidden:"true"}),
    helpBadge
  );
  const skipBtn = h(
    "button",
    { class:"icon-btn", "data-sfx":"takhate", onClick: ()=> onSkip() },
    h("i",{class:"fas fa-forward", ariaHidden:"true"}),
    skipBadge
  );
  const logBtn = h("button",{class:"icon-btn", "data-sfx":"tap_secondary", onClick: ()=> showHelpLog()}, h("i",{class:"fas fa-scroll", ariaHidden:"true"}));
  const setBtn = h("button",{class:"icon-btn", "data-sfx":"naker", onClick: ()=> openSettingsOverlay()}, h("i",{class:"fas fa-sliders", ariaHidden:"true"}));
  header.appendChild(h("div",{class:"top-right"}, helpBtn, logBtn, skipBtn, setBtn));

  function refreshOwnedCounters(){
    helpBadge.textContent = String(state.economy.helps ?? 0);
    skipBadge.textContent = String(state.economy.skips ?? 0);
  }

  window.lucide?.createIcons?.();

  // مؤشر عدد البطاقات المتبقية داخل اللعبة الحالية (مثال: 7/20)
  const remainEl = header.querySelector("#remain");
  function setRemain(done, total){
    if(!remainEl) return;
    const t = Math.max(0, Number(total || 0));
    const d = Math.max(0, Number(done || 0));
    const r = Math.max(0, t - d);
    remainEl.textContent = t ? `${r}/${t}` : "";
  }

  // timer
  let timerId = setInterval(()=>{
    const t = Math.floor((performance.now()-session.startedAt)/1000);
    const mm = String(Math.floor(t/60)).padStart(2,"0");
    const ss = String(t%60).padStart(2,"0");
    header.querySelector("#timer").textContent = `${mm}:${ss}`;
  }, 250);

  // help log (per current game only)
  let helpLog = [];
  let helpSeq = 0;
  let helpHandler = null;
  function openSettingsOverlay(){
    const isEn = getLang()==="en";
    const music = Number(localStorage.getItem("dm_music") ?? "0.65");
    const sfx = Number(localStorage.getItem("dm_sfx") ?? "0.85");

    const m = h("input",{type:"range", min:"0", max:"1", step:"0.01", value:String(music)});
    const s = h("input",{type:"range", min:"0", max:"1", step:"0.01", value:String(sfx)});

    // Vibration toggle
    const vibIcon = h("i",{class: vibOn ? "fas fa-toggle-on" : "fas fa-toggle-off"});
    const vibTxt  = h("span",{class:"vib-txt"}, vibOn ? (isEn?"On":"شغال") : (isEn?"Off":"متوقف"));
    const vibBtn  = h("button",{type:"button", class:`vibrate-pill ${vibOn?"on":"off"} btn-active`, "data-sfx":"tap_secondary", onClick: ()=>{
      vibOn = !vibOn;
      setVibrationEnabled(vibOn);
      vibBtn.classList.toggle("on", vibOn);
      vibBtn.classList.toggle("off", !vibOn);
      vibIcon.className = vibOn ? "fas fa-toggle-on" : "fas fa-toggle-off";
      vibTxt.textContent = vibOn ? (isEn?"On":"شغال") : (isEn?"Off":"متوقف");
      playSfx("tap_secondary");
    }}, vibIcon, vibTxt);

    // Theme toggle
    const thIcon = h("i",{class: darkOn ? "fas fa-moon" : "fas fa-sun"});
    const thTxt  = h("span",{}, darkOn ? (isEn?"Dark":"داكن") : (isEn?"Light":"فاتح"));
    const thBtn  = h("button",{type:"button", class:`theme-pill ${darkOn?"on":"off"} btn-active`, "data-sfx":"tap_secondary", onClick: ()=>{
      darkOn = !darkOn;
      applyTheme(darkOn ? "dark" : "light");
      thBtn.classList.toggle("on", darkOn);
      thBtn.classList.toggle("off", !darkOn);
      thIcon.className = darkOn ? "fas fa-moon" : "fas fa-sun";
      thTxt.textContent = darkOn ? (isEn?"Dark":"داكن") : (isEn?"Light":"فاتح");
      playSfx("tap_secondary");
    }}, thIcon, thTxt);

    const body = h("div",{class:"settings-list"},
      h("div",{class:"setting-item"},
        h("div",{class:"setting-label"}, h("i",{class:"fas fa-music"}), isEn?"Music volume":"مستوى الموسيقى"),
        m
      ),
      h("div",{class:"setting-item"},
        h("div",{class:"setting-label"}, h("i",{class:"fas fa-volume-up"}), isEn?"Interaction volume":"مستوى التفاعلات"),
        s
      ),
      h("div",{class:"setting-item"},
        h("div",{class:"setting-row"},
          h("div",{class:"setting-label"}, h("i",{class:"fas fa-mobile-screen-button"}), isEn?"Tap vibration":"اهتزاز اللمس"),
          vibBtn
        )
      ),
      h("div",{class:"setting-item"},
        h("div",{class:"setting-row"},
          h("div",{class:"setting-label"}, h("i",{class:"fas fa-circle-half-stroke"}), isEn?"Theme":"المظهر"),
          thBtn
        )
      ),
      h("div",{class:"support-email"}, isEn?"Full settings are on Home.":"ملاحظة: الإعدادات الكاملة من الصفحة الرئيسية.")
    );

    modal(isEn?"Quick Settings":"إعدادات سريعة", body, [
      { label:isEn?"Back":"رجوع", kind:"primary", sfx:"tap_secondary", onClick:(close)=> close() },
    ]);

    m.addEventListener("input", ()=>{
      localStorage.setItem("dm_music", m.value);
      setVolumes({music:Number(m.value), sfx:Number(s.value)});
    }, {passive:true});
    s.addEventListener("input", ()=>{
      localStorage.setItem("dm_sfx", s.value);
      setVolumes({music:Number(m.value), sfx:Number(s.value)});
    }, {passive:true});
  }

  function onHelp(){
    const isEn = getLang()==="en";

    // must have a help handler for the current game
    if(!helpHandler || typeof helpHandler.apply !== "function"){
      toast(isEn ? "No help available here." : "لا تتوفر مساعدة هنا.");
      return false;
    }
    if(typeof helpHandler.canApply === "function"){
      // Backward compatible: canApply may return boolean OR {ok:boolean, message?:string}
      const ca = helpHandler.canApply();
      if(ca && typeof ca === "object"){
        if(!ca.ok){
          toast(String(ca.message ?? (isEn ? "No applicable help right now." : "لا توجد مساعدة ممكنة الآن.")));
          return false;
        }
      } else if(!ca){
        toast(isEn ? "No applicable help right now." : "لا توجد مساعدة ممكنة الآن.");
        return false;
      }
    }

    if((state.economy.helps ?? 0) <= 0){
      toast(isEn ? "No help left." : "لا توجد مساعدة.");
      return false;
    }

    // Apply first; only consume a help if apply succeeded
    let entryRaw;
    try{
      entryRaw = helpHandler.apply();
    }catch(e){
      toast(isEn ? "Help failed." : "فشلت المساعدة.");
      return false;
    }

    // If handler indicates "noop", do not consume help.
    if(entryRaw && typeof entryRaw === "object" && entryRaw.noop){
      toast(String(entryRaw.message ?? (isEn ? "No applicable help right now." : "لا توجد مساعدة ممكنة الآن.")));
      return false;
    }

    // consume
    state.economy.helps -= 1;
    session.usedHelps += 1;
    saveState(state);
    refreshOwnedCounters();

    const entry = (entryRaw && typeof entryRaw === "object" && ("payload" in entryRaw || "title" in entryRaw))
      ? entryRaw
      : { title: isEn ? "Help used" : "مساعدة مستخدمة", payload: String(entryRaw ?? "") };

    entry.id = entry.id ?? (++helpSeq);
    helpLog.push(entry);

    toast(isEn ? "Help used." : "تم استخدام مساعدة.");
    return true;
  }


  function onSkip(){
    if((state.economy.skips ?? 0) <= 0){
      toast((getLang()==="en") ? "No skips left." : "لا توجد محاولات تخطي.");
      return false;
    }
    const body = h("div",{}, h("div",{class:"subtle"}, (getLang()==="en") ? "Confirm skip?" : "تأكيد التخطي؟"));
    modal((getLang()==="en") ? "Skip" : "تخطي", body, [
      { label:(getLang()==="en") ? "Cancel" : "إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() },
      { label:(getLang()==="en") ? "Confirm" : "تأكيد", kind:"primary", sfx:"none", onClick:(close)=> {
        state.economy.skips -= 1;
        session.usedSkips += 1;
        saveState(state);
        refreshOwnedCounters();
        close();
        finishGame({ skipped:true, perfect:true });
        return true;
      }},
    ]);
    return true;
  }

  function showHelpLog(){
    const isEn = getLang()==="en";

    if(!helpLog.length){
      modal(isEn ? "Help log" : "سجل المساعدات",
            h("div",{class:"subtle"}, isEn ? "No help used in this game." : "لم يتم استخدام مساعدة في هذه اللعبة."),
            [{ label:isEn ? "Close" : "إغلاق", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close() }]);
      return;
    }

    const list = h("div",{class:"settings-list"});
    helpLog.forEach((entry, idx)=>{
      const title = entry?.title || (isEn ? `Help #${idx+1}` : `مساعدة #${idx+1}`);
      const b = h("button",{class:"btn-wide btn-active", type:"button", "data-sfx":"tap_secondary"});
      b.textContent = `${idx+1}) ${title}`;
      b.addEventListener("click", ()=>{
        try{
          if(helpHandler && typeof helpHandler.replay === "function"){
            helpHandler.replay(entry);
          }else{
            modal(title,
                  h("div",{class:"subtle", style:"white-space:pre-wrap; font-family: ui-monospace, SFMono-Regular, Menlo, monospace;"},
                    (typeof entry?.payload === "string" ? entry.payload : JSON.stringify(entry?.payload ?? {}, null, 2))
                  ),
                  [{ label:isEn ? "Close" : "إغلاق", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close() }]);
          }
        }catch{
          toast(isEn ? "Could not replay help." : "تعذّر إعادة عرض المساعدة.");
        }
      }, {passive:true});
      list.appendChild(b);
    });

    modal(isEn ? "Help log" : "سجل المساعدات", list, [
      { label:isEn ? "Close" : "إغلاق", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close() }
    ]);
  }

  function nextStepPopup(){

    const body = h("div",{}, h("div",{class:"subtle"}, "هل تود الاستمرار؟"));
    modal("استمرار", body, [
      { label:"اللعبة التالية", kind:"primary", sfx:"tap_primary", onClick:(close)=> { close(); gameIndex++; runGame(); } },
      { label:"إنهاء التدريب", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> { close(); finalizeTraining(); } },
    ]);
  }

  function runGame(){
    clear(stage);
    window.lucide?.createIcons?.();
    helpLog = [];
    helpSeq = 0;
    helpHandler = null;
    if(gameIndex >= games.length){
      finalizeTraining();
      return;
    }
    const gk = games[gameIndex];

    if(gk === "match") renderMatch();
    if(gk === "memory") renderMemory();
    if(gk === "anagram") renderAnagram();
    if(gk === "typing") renderTyping();
  }

  function finishGame(result){
    // إذا تم التخطي، نعتبر اللعبة مثالية بزمن سريع وبدون أخطاء
    if(result.skipped){
      const fastSec = 4.5;
      const avgLevel = Math.round(session.cards.reduce((s,c)=>s+c.level,0)/session.cards.length);
      const base = computeRewardsForGame({
        gameKey: games[gameIndex],
        pairsOrCards: session.cards.length,
        successCount: session.cards.length,
        wrongCount: 0,
        avgSec: fastSec,
        usedHelp: false,
        levelAvg: avgLevel,
        rankKey: state.rating.rankKey,
        subLevel: state.rating.subLevel,
        priceExtraCard: 100
      });
      // Training tuning:
      // - keep XP/Rating at 50% of Play (as before)
      // - reduce Gold by an additional 50% (i.e., 25% of the base Play formula)
      const mult = 0.5;
      const goldMult = 0.25;
      const scaled = {
        xp: Math.round((base.xp || 0) * mult),
        gold: Math.round((base.gold || 0) * goldMult),
        rating: Math.round((base.rating || 0) * mult),
      };
      const dailyCap = 300;
      const remaining = Math.max(0, dailyCap - state.economy.dailyGoldEarned);
      const goldGranted = Math.min(scaled.gold, remaining);
      state.economy.dailyGoldEarned += goldGranted;
      session.pending.xp += scaled.xp;
      session.pending.gold += goldGranted;
      session.pending.rating += scaled.rating;
      session.perGame.push({ ...base, ...scaled, gold: goldGranted, gameKey: games[gameIndex] });
      if(gameIndex < games.length-1) nextStepPopup(); else finalizeTraining();
      return;
    }

    const avgLevel = Math.round(session.cards.reduce((s,c)=>s+c.level,0)/session.cards.length);
    const priceExtraCard = 100;
    const usedHelp = result.usedHelp || false;
    const base = computeRewardsForGame({
      gameKey: result.gameKey,
      pairsOrCards: result.pairsOrCards,
      successCount: result.successCount,
      wrongCount: result.wrongCount,
      avgSec: result.avgSec,
      usedHelp,
      levelAvg: avgLevel,
      rankKey: state.rating.rankKey,
      subLevel: state.rating.subLevel,
      priceExtraCard
    });

    // Training tuning:
    // - keep XP/Rating at 50% of Play (as before)
    // - reduce Gold by an additional 50% (i.e., 25% of the base Play formula)
    const mult = 0.5;
    const goldMult = 0.25;
    const scaled = {
      xp: Math.round((base.xp || 0) * mult),
      gold: Math.round((base.gold || 0) * goldMult),
      rating: Math.round((base.rating || 0) * mult),
    };

    // نفس سقف ذهب اليوم لمنع الاستغلال
    const dailyCap = 300;
    const remaining = Math.max(0, dailyCap - state.economy.dailyGoldEarned);
    const goldGranted = Math.min(scaled.gold, remaining);
    state.economy.dailyGoldEarned += goldGranted;

    session.pending.xp += scaled.xp;
    session.pending.gold += goldGranted;
    session.pending.rating += scaled.rating;
    session.perGame.push({ ...base, ...scaled, gold: goldGranted, gameKey: result.gameKey });

    if(gameIndex < games.length-1){
      nextStepPopup();
    } else {
      finalizeTraining();
    }
  }

  function finalizeTraining(){
    clearInterval(timerId);

    // تطبيق الجوائز مباشرة (بدون حضور وبدون تقييم)
    state.economy.xp += session.pending.xp;
    state.economy.gold += session.pending.gold;
    state.economy.totalGoldEarned = Math.max(0, Number(state.economy.totalGoldEarned||0) + Number(session.pending.gold||0));
    applyRatingDelta(state, session.pending.rating);
    
    // mastery path
    try{ masteryMarkTrainingFinished(state, { durationSec: Math.max(0, (performance.now() - session.startedAt) / 1000), cardsPlayed: (session.cards||[]).length }); }catch(_){ }

    saveState(state);
    syncTopbar();
    clearTrainingPayload();

    const box = h("div",{class:"glass gold panel center"},
      h("h2",{class:"h1"}, "الجوائز"),
      h("div",{class:"hr"}),
      h("div",{class:"card-list"},
        h("div",{class:"glass gold card"}, h("div",{class:"subtle"}, "XP"), h("div",{style:"font-size:24px;font-weight:900;", id:"xpN"}, "0")),
        h("div",{class:"glass gold card"}, h("div",{class:"subtle"}, "الذهب"), h("div",{style:"font-size:24px;font-weight:900;", id:"gN"}, "0")),
        h("div",{class:"glass gold card"}, h("div",{class:"subtle"}, "التصنيف"), h("div",{style:"font-size:24px;font-weight:900;", id:"rN"}, "0")),
      ),
      h("div",{class:"hr"}),
      h("button",{class:"btn primary", "data-sfx":"tap_primary", onClick: ()=> nav(payload?.origin || "#/library")}, "عودة")
    );
    clear(app);
    app.appendChild(box);

    const xpEl = document.getElementById("xpN");
    const gEl = document.getElementById("gN");
    const rEl = document.getElementById("rN");
    countUp(xpEl, session.pending.xp);
    setTimeout(()=> countUp(gEl, session.pending.gold), 420);
    setTimeout(()=> countUp(rEl, session.pending.rating), 860);
  }

  // === Game 1: matching ===
  function countNonSpace(str){
  return String(str ?? "").replace(/\s+/g, "").length;
}

// Reveal next 20% letters (spaces are preserved and not counted)
function build20Mask(answer, enteredCount){
  const s = String(answer ?? "");
  const letterIdx = [];
  for(let i=0;i<s.length;i++){
    if(s[i] !== " ") letterIdx.push(i);
  }
  const totalLetters = letterIdx.length;
  const reveal = Math.max(1, Math.ceil(totalLetters * 0.20));

  const start = Math.min(Math.max(0, enteredCount), totalLetters);
  const end = Math.min(totalLetters, start + reveal);

  const revealSet = new Set(letterIdx.slice(start, end));
  const out = [];
  for(let i=0;i<s.length;i++){
    if(s[i] === " "){
      out.push(" ");
    }else{
      out.push(revealSet.has(i) ? s[i] : "_");
    }
  }
  return out.join("");
}

// === Unified normalization (MUST be used in check + hint) ===

function clampHintBudget(L){
  const len = Math.max(0, Number(L||0));
  const raw = Math.ceil(0.2 * len);
  return Math.min(4, Math.max(1, raw));
}

// Levenshtein alignment from attempt -> answer, returns ops plus match indices.
// Strings are expected to be ALREADY normalized.
function alignAttemptToAnswer(attempt, answer){
  const T = Array.from(String(attempt ?? ""));
  const A = Array.from(String(answer ?? ""));
  const n = T.length, m = A.length;

  // DP table (n+1)*(m+1)
  const dp = Array.from({length:n+1}, ()=> new Array(m+1).fill(0));
  for(let i=0;i<=n;i++) dp[i][0] = i;
  for(let j=0;j<=m;j++) dp[0][j] = j;
  for(let i=1;i<=n;i++){
    for(let j=1;j<=m;j++){
      const cost = (T[i-1] === A[j-1]) ? 0 : 1;
      const del = dp[i-1][j] + 1;      // delete from attempt
      const ins = dp[i][j-1] + 1;      // insert into attempt (i.e., take from answer)
      const sub = dp[i-1][j-1] + cost; // replace/match
      dp[i][j] = Math.min(del, ins, sub);
    }
  }

  const opsRev = [];
  let i = n, j = m;
  while(i>0 || j>0){
    if(i>0 && j>0){
      const cost = (T[i-1] === A[j-1]) ? 0 : 1;
      if(dp[i][j] === dp[i-1][j-1] + cost){
        opsRev.push({
          type: cost===0 ? "match" : "replace",
          tIndex: i-1,
          aIndex: j-1,
          tChar: T[i-1],
          aChar: A[j-1],
        });
        i--; j--; continue;
      }
    }
    if(j>0 && dp[i][j] === dp[i][j-1] + 1){
      opsRev.push({
        type: "insert",
        tIndex: i,
        aIndex: j-1,
        tChar: "",
        aChar: A[j-1],
      });
      j--; continue;
    }
    if(i>0){
      opsRev.push({
        type: "delete",
        tIndex: i-1,
        aIndex: j, // position *between* answer chars
        tChar: T[i-1],
        aChar: "",
      });
      i--; continue;
    }
  }
  const ops = opsRev.reverse();

  const matchedA = new Set();
  for(const op of ops){
    if(op.type === "match" && typeof op.aIndex === "number") matchedA.add(op.aIndex);
  }
  return { ops, matchedA, A };
}

function pickSpreadPositions(sortedPositions, k){
  const arr = Array.from(sortedPositions||[]);
  if(k <= 0) return [];
  if(arr.length <= k) return arr.slice();
  if(k === 1) return [arr[Math.floor((arr.length-1)/2)]];
  const out = [];
  const step = (arr.length - 1) / (k - 1);
  const used = new Set();
  for(let i=0;i<k;i++){
    const idx = Math.round(i * step);
    used.add(idx);
  }
  for(const idx of used){
    out.push(arr[idx]);
  }
  out.sort((a,b)=> arr.indexOf(a)-arr.indexOf(b));
  if(out.length < k){
    for(const p of arr){
      if(out.length >= k) break;
      if(!out.includes(p)) out.push(p);
    }
  }
  return out.slice(0, k);
}

// Typing hint: fair distribution over error segments (alignment-based) and never wastes a help.
// Returns {mask, newReveals:number[]} or null if already correct.
function buildTypingHintMask(answerNorm, attemptNorm, alreadyRevealed){
  const A = String(answerNorm ?? "");
  const T = String(attemptNorm ?? "");
  if(T === A) return null;

  const AChars = Array.from(A);
  const L = AChars.length;
  const budget = clampHintBudget(L);
  const revealed = alreadyRevealed instanceof Set ? alreadyRevealed : new Set();

  // Alignment
  const { ops, matchedA } = alignAttemptToAnswer(T, A);

  // Build error segments as contiguous non-match ops.
  const segments = [];
  let cur = null;
  const toAIndex = (op)=>{
    if(!L) return null;
    if(op.type === "match" || op.type === "replace" || op.type === "insert"){
      return Math.min(Math.max(0, op.aIndex), L-1);
    }
    // delete: tie to the current answer position
    const pos = Math.min(Math.max(0, op.aIndex), L);
    return Math.min(pos, L-1);
  };

  for(const op of ops){
    if(op.type === "match"){
      if(cur){ segments.push(cur); cur = null; }
      continue;
    }
    if(!cur) cur = { idxSet: new Set(), min: Infinity, max: -Infinity };
    const ai = toAIndex(op);
    if(typeof ai === "number" && ai >= 0 && ai < L){
      if(AChars[ai] !== " "){
        cur.idxSet.add(ai);
        if(ai < cur.min) cur.min = ai;
        if(ai > cur.max) cur.max = ai;
      }
    }
  }
  if(cur) segments.push(cur);

  // If alignment produces no segments (rare), fall back to spreading over the whole answer.
  if(!segments.length){
    const all = [];
    for(let i=0;i<L;i++) if(AChars[i] !== " ") all.push(i);
    const candidates = all.filter(i=>!revealed.has(i));
    const pick = pickSpreadPositions(candidates, Math.min(budget, candidates.length));
    const newReveals = pick;
    const revealAll = new Set([...revealed, ...newReveals]);
    const mask = AChars.map((ch,i)=> ch === " " ? " " : (matchedA.has(i) || revealAll.has(i) ? ch : "_")).join("");
    return { mask, newReveals };
  }

  // Convert segments to sorted candidate arrays (excluding already revealed)
  const segs = segments.map(s=>{
    const arr = Array.from(s.idxSet).sort((a,b)=>a-b).filter(i=>!revealed.has(i));
    return { arr, min:s.min, max:s.max };
  }).filter(s=>s.arr.length>0);

  if(!segs.length){
    const revealAll = new Set([...revealed]);
    const mask = AChars.map((ch,i)=> ch === " " ? " " : (matchedA.has(i) || revealAll.has(i) ? ch : "_")).join("");
    return { mask, newReveals: [] };
  }

  const pickFromSeg = (seg)=>{
    const mid = (seg.min + seg.max) / 2;
    let best = seg.arr[0];
    let bestD = Math.abs(best - mid);
    for(const v of seg.arr){
      const d = Math.abs(v - mid);
      if(d < bestD){ bestD = d; best = v; }
    }
    return best;
  };

  let remaining = budget;
  const chosen = [];

  if(segs.length === 1){
    const pick = pickSpreadPositions(segs[0].arr, Math.min(remaining, segs[0].arr.length));
    chosen.push(...pick);
    remaining -= pick.length;
  } else {
    for(const seg of segs){
      if(remaining <= 0) break;
      const p = pickFromSeg(seg);
      chosen.push(p);
      seg.arr = seg.arr.filter(x=>x!==p);
      remaining -= 1;
    }
    while(remaining > 0){
      segs.sort((a,b)=> b.arr.length - a.arr.length);
      const seg = segs.find(s=>s.arr.length>0);
      if(!seg) break;
      const p = pickFromSeg(seg);
      chosen.push(p);
      seg.arr = seg.arr.filter(x=>x!==p);
      remaining -= 1;
    }
  }

  const newReveals = chosen.filter(i=>!revealed.has(i));
  const revealAll = new Set([...revealed, ...newReveals]);
  const mask = AChars.map((ch,i)=> ch === " " ? " " : (matchedA.has(i) || revealAll.has(i) ? ch : "_")).join("");
  return { mask, newReveals };
}

function renderMatch(){
    const cs = session.cards;
    const left = shuffle(cs.map(c=>({ id:c.id, type:"a", text:c.a })));
    const right = shuffle(cs.map(c=>({ id:c.id, type:"b", text:c.b })));
    let sel = null;
    let selBtn = null;
    let correct = 0, wrong = 0;
    const matchedIds = new Set();
    let lastT = performance.now();
    const times = [];

    setRemain(0, cs.length);

    const wrap = h("div",{class:"match-wrap"});
    const colA = h("div",{class:"match-col"});
    const colB = h("div",{class:"match-col"});
    wrap.append(colA, colB);

    const makeBtn = (item)=> {
      const b = h("button",{class:"btn match-choice", "data-sfx":"ekhtiar"}, item.text);
      b.dataset.cardid = item.id;
      b.dataset.type = item.type;
      b.addEventListener("click", ()=> pick(item, b), {passive:true});
      return b;
    };

    left.forEach(it=> colA.appendChild(makeBtn(it)));
    right.forEach(it=> colB.appendChild(makeBtn(it)));

    stage.appendChild(h("div",{class:"subtle"}, "التوصيل"));
    stage.appendChild(h("div",{class:"hr"}));
    stage.appendChild(wrap);
// HELP (20%): reveal ONE pair as popup (card + answer). Logged per this game, replayable.
helpHandler = {
  canApply: ()=> {
    const remaining = session.cards.filter(c=>!matchedIds.has(c.id));
    return remaining.length > 0;
  },
  apply: ()=> {
    const isEn = getLang()==="en";
    const remaining = session.cards.filter(c=>!matchedIds.has(c.id));
    if(!remaining.length) throw new Error("No remaining pairs");

    // Always reveal exactly one pair (minimum help)
    const c = remaining[0];

    const body = h("div",{},
      h("div",{class:"subtle"}, isEn ? "Card & Answer" : "البطاقة والجواب"),
      h("div",{class:"hr"}),
      h("div",{style:"font-weight:800; font-size:18px; margin:8px 0 2px;"}, c.a),
      h("div",{class:"subtle", style:"margin:0 0 6px;"}, "→"),
      h("div",{style:"font-weight:800; font-size:18px; margin:2px 0 0;"}, c.b),
    );

    modal(isEn ? "Help" : "مساعدة", body, [
      {label:isEn?"OK":"حسناً", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close()}
    ]);

    return {
      title: isEn ? `Match: revealed 1 pair` : `التوصيل: عرض زوج واحد`,
      payload: { id: c.id, a: c.a, b: c.b }
    };
  },
  replay: (entry)=> {
    const isEn = getLang()==="en";
    const p = entry?.payload || {};
    const body = h("div",{},
      h("div",{class:"subtle"}, isEn ? "Card & Answer" : "البطاقة والجواب"),
      h("div",{class:"hr"}),
      h("div",{style:"font-weight:800; font-size:18px; margin:8px 0 2px;"}, p.a ?? ""),
      h("div",{class:"subtle", style:"margin:0 0 6px;"}, "→"),
      h("div",{style:"font-weight:800; font-size:18px; margin:2px 0 0;"}, p.b ?? ""),
    );
    modal(isEn ? "Help" : "مساعدة", body, [
      {label:isEn?"OK":"حسناً", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close()}
    ]);
  }
};



    function pick(item, btn){
      const now = performance.now();
      const dt = (now-lastT)/1000;
      lastT = now;
      times.push(dt);

      // اختيار/إلغاء اختيار البطاقة الأولى
      if(!sel){
        sel = item;
        selBtn = btn;
        btn.classList.add("selected");
        return;
      }
      if(sel.id === item.id && sel.type === item.type){
        selBtn?.classList.remove("selected");
        sel = null;
        selBtn = null;
        return;
      }

      const x = sel;
      const bx = selBtn;
      sel = null;
      selBtn = null;

      const y = item;

      bx?.classList.remove("selected");
      btn.classList.remove("selected");
      const ok = x.id === y.id && x.type !== y.type;
      if(ok){
        correct += 1;
        matchedIds.add(x.id);
        playSfx("saheh");
        setRemain(correct, cs.length);
        // remove matched pair buttons (حسب المعرف)
        const aBtn = colA.querySelector(`button[data-cardid="${x.id}"][data-type="a"]`);
        const bBtn = colB.querySelector(`button[data-cardid="${x.id}"][data-type="b"]`);
        aBtn?.remove();
        bBtn?.remove();
      } else {
        wrong += 1;
        playSfx("khata");
        if(vibOn && navigator.vibrate) navigator.vibrate(40);

        [bx, btn].forEach((el)=>{
          if(!el) return;
          el.classList.add("wrong");
          try{
            if(window.gsap && typeof gsap.fromTo === "function"){
              gsap.fromTo(el, {x:0}, {x:10, duration:0.06, yoyo:true, repeat:3});
            } else if(el.animate){
              el.animate(
                [{transform:"translateX(0px)"},{transform:"translateX(10px)"},{transform:"translateX(0px)"}],
                {duration:240, iterations:2, easing:"ease-in-out"}
              );
            }
          }catch{}
          setTimeout(()=> el.classList.remove("wrong"), 450);
        });
      }
      if(colA.children.length === 0){
        const avgSec = average(times);
        finishGame({ gameKey:"match", pairsOrCards: correct, successCount: correct, wrongCount: wrong, avgSec, usedHelp: session.usedHelps>0 });
      }
    }
  }

  // === Game 2: memory ===
  function renderMemory(){
    const cs = session.cards;
    const items = shuffle([
      ...cs.map(c=>({ id:c.id, kind:"a", text:c.a })),
      ...cs.map(c=>({ id:c.id, kind:"b", text:c.b })),
    ]);

    let revealed = [];
    const matched = new Set();
    let correct = 0;
    let lastSuccessT = performance.now();
    const times = [];

    const grid = h("div",{class:"memory-grid"});
    const tiles = [];

    const open = new Set(items.map((_,i)=>i));

    items.forEach((it, idx)=>{
      const tile = h("div",{class:"memory-tile open"});
      const inner = h("div",{class:"memory-inner"});
      const front = h("div",{class:"memory-face memory-front"}, "✦");
      const back  = h("div",{class:"memory-face memory-back"}, it.text);
      inner.append(front, back);
      tile.appendChild(inner);

      // show all at start for a few seconds
      tile.classList.add("flipped");

      tile.addEventListener("click", ()=> flip(idx, tile), {passive:true});
      tiles[idx] = tile;
      grid.appendChild(tile);
    });

    stage.appendChild(h("div",{class:"subtle"}, "قلب البطاقات"));
    stage.appendChild(h("div",{class:"hr"}));
    stage.appendChild(grid);
// HELP (20%): show ONE matching pair visually (focus + flip), replayable from log.
// We dim other tiles briefly so user can clearly see the two matching cards.
helpHandler = {
  canApply: ()=> {
    const m = new Map();
    items.forEach((it, i)=>{
      if(matched.has(i)) return;
      const arr = m.get(it.id) || [];
      arr.push(i);
      m.set(it.id, arr);
    });
    for(const arr of m.values()){
      if(arr.length >= 2) return true;
    }
    return false;
  },
  apply: ()=> {
    const isEn = getLang()==="en";
    const m = new Map();
    items.forEach((it, i)=>{
      if(matched.has(i)) return;
      const arr = m.get(it.id) || [];
      arr.push(i);
      m.set(it.id, arr);
    });

    let pickId = null;
    let pair = null;
    for(const [id, arr] of m.entries()){
      if(arr.length >= 2){
        pickId = id;
        pair = arr.slice(0,2);
        break;
      }
    }
    if(!pair) throw new Error("No available pair");

    const [i1,i2] = pair;
    const t1 = tiles[i1], t2 = tiles[i2];
    if(!t1 || !t2) throw new Error("Tiles missing");

    const wasFlipped1 = t1.classList.contains("flipped");
    const wasFlipped2 = t2.classList.contains("flipped");

    // lock interactions + dim others
    grid.classList.add("hint-lock","hint-mode");
    tiles.forEach((t)=>{
      if(!t) return;
      t.classList.remove("hint-focus");
    });

    t1.classList.add("flipped","hint-focus","hint-flash");
    t2.classList.add("flipped","hint-focus","hint-flash");

    setTimeout(()=>{
      t1.classList.remove("hint-flash");
      t2.classList.remove("hint-flash");
      if(!wasFlipped1 && !matched.has(i1)) t1.classList.remove("flipped");
      if(!wasFlipped2 && !matched.has(i2)) t2.classList.remove("flipped");
      t1.classList.remove("hint-focus");
      t2.classList.remove("hint-focus");
      grid.classList.remove("hint-lock","hint-mode");
    }, 1600);

    const card = session.cards.find(c=>c.id===pickId) || {};
    return {
      title: isEn ? "Memory: showed a matching pair" : "قلب البطاقات: إظهار زوج مطابق",
      payload: { id: pickId, a: card.a, b: card.b, i1, i2 }
    };
  },
  replay: (entry)=> {
    const p = entry?.payload || {};
    const i1 = p.i1, i2 = p.i2;
    const t1 = tiles?.[i1], t2 = tiles?.[i2];
    if(!t1 || !t2) return;

    const wasFlipped1 = t1.classList.contains("flipped");
    const wasFlipped2 = t2.classList.contains("flipped");

    grid.classList.add("hint-lock","hint-mode");
    tiles.forEach((t)=>{
      if(!t) return;
      t.classList.remove("hint-focus");
    });

    t1.classList.add("flipped","hint-focus","hint-flash");
    t2.classList.add("flipped","hint-focus","hint-flash");

    setTimeout(()=>{
      t1.classList.remove("hint-flash");
      t2.classList.remove("hint-flash");
      if(!wasFlipped1) t1.classList.remove("flipped");
      if(!wasFlipped2) t2.classList.remove("flipped");
      t1.classList.remove("hint-focus");
      t2.classList.remove("hint-focus");
      grid.classList.remove("hint-lock","hint-mode");
    }, 1600);
  }
};



    setTimeout(()=>{
      open.clear();
      tiles.forEach((t)=>{
        if(!t) return;
        t.classList.remove("open","flipped","selected");
      });
    }, 5000);

    function flip(idx, tile){
      if(matched.has(idx)) return;
      if(open.has(idx)) return;

      // allow user to unselect the same revealed tile
      if(revealed.includes(idx)){
        playSfx("ekhtiar");
        revealed = revealed.filter(x=>x!==idx);
        tile.classList.remove("flipped","selected");
        return;
      }
      if(revealed.length >= 2) return;

      if(revealed.length === 0) playSfx("ekhtiar");
      tile.classList.add("flipped","selected");
      revealed.push(idx);

      if(revealed.length === 2){
        const [i1,i2] = revealed;
        const a = items[i1], b = items[i2];
        const ok = a.id === b.id && a.kind !== b.kind;
        if(ok){
          matched.add(i1); matched.add(i2);
          correct += 1;
          const now = performance.now();
          times.push((now-lastSuccessT)/1000);
          lastSuccessT = now;
          playSfx("saheh");

          setTimeout(()=>{
            const t1 = tiles[i1];
            const t2 = tiles[i2];
            if(t1){ t1.classList.add("matched"); t1.classList.remove("selected"); }
            if(t2){ t2.classList.add("matched"); t2.classList.remove("selected"); }
            revealed = [];
            if(correct >= cs.length){
              const avgSec = average(times.length?times:[6]);
              finishGame({ gameKey:"memory", pairsOrCards: correct, successCount: correct, wrongCount: 0, avgSec, usedHelp: session.usedHelps>0 });
            }
          }, 250);
        } else {
          playSfx("khata");
          setTimeout(()=>{
            const t1 = tiles[i1];
            const t2 = tiles[i2];
            if(t1) t1.classList.remove("flipped","selected");
            if(t2) t2.classList.remove("flipped","selected");
            revealed = [];
          }, 800);
        }
      }
    }

  }

  // === Game 3: anagram ===
  function renderAnagram(){
    const pool = session.cards.filter(c=>anagramSet.has(c.id));
    if(pool.length === 0){
      finishGame({ gameKey:"anagram", pairsOrCards: 0, successCount: 0, wrongCount: 0, avgSec: 6, usedHelp:false });
      return;
    }
    let idx = 0;
    let correct = 0, wrong = 0;
    let lastT = performance.now();
    const times = [];

    const card = h("div",{class:"card glass anagram-card"});
    stage.appendChild(h("div",{class:"subtle"}, "ترتيب الحروف"));
    stage.appendChild(h("div",{class:"hr"}));
    stage.appendChild(card);

    function show(){
      setRemain(idx, pool.length);
      if(idx >= pool.length){
        const avgSec = average(times.length?times:[6]);
        finishGame({ gameKey:"anagram", pairsOrCards: pool.length, successCount: correct, wrongCount: wrong, avgSec, usedHelp: session.usedHelps>0 });
        return;
      }
      const isEn = getLang()==="en";

      // Unified normalization for check + hint
      const c = pool[idx];
      const A = normalizeText(c.a);
      const AChars = Array.from(A);
      const spaceMask = AChars.map(ch=>ch===" ");
      const ALetters = AChars.filter(ch=>ch!==" ");
      const L = ALetters.length;

      // No tiles for spaces: spaces are fixed automatically.
      const bankLetters = shuffle(ALetters.slice());

      // Slots correspond to ALetters (spaces removed)
      const slots = Array.from({length:L}, ()=> null);
      const slotBtn = Array.from({length:L}, ()=> null);
      const locked = Array.from({length:L}, ()=> false);
      const stack = []; // {pos, btn}

      const progress = h("div",{class:"subtle", style:"text-align:center;"}, `(${Math.max(0, pool.length-idx)}/${pool.length})`);
      const answer = h("div",{class:"anagram-answer", dir:"auto"}, "");
      const grid = h("div",{class:"anagram-grid"});

      function buildAttemptString(){
        const out = [];
        let k = 0;
        for(let i=0;i<AChars.length;i++){
          if(spaceMask[i]) out.push(" ");
          else { out.push(slots[k] ?? ""); k++; }
        }
        return out.join("");
      }

      function renderAnswer(){
        clear(answer);
        let k = 0;
        for(let i=0;i<AChars.length;i++){
          if(spaceMask[i]){
            answer.appendChild(h("span",{class:"anagram-space"}, " "));
          } else {
            const ch = slots[k];
            const isLocked = locked[k] && ch === ALetters[k];
            answer.appendChild(h("span",{class:`anagram-slot ${isLocked?"locked":""}`}, ch ?? "_"));
            k++;
          }
        }
      }

      function update(){
        renderAnswer();
        undoBtn.disabled = stack.length === 0;
      }

      function placeLetter(btn){
        let pos = -1;
        for(let i=0;i<L;i++){
          if(!locked[i] && slots[i] === null){ pos = i; break; }
        }
        if(pos < 0) return;
        slots[pos] = btn._ch;
        slotBtn[pos] = btn;
        stack.push({pos, btn});
        btn.disabled = true;
        btn.classList.add("is-used");
        update();
      }

      const undoBtn = h("button",{class:"btn-wide btn-active anagram-undo","data-sfx":"naker", type:"button", disabled:true, onClick: ()=> {
        if(!stack.length) return;
        const last = stack.pop();
        const pos = last.pos;
        if(locked[pos]){ update(); return; }
        slots[pos] = null;
        slotBtn[pos] = null;
        last.btn.disabled = false;
        last.btn.classList.remove("is-used");
        update();
      }}, h("i",{class:"fas fa-rotate-left", ariaHidden:"true"}), isEn?"Undo":"تراجع");

      const bankButtons = [];
      bankLetters.forEach((ch)=>{
        const b = h("button",{class:"anagram-letter btn-active","data-sfx":"ekhtiar", type:"button", onClick: ()=> placeLetter(b)}, ch);
        b._ch = ch;
        bankButtons.push(b);
        grid.appendChild(b);
      });

      function computeWrongPositions(){
        const wrong = [];
        for(let i=0;i<L;i++){
          if((slots[i] ?? "") !== ALetters[i]) wrong.push(i);
        }
        return wrong;
      }

      function lockCorrectAt(pos){
        const target = ALetters[pos];
        if((slots[pos] ?? "") === target){
          if(!locked[pos]){ locked[pos] = true; return true; }
          return false;
        }

        const bankBtn = bankButtons.find(b=>!b.disabled && b._ch === target);
        if(bankBtn){
          const oldBtn = slotBtn[pos];
          if(oldBtn){ oldBtn.disabled = false; oldBtn.classList.remove("is-used"); }
          for(let i=stack.length-1;i>=0;i--){ if(stack[i].pos === pos){ stack.splice(i,1); break; } }
          slots[pos] = target;
          slotBtn[pos] = bankBtn;
          bankBtn.disabled = true;
          bankBtn.classList.add("is-used");
          locked[pos] = true;
          return true;
        }

        let other = -1;
        for(let i=0;i<L;i++){
          if(i === pos) continue;
          if(!locked[i] && slots[i] === target && ALetters[i] !== target){ other = i; break; }
        }
        if(other >= 0){
          const tmpCh = slots[pos];
          const tmpBtn = slotBtn[pos];
          slots[pos] = slots[other];
          slotBtn[pos] = slotBtn[other];
          slots[other] = tmpCh;
          slotBtn[other] = tmpBtn;
          for(const e of stack){
            if(e.pos === pos) e.pos = other;
            else if(e.pos === other) e.pos = pos;
          }
          locked[pos] = true;
          return true;
        }
        return false;
      }

      helpHandler = {
        canApply: ()=> {
          const wrong = computeWrongPositions();
          if(wrong.length === 0){
            return { ok:false, message: isEn ? "Your answer is already correct." : "إجابتك صحيحة، لا توجد مساعدة مطلوبة." };
          }
          return { ok:true };
        },
        apply: ()=> {
          const wrong = computeWrongPositions();
          if(wrong.length === 0){
            return { noop:true, message: isEn ? "Your answer is already correct." : "إجابتك صحيحة، لا توجد مساعدة مطلوبة." };
          }
          const budget = clampHintBudget(L);
          const picks = pickSpreadPositions(wrong, Math.min(budget, wrong.length));
          let fixed = 0;
          for(const pos of picks){ if(lockCorrectAt(pos)) fixed += 1; }
          update();
          if(fixed <= 0){
            return { noop:true, message: isEn ? "Cannot apply help now." : "لا يمكن تطبيق المساعدة الآن." };
          }

          const after = buildAttemptString();
          const body = h("div",{},
            h("div",{class:"subtle"}, isEn ? `Locked ${fixed} letter(s)` : `تم تثبيت ${fixed} حرف/حروف`),
            h("div",{class:"hr"}),
            h("div",{class:"bidi-plaintext", dir:"auto", style:"white-space:pre; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size:22px; text-align:center; letter-spacing:1px;"}, after)
          );
          modal(isEn ? "Help" : "مساعدة", body, [
            {label:isEn?"OK":"حسناً", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close()}
          ]);
          return { title: (isEn ? "Anagram: lock letters" : "ترتيب الحروف: تثبيت"), payload: { after, fixed } };
        },
        replay: (entry)=> {
          const after = entry?.payload?.after ?? "";
          const fixed = entry?.payload?.fixed ?? 0;
          const body = h("div",{},
            h("div",{class:"subtle"}, isEn ? `Locked ${fixed} letter(s)` : `تم تثبيت ${fixed} حرف/حروف`),
            h("div",{class:"hr"}),
            h("div",{class:"bidi-plaintext", dir:"auto", style:"white-space:pre; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size:22px; text-align:center; letter-spacing:1px;"}, after)
          );
          modal(isEn ? "Help" : "مساعدة", body, [
            {label:isEn?"OK":"حسناً", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close()}
          ]);
        }
      };

      const okBtn = h("button",{class:"btn-wide btn-active anagram-ok","data-sfx":"none", type:"button", onClick: ()=> check(c, buildAttemptString())},
        h("i",{class:"fas fa-check-circle", ariaHidden:"true"}),
        isEn?"OK":"موافق"
      );

      clear(card);
      card.append(progress, answer, grid, h("div",{class:"anagram-actions"}, undoBtn, okBtn));
      update();
    }

    function check(c, answer){
      const now = performance.now();
      times.push((now-lastT)/1000);
      lastT = now;

      const isEn = getLang()==="en";
      if(normalizeText(answer) === normalizeText(c.a)){
        correct += 1;
        playSfx("saheh");
        idx += 1;
        show();
      } else {
        wrong += 1;
        playSfx("khata");
        toast(isEn ? "Wrong" : "خطأ");
        show();
      }
    }

    show();
  }

  // === Game 4: typing ===
    function renderTyping(){
      const pool = session.cards.filter(c=>typingSet.has(c.id) && !anagramSet.has(c.id));
      if(pool.length === 0){
        finishGame({ gameKey:"typing", pairsOrCards: 0, successCount: 0, wrongCount: 0, avgSec: 6, usedHelp:false });
        return;
      }
      let idx = 0;
      let correct = 0, wrong = 0;
      let lastT = performance.now();
      const times = [];

      const card = h("div",{class:"card glass typing-card"});
      stage.appendChild(h("div",{class:"subtle"}, "الكتابة"));
      stage.appendChild(h("div",{class:"hr"}));
      stage.appendChild(card);

      function show(){
        setRemain(idx, pool.length);

        if(idx >= pool.length){
          const avgSec = average(times.length?times:[6]);
          finishGame({ gameKey:"typing", pairsOrCards: pool.length, successCount: correct, wrongCount: wrong, avgSec, usedHelp: session.usedHelps>0 });
          return;
        }

        const c = pool[idx];

        const progress = h("div",{class:"subtle", style:"text-align:center;"}, `(${Math.max(0, pool.length-idx)}/${pool.length})`);
        const prompt = h("div",{class:"typing-prompt"}, c.b || "");
        const hint = h("div",{class:"typing-hint subtle"}, "اكتب الإجابة ثم اضغط موافق");

        const input = h("input",{type:"text", class:"modern-input typing-input", maxlength:"45", placeholder:"اكتب هنا...", dir:"auto", autocapitalize:"none", autocomplete:"off", spellcheck:"false"});

        // --- HELP (fair 20% budget, alignment-based, never wastes a help) ---
        const revealedSet = new Set();
        helpHandler = {
          canApply: ()=> {
            const isEn = getLang()==="en";
            const A = normalizeText(c.a);
            const T = normalizeText(input.value);
            if(T === A){
              return { ok:false, message: (isEn ? "Your answer is correct. No help needed." : "إجابتك صحيحة، لا توجد مساعدة مطلوبة.") };
            }
            const res = buildTypingHintMask(A, T, revealedSet);
            if(!res || !res.newReveals || res.newReveals.length === 0){
              return { ok:false, message: (isEn ? "No applicable help right now." : "لا يمكن تطبيق المساعدة الآن.") };
            }
            return { ok:true };
          },
          apply: ()=> {
            const isEn = getLang()==="en";
            const A = normalizeText(c.a);
            const T = normalizeText(input.value);
            if(T === A){
              return { noop:true, message: (isEn ? "Your answer is correct. No help needed." : "إجابتك صحيحة، لا توجد مساعدة مطلوبة.") };
            }
            const res = buildTypingHintMask(A, T, revealedSet);
            if(!res || !res.newReveals || res.newReveals.length === 0){
              return { noop:true, message: (isEn ? "No applicable help right now." : "لا يمكن تطبيق المساعدة الآن.") };
            }
            for(const i of res.newReveals) revealedSet.add(i);

            const body = h("div",{},
              h("div",{class:"subtle"}, isEn ? "Hint" : "تلميح"),
              h("div",{class:"hr"}),
              h("div",{class:"bidi-plaintext", dir:"auto", style:"white-space:pre; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size:22px; text-align:center; letter-spacing:1px;"}, res.mask)
            );
            modal(isEn ? "Help" : "مساعدة", body, [
              {label:isEn?"OK":"حسناً", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close()}
            ]);

            return { title: (isEn ? "Typing: hint" : "الكتابة: تلميح"), payload: { mask: res.mask } };
          },
          replay: (entry)=> {
            const isEn = getLang()==="en";
            const mask = entry?.payload?.mask ?? "";
            const body = h("div",{},
              h("div",{class:"subtle"}, isEn ? "Hint" : "تلميح"),
              h("div",{class:"hr"}),
              h("div",{class:"bidi-plaintext", dir:"auto", style:"white-space:pre; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size:22px; text-align:center; letter-spacing:1px;"}, mask)
            );
            modal(isEn ? "Help" : "مساعدة", body, [
              {label:isEn?"OK":"حسناً", kind:"primary", sfx:"tap_secondary", onClick:(close)=>close()}
            ]);
          }
        };

        // Enter to submit
        input.addEventListener("keydown", (e)=>{
          if(e.key === "Enter"){
            e.preventDefault();
            check(c, input.value);
          }
        });

        const okBtn = h("button",{class:"btn-wide btn-active typing-ok", "data-sfx":"none", style:"background: var(--accent-color); color: white; margin-top:12px;", onClick: ()=> check(c, input.value)}, 
          h("i",{class:"fas fa-check-circle", ariaHidden:"true"}), 
          "موافق"
        );

        clear(card);
        card.append(progress, prompt, hint, h("div",{class:"field"}, input), okBtn);

        // Focus for speed
        setTimeout(()=> { try{ input.focus(); }catch{} }, 0);
      }

      function check(c, val){
        const now = performance.now();
        times.push((now-lastT)/1000);
        lastT = now;

        const isEn = getLang()==="en";
        const a = normalizeText(val);
        const b = normalizeText(c.a);

        if(a === b){
          correct += 1;
          playSfx("saheh");
          idx += 1;
          show();
        } else {
          wrong += 1;
          playSfx("khata");
          toast(isEn ? "Wrong" : "خطأ");
        }
      }

      show();
    }

  runGame();
}

function renderLibrary(){
  const isEn = getLang()==="en";

  // header like the UI prototype (profile chip + gold)
  const headerCardStack = h("div",{class:"avatar-stack", ariaHidden:"true"});
  try{
    const {frame, bg, av} = computeAvatarLayers(state);
    const bgImg = new Image(); bgImg.src = bg?.src || ""; bgImg.className = "layer bg";
    const avImg = new Image(); avImg.src = av?.src || ""; avImg.className = "layer av";
    const frameImg = new Image(); frameImg.src = frame?.src || ""; frameImg.className = "layer frame";
    // Layering per spec: frame (bottom) -> background -> avatar (top)
    headerCardStack.append(frameImg, bgImg, avImg);
  }catch{}

  const header = h("header",{class:"page-header"},
    h("div",{class:"profile-card", "data-sfx":"naker", onClick: ()=> nav("#/profile")},
      headerCardStack,
      h("div",{class:"profile-text"},
        h("div",{class:"profile-name"}, isEn?"Packs":"رزم البطاقات"),
        h("div",{class:"user-meta"}, isEn?"Your library":"مكتبتك")
      )
    ),
    h("div",{class:"gold-tag", "aria-label": isEn?"Gold":"الذهب"},
      h("i",{class:"fas fa-coins"}),
      h("span",{class:"gold-val"}, String(Math.max(0, Math.floor(state.economy.gold||0))))
    )
  );

  const searchInput = h("input",{id:"pack-search", class:"modern-input", placeholder: isEn?"Search packs...":"بحث عن رزمة...", style:"padding-left: 40px;"});
  const search = h("div",{class:"search-box"}, searchInput, h("i",{class:"fas fa-search", ariaHidden:"true"}));

  const btnAddPack = h("button",{class:"btn-wide btn-active accent-outline", style:"margin-bottom: 10px;", "data-sfx":"edafe", onClick: ()=> openCreatePack()},
    h("i",{class:"fas fa-plus-square"}),
    isEn?"Add a new pack":"إضافة رزمة جديدة"
  );

  const grid = h("div",{class:"packs-grid", id:"packsGrid"});
  const scroll = h("div",{class:"packs-scroll"}, grid);

  const backBtn = h("button",{class:"fixed-back-btn btn-active", "data-sfx":"naker", onClick: ()=> navBack()},
    h("i",{class:"fas fa-arrow-right"}),
    isEn?"Back":"رجوع"
  );

  const wrap = h("div",{class:"packs-wrap"}, header, search, btnAddPack, scroll, backBtn);
  app.appendChild(wrap);

  const packs = [
    { key:"all", name: tr("جميع البطاقات") },
    // Virtual built-in pack: cards due to play today (auto-updated daily)
    { key:"dueToday", name: tr("بطاقات اللعب اليوم") },
    { key:"daily", name: tr("المجموعات اليومية") },
    { key:"easy", name: tr("سهل") },
    { key:"medium", name: tr("متوسط") },
    { key:"hard", name: tr("صعب") },
    { key:"ignored", name: tr("التجاهل") },
    { key:"completed", name: tr("المكتملة") },
    ...state.library.customPacks.map(p=>({ key:`custom:${p.id}`, name:p.name })),
  ];

  function iconForPack(key){
    if(key === "all") return { cls:"fas fa-layer-group", color:"#4682b4" };
    if(key === "dueToday") return { cls:"fas fa-calendar-check", color:"#8e44ad" };
    if(key === "daily") return { cls:"fas fa-calendar-day", color:"#ff5722" };
    if(key === "easy") return { cls:"fas fa-smile", color:"#4caf50" };
    if(key === "medium") return { cls:"fas fa-meh", color:"#ffc107" };
    if(key === "hard") return { cls:"fas fa-dizzy", color:"#f44336" };
    if(key === "ignored") return { cls:"fas fa-ban", color:"#9e9e9e" };
    if(key === "completed") return { cls:"fas fa-check-circle", color:"#009688" };
    if(String(key).startsWith("custom:")) return { cls:"fas fa-folder", color:"#6d4c41" };
    return { cls:"fas fa-box-open", color:"var(--accent-color)" };
  }

  function rebuild(){
    clear(grid);
    const q = (searchInput.value||"").trim().toLowerCase();
    for(const p of packs){
      if(q && !p.name.toLowerCase().includes(q)) continue;
      const ic = iconForPack(p.key);
      const locked = (p.key === "dueToday" && !isDueTodayUnlocked());
      const tile = h("div",{class:`pack-item btn-active ${locked?"locked-pack":""}`, "data-sfx":"naker", onClick: ()=> openPack(p.key, p.name)},
        h("i",{class: locked ? "fas fa-lock" : ic.cls, style: locked ? "color:#c0392b" : `color:${ic.color}`}),
        h("div",{class:"pack-name"}, p.name),
        locked ? h("div",{class:"subtle", style:"font-size:12px;margin-top:4px;"}, (getLang()==="en") ? "Locked: watch ad or pay 150 gold" : "مقفلة: إعلان أو 150 ذهب") : null
      );
      grid.appendChild(tile);
    }
  }
  searchInput.addEventListener("input", rebuild, {passive:true});
  rebuild();

  function openCreatePack(){
    const name = h("input",{maxlength:"30"});
    const body = h("div",{}, h("div",{class:"field"}, h("label",{}, "الاسم"), name));
    modal("إضافة رزمة", body, [
      { label:"إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() },
      { label:"حفظ", kind:"primary", sfx:"tap_primary", onClick:(close)=> {
        const n = name.value.trim();
        if(!n){ toast("اسم فارغ."); return; }
        if(state.library.customPacks.some(x=>x.name===n)){ toast("اسم مستخدم."); return; }
        state.library.customPacks.push({ id: uuid(), name:n, cardIds:[] });
        saveState(state);
        close();
        nav("#/library");
      }},
    ]);
  }

  
  function isDueTodayUnlocked(){
    try{
      const econ = state.economy || (state.economy = {});
      return String(econ.dueTodayUnlockedIso||"") === todayISO();
    }catch(_e){ return false; }
  }

  function unlockDueTodayByGoldAndOpen(title){
    const econ = state.economy || (state.economy = {});
    const cost = 150;
    const g = Number(econ.gold||0);
    if(g < cost){
      toast((getLang()==="en") ? "Not enough gold." : "لا يوجد ذهب كافي.");
      return;
    }
    econ.gold = Math.max(0, g - cost);
    econ.dueTodayUnlockedIso = todayISO();
    econ.dueTodayUnlockAt = Date.now();
    saveState(state);
    syncTopbar();
    // open now
    const ids = getPackCardIds("dueToday");
    openCardsList(title, ids, { packKey:"dueToday" });
  }

  function openDueTodayGate(title){
    const isEn = (getLang()==="en");
    const econ = state.economy || (state.economy = {});
    const cost = 150;

    if(isDueTodayUnlocked()){
      const ids = getPackCardIds("dueToday");
      openCardsList(title, ids, { packKey:"dueToday" });
      return;
    }

    const info = h("div",{style:"line-height:1.9;white-space:pre-wrap;"},
      isEn
        ? `This pack can be opened once per day.
Choose: watch a rewarded ad OR pay ${cost} gold.
After unlocking, it stays open for the rest of the day.`
        : `هذه الرزمة تُفتح مرة واحدة يوميًا.
اختر: مشاهدة إعلان مقابل جائزة أو دفع ${cost} ذهب.
بعد الفتح ستبقى مفتوحة حتى نهاية اليوم.`
    );

    const warn = h("div",{class:"subtle", style:"margin-top:10px;line-height:1.9;"},
      isEn
        ? `If ads are blocked or no ad is available, you can still unlock with gold.`
        : `إذا كانت الإعلانات محجوبة أو لا يوجد إعلان متاح، يمكنك الفتح بالذهب.`
    );

    const goldLine = h("div",{class:"row", style:"gap:10px;justify-content:center;margin-top:12px;"},
      h("div",{class:"pill"}, (isEn?"Your gold: ":"ذهبك: ") + String(Number(econ.gold||0)))
    );

    const body = h("div",{}, info, warn, goldLine);

    modal(isEn ? "Today’s Play Cards" : "بطاقات اللعب اليوم", body, [
      { label: isEn ? "Back" : "رجوع", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() },
      { label: isEn ? "Watch Ad" : "مشاهدة إعلان", kind:"primary", sfx:"tap_primary", onClick: async (close)=>{
          // Do NOT unlock unless reward event fires
          close();
          const ok = await dmAds.show("unlockDueToday");
          if(!ok){
            // user can try again or pay with gold
            return;
          }
          // After ad completion, user can open the pack (now unlocked) by tapping again.
        }
      },
      { label: (isEn ? `Pay ${cost} Gold` : `دفع ${cost} ذهب`), kind:"primary", sfx:"tap_primary", onClick:(close)=>{
          close();
          unlockDueTodayByGoldAndOpen(title);
        }
      },
    ]);
  }
function openPack(key, title){
    // daily groups special
    if(key === "daily"){
      openDailyGroups();
      return;
    }

    if(key === "dueToday"){
      if(!isDueTodayUnlocked()){
        openDueTodayGate(title);
        return;
      }
      try{ masteryMarkDueOpened(state, (getPackCardIds(key)||[]).length); }catch(_){ }
    }
    const ids = getPackCardIds(key);
    openCardsList(title, ids, { packKey:key });
  }

  function openDailyGroups(){
    const isEn = getLang()==="en";
    const groups = Object.values(state.cards.dailyGroups).sort((a,b)=> b.iso.localeCompare(a.iso));

    // Header (same style as packs page)
    const headerCardStack = h("div",{class:"avatar-stack", ariaHidden:"true"});
    try{
      const {frame, bg, av} = computeAvatarLayers(state);
      const bgImg = new Image(); bgImg.src = bg?.src || ""; bgImg.className = "layer bg";
      const avImg = new Image(); avImg.src = av?.src || ""; avImg.className = "layer av";
      const frameImg = new Image(); frameImg.src = frame?.src || ""; frameImg.className = "layer frame";
      // Layering per spec: frame (bottom) -> background -> avatar (top)
      headerCardStack.append(frameImg, bgImg, avImg);
    }catch{}

    const header = h("header",{class:"page-header"},
      h("div",{class:"profile-card", "data-sfx":"naker", onClick: ()=> nav("#/profile")},
        headerCardStack,
        h("div",{class:"profile-text"},
          h("div",{class:"profile-name"}, isEn?"Daily groups":"المجموعات اليومية"),
          h("div",{class:"user-meta"}, isEn?"Pick a day":"اختر يومًا")
        )
      ),
      h("div",{class:"gold-tag", "aria-label": isEn?"Gold":"الذهب"},
        h("i",{class:"fas fa-coins"}),
        h("span",{class:"gold-val"}, String(Math.max(0, Math.floor(state.economy.gold||0))))
      )
    );

    // Search (prototype style)
    const searchInput = h("input",{id:"daily-search", class:"modern-input", placeholder: isEn?"Search by date...":"بحث بالتاريخ...", style:"padding-left: 40px;"});
    const search = h("div",{class:"search-box"}, searchInput, h("i",{class:"fas fa-search", ariaHidden:"true"}));

    // Grid list
    const grid = h("div",{class:"packs-grid", id:"dailyGrid"});
    const scroll = h("div",{class:"packs-scroll"}, grid);

    const backBtn = h("button",{class:"fixed-back-btn btn-active", "data-sfx":"tap_secondary", onClick: ()=> {
      clear(app);
      renderLibrary();
    }},
      h("i",{class:"fas fa-arrow-right"}),
      isEn?"Back":"العودة للرزم"
    );

    const wrap = h("div",{class:"packs-wrap"}, header, search, scroll, backBtn);
    clear(app);
    app.appendChild(wrap);

    // SFX is delegated globally via wireSfx(document)

    function rebuildG(){
      clear(grid);
      const q = (searchInput.value||"").trim();

      if(!groups.length){
        grid.appendChild(h("div",{class:"glass gold panel center", style:"grid-column: 1 / -1; text-align:center;"},
          h("div",{class:"subtle"}, isEn?"No daily groups yet.":"لا توجد مجموعات يومية بعد.")
        ));
        return;
      }

      for(const g of groups){
        if(q && !String(g.titleDMY||"").includes(q)) continue;
        const tile = h("div",{class:"pack-item btn-active", "data-sfx":"naker", onClick: ()=> openCardsList(g.titleDMY, g.cardIds, { packKey:"dailyGroup", groupIso:g.iso })},
          h("i",{class:"fas fa-calendar-day", style:"color:#ff5722"}),
          h("div",{class:"pack-name"}, g.titleDMY),
          h("div",{class:"pack-count"}, `${g.cardIds.length} ${isEn?"cards":"بطاقة"}`)
        );
        grid.appendChild(tile);
      }

      if(!grid.childElementCount){
        grid.appendChild(h("div",{class:"glass gold panel center", style:"grid-column: 1 / -1; text-align:center;"},
          h("div",{class:"subtle"}, isEn?"No results.":"لا توجد نتائج.")
        ));
      }
    }

    searchInput.addEventListener("input", rebuildG, {passive:true});
    rebuildG();
  }

  function openCardsList(title, ids, ctx){
    const isEn = getLang()==="en";

    // Custom pack management (rename/delete) only for user-created packs
    let packTitle = title;
    const isCustomPack = !!(ctx?.packKey && String(ctx.packKey).startsWith("custom:"));
    const customPackId = isCustomPack ? String(ctx.packKey).split(":")[1] : null;

    // Header (same style as packs page)
    const headerCardStack = h("div",{class:"avatar-stack", ariaHidden:"true"});
    try{
      const {frame, bg, av} = computeAvatarLayers(state);
      const bgImg = new Image(); bgImg.src = bg?.src || ""; bgImg.className = "layer bg";
      const avImg = new Image(); avImg.src = av?.src || ""; avImg.className = "layer av";
      const frameImg = new Image(); frameImg.src = frame?.src || ""; frameImg.className = "layer frame";
      // Layering per spec: frame (bottom) -> background -> avatar (top)
      headerCardStack.append(frameImg, bgImg, avImg);
    }catch{}

    const titleEl = h("div",{class:"profile-name"}, packTitle);
    const header = h("header",{class:"page-header"},
      h("div",{class:"profile-card", "data-sfx":"naker", onClick: ()=> nav("#/profile")},
        headerCardStack,
        h("div",{class:"profile-text"},
          titleEl,
          h("div",{class:"user-meta"}, isEn?"Cards":"البطاقات")
        )
      ),
      h("div",{class:"gold-tag", "aria-label": isEn?"Gold":"الذهب"},
        h("i",{class:"fas fa-coins"}),
        h("span",{class:"gold-val"}, String(Math.max(0, Math.floor(state.economy.gold||0))))
      )
    );

    // Search box (prototype style)
    const searchInput = h("input",{id:"card-search", class:"modern-input", placeholder: isEn?"Search cards...":"بحث في البطاقات...", style:"padding-left: 40px;"});
    const search = h("div",{class:"search-box"}, searchInput, h("i",{class:"fas fa-search", ariaHidden:"true"}));

    // Custom pack tools (rename/delete)
    const packTools = isCustomPack ? h("div",{class:"row", style:"gap:10px; margin-bottom: 10px;"},
      h("button",{class:"btn-wide btn-active accent-outline", style:"flex:1;", "data-sfx":"naker", onClick: ()=> renameCustomPack()},
        h("i",{class:"fas fa-pen"}),
        isEn?"Rename pack":"تغيير الاسم"
      ),
      h("button",{class:"btn-wide btn-active danger", style:"flex:1;", "data-sfx":"hathef", onClick: ()=> deleteCustomPack()},
        h("i",{class:"fas fa-trash-alt"}),
        isEn?"Delete pack":"حذف الرزمة"
      )
    ) : null;

    function getCustomPack(){
      if(!customPackId) return null;
      return state.library.customPacks.find(p=> p.id === customPackId) || null;
    }

    function renameCustomPack(){
      const p = getCustomPack();
      if(!p){ toast(isEn?"Pack not found.":"الرزمة غير موجودة."); return false; }

      const input = h("input",{maxlength:"30", value: p.name || ""});
      const body = h("div",{},
        h("div",{class:"field"},
          h("label",{}, isEn?"Name":"الاسم"),
          input
        )
      );

      modal(isEn?"Rename pack":"تغيير اسم الرزمة", body, [
        { label: isEn?"Cancel":"إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> { close(); return true; } },
        { label: isEn?"Save":"حفظ", kind:"primary", sfx:"naker", onClick:(close)=> {
          const n = (input.value||"").trim();
          if(!n){ toast(isEn?"Empty name.":"اسم فارغ."); return false; }
          const exists = state.library.customPacks.some(x=> x.id !== p.id && x.name === n);
          if(exists){ toast(isEn?"Name already used.":"اسم مستخدم."); return false; }
          p.name = n;
          packTitle = n;
          try{ titleEl.textContent = n; }catch{}
          saveState(state);
          close();
          toast(isEn?"Done.":"تم.");
          return true;
        }},
      ]);
      // Focus
      setTimeout(()=> { try{ input.focus(); input.select?.(); }catch{} }, 0);
      return true;
    }

    function deleteCustomPack(){
      const p = getCustomPack();
      if(!p){ toast(isEn?"Pack not found.":"الرزمة غير موجودة."); return false; }

      const body = h("div",{},
        h("div",{class:"subtle", style:"white-space:pre-wrap; line-height:1.8;"},
          isEn
            ? "Delete this pack? Cards will NOT be deleted, only the pack."
            : "هل تريد حذف هذه الرزمة؟\nالبطاقات لن تُحذف، سيتم حذف الرزمة فقط."
        )
      );

      modal(isEn?"Confirm":"تأكيد", body, [
        { label: isEn?"Cancel":"إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> { close(); return true; } },
        { label: isEn?"Delete":"حذف", kind:"danger", sfx:"hathef", onClick:(close)=> {
          state.library.customPacks = state.library.customPacks.filter(x=> x.id !== p.id);
          saveState(state);
          close();
          toast(isEn?"Deleted.":"تم حذف الرزمة.");
          clear(app);
          renderLibrary();
          return true;
        }},
      ]);
      return true;
    }

    // Selection controls
    const normalControls = h("div",{id:"normal-controls"},
      h("button",{class:"btn-wide btn-active accent-outline", style:"margin-bottom: 10px;", "data-sfx":"naker", onClick: ()=> toggleSelectionMode(true)},
        h("i",{class:"fas fa-check-double"}),
        isEn?"Select":"تحديد"
      )
    );

    const selectionControls = h("div",{id:"selection-controls", style:"display:none; gap: 10px; margin-bottom: 10px;"},
      h("button",{class:"btn-wide btn-active accent-outline", style:"flex: 1;", "data-sfx":"naker", onClick: ()=> selectAllCards()},
        isEn?"Select all":"تحديد الكل"
      ),
      h("button",{class:"btn-wide btn-active muted", style:"flex: 1;", "data-sfx":"naker", onClick: ()=> toggleSelectionMode(false)},
        isEn?"Cancel":"إلغاء"
      )
    );

    const actionBar = h("div",{class:"selection-actions", id:"action-bar", style:"display:none;"});
    const grid = h("div",{class:"cards-grid", id:"cards-grid"});

    const backBtn = h("button",{class:"fixed-back-btn btn-active", "data-sfx":"tap_secondary", onClick: ()=> {
      if(ctx?.packKey === "dailyGroup"){
        openDailyGroups();
        return;
      }
      clear(app);
      renderLibrary();
    }},
      h("i",{class:"fas fa-arrow-right"}),
      isEn?"Back":"العودة للرزم"
    );

    const wrap = h("div",{class:"cards-pack-wrap"}, header, search, packTools, normalControls, selectionControls, actionBar, grid, backBtn);
    clear(app);
    app.appendChild(wrap);
    // SFX is delegated globally via wireSfx(document)

    let selecting = false;
    const selected = new Set();
    const isSpecial = (ctx?.packKey === "ignored" || ctx?.packKey === "completed");

    function currentIds(){
      if(ctx?.packKey === "dailyGroup"){
        const g = state.cards.dailyGroups[ctx.groupIso];
        return g?.cardIds ? [...g.cardIds] : [];
      }
      if(ctx?.packKey){
        return getPackCardIds(ctx.packKey);
      }
      return [...ids];
    }

    function statusLabel(c){
      const r = c.lastRating;
      if(r === "easy") return isEn?"Easy":"سهل";
      if(r === "medium") return isEn?"Medium":"متوسط";
      if(r === "hard") return isEn?"Hard":"صعب";
      return "—";
    }

    function statusClass(c){
      const r = c.lastRating;
      if(r === "easy") return "easy";
      if(r === "medium") return "medium";
      if(r === "hard") return "hard";
      return "none";
    }

    function filteredCards(){
      const query = (searchInput.value||"").trim().toLowerCase();
      const idsNow = currentIds();
      const cards = idsNow.map(id=> state.cards.byId[id]).filter(Boolean);
      if(!query) return cards;
      return cards.filter(c=> ((c.a||"") + " " + (c.b||"") + " " + (c.hint||"")).toLowerCase().includes(query));
    }

    function actionBtn({label, icon, onClick, disabled=false, danger=false, disabledTip, sfx}){
      const cls = `action-btn${disabled?" disabled":""}${danger?" danger":""}`;
      const btn = h("div",{
        class: cls,
        "data-sfx": disabled ? "tap_secondary" : (sfx || (danger ? "tahther" : "naker")),
        onClick: (e)=>{
          if(disabled){
            e.preventDefault();
            e.stopPropagation();
            if(disabledTip) tip(btn, disabledTip);
            return false;
          }
          return onClick?.();
        }
      },
        h("i",{class: icon, ariaHidden:"true"}),
        label
      );
      return btn;
    }

    function requireSelection(){
      if(selected.size === 0){
        toast(isEn?"Select cards first":"حدد بطاقات أولاً");
        return false;
      }
      return true;
    }

    function renameCustomPack(){
      if(!isCustomPack || !customPackId) return false;
      const p = state.library.customPacks.find(x=>x.id===customPackId);
      if(!p){ toast(isEn?"Pack not found":"الرزمة غير موجودة"); return false; }

      const input = h("input",{maxlength:"30", value: p.name || ""});
      const body = h("div",{},
        h("div",{class:"field"},
          h("label",{}, isEn?"Name":"الاسم"),
          input
        )
      );

      modal(isEn?"Rename pack":"تغيير اسم الرزمة", body, [
        { label: isEn?"Cancel":"إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> { close(); return true; } },
        { label: isEn?"Save":"حفظ", kind:"primary", sfx:"hefth", onClick:(close)=> {
          const n = (input.value||"").trim();
          if(!n){ toast(isEn?"Name is empty":"الاسم فارغ"); return false; }
          const exists = state.library.customPacks.some(x=> x.id!==p.id && String(x.name||"").trim()===n);
          if(exists){ toast(isEn?"Name already used":"اسم مستخدم"); return false; }
          p.name = n;
          packTitle = n;
          try{ titleEl.textContent = n; }catch{}
          saveState(state);
          close();
          toast(isEn?"Done":"تم");
          return true;
        }},
      ]);
      return true;
    }

    function deleteCustomPack(){
      if(!isCustomPack || !customPackId) return false;
      const p = state.library.customPacks.find(x=>x.id===customPackId);
      if(!p){ toast(isEn?"Pack not found":"الرزمة غير موجودة"); return false; }

      const body = h("div",{},
        h("div",{class:"subtle", style:"white-space:pre-wrap; line-height:1.8;"},
          isEn
            ? "Delete this pack? Cards will stay in your library."
            : "هل تريد حذف هذه الرزمة؟\nالبطاقات لن تُحذف من مكتبتك."
        )
      );

      modal(isEn?"Confirm":"تأكيد", body, [
        { label: isEn?"Cancel":"إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> { close(); return true; } },
        { label: isEn?"Delete":"حذف", kind:"danger", sfx:"hathef", onClick:(close)=> {
          state.library.customPacks = state.library.customPacks.filter(x=>x.id!==customPackId);
          saveState(state);
          close();
          toast(isEn?"Deleted":"تم الحذف");
          nav("#/library");
          return true;
        }},
      ]);
      return true;
    }

    function bulkShare(){
      if(!requireSelection()) return;
      const idsSel = Array.from(selected);
      exportShareDeck({ kind: ctx?.packKey ? "pack" : (ctx?.dayKey ? "dailyGroup" : "selection"), title: packTitle, cardIds: idsSel });
      toggleSelectionMode(false);
    }

    function bulkAddToPack(){
      if(!requireSelection()) return;
      const packs = state.library.customPacks;
      if(packs.length === 0){ toast(isEn?"No packs":"لا توجد رزم."); return; }
      const sel = new Set();
      const body = h("div",{}, ...packs.map(p=>{
        const btn = h("button",{
          class:"btn",
          "data-sfx":"tap_secondary",
          "aria-pressed": "false",
          onClick: ()=> {
            if(sel.has(p.id)) sel.delete(p.id);
            else sel.add(p.id);
            const isSelected = sel.has(p.id);
            btn.classList.toggle("selected", isSelected);
            btn.setAttribute("aria-pressed", String(isSelected));
          }
        }, p.name);
        return btn;
      }));
      modal(isEn?"Choose packs":"اختيار رزم", body, [
        { label: isEn?"Cancel":"إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() },
        { label: isEn?"Save":"حفظ", kind:"primary", sfx:"tap_primary", onClick:(close)=> {
          for(const pid of sel){
            const p = state.library.customPacks.find(x=>x.id===pid);
            if(!p) continue;
            for(const id of Array.from(selected)){
              if(!p.cardIds.includes(id)) p.cardIds.push(id);
            }
          }
          saveState(state);
          close();
          toast(isEn?"Done":"تم.");
          toggleSelectionMode(false);
        }}
      ]);
    }

    function bulkTrain(){
      if(!requireSelection()) return false;
      const idsSel = Array.from(selected);
      if(idsSel.length < 4){ toast(isEn?"Pick at least 4":"اختر 4 بطاقات على الأقل"); return false; }
      setTrainingPayload({ ids: idsSel, origin: "#/library" });
      selected.clear();
      toggleSelectionMode(false);
      nav("#/training");
      return true;
    }

    function bulkIgnore(){
      if(!requireSelection()) return false;
      for(const id of Array.from(selected)){
        const c = state.cards.byId[id];
        if(!c) continue;
        c.ignored = true;
        if(!state.cards.ignored.includes(id)) state.cards.ignored.unshift(id);
        // remove from custom packs
        for(const p of state.library.customPacks){
          p.cardIds = p.cardIds.filter(x=>x!==id);
        }
      }
      // remove from completed list if any
      state.cards.completed = state.cards.completed.filter(x=> !selected.has(x));
      saveState(state);
      toast(isEn?"Done":"تم.");
      toggleSelectionMode(false);
      return true;
    }

    function bulkReset(){
      if(!requireSelection()) return false;
      const iso = todayISO();
      for(const id of Array.from(selected)){
        const c = state.cards.byId[id];
        if(!c) continue;
        c.ignored = false;
        c.completed = false;
        c.level = 0;
        c.lastRating = null;
        c.frozen = false;
        c.lastReviewedIso = null;
        const gIso = c.groupIso || iso;
        c.groupIso = gIso;
        if(!state.cards.dailyGroups[gIso]){
          state.cards.dailyGroups[gIso] = { iso: gIso, titleDMY: formatDateDMY(gIso), cardIds: [] };
        }
        if(!state.cards.dailyGroups[gIso].cardIds.includes(id)){
          state.cards.dailyGroups[gIso].cardIds.push(id);
        }
      }
      state.cards.ignored = state.cards.ignored.filter(x=> !selected.has(x));
      state.cards.completed = state.cards.completed.filter(x=> !selected.has(x));
      saveState(state);
      toast(isEn?"Done":"تم.");
      toggleSelectionMode(false);
      return true;
    }

    function bulkDeleteForever(){
      if(!requireSelection()) return false;
      const body = h("div",{},
        h("div",{class:"subtle", style:"white-space:pre-wrap; line-height:1.8;"}, isEn?"Delete selected cards permanently? This cannot be undone.":"هل تريد حذف البطاقات المحددة نهائيًا؟ لا يمكن التراجع.")
      );
      modal(isEn?"Confirm":"تأكيد", body, [
        { label: isEn?"Cancel":"إلغاء", kind:"ghost", sfx:"naker", onClick:(close)=> close() },
        { label: isEn?"Delete":"حذف", kind:"danger", sfx:"hathef", onClick:(close)=> {
          for(const id of Array.from(selected)){
            delete state.cards.byId[id];
            state.cards.order = state.cards.order.filter(x=>x!==id);
            state.cards.ignored = state.cards.ignored.filter(x=>x!==id);
            state.cards.completed = state.cards.completed.filter(x=>x!==id);
            for(const g of Object.values(state.cards.dailyGroups)){
              g.cardIds = g.cardIds.filter(x=>x!==id);
            }
            for(const p of state.library.customPacks){
              p.cardIds = p.cardIds.filter(x=>x!==id);
            }
          }
          saveState(state);
          close();
          toast(isEn?"Deleted":"تم الحذف");
          toggleSelectionMode(false);
          return true;
        }}
      ]);
      return true;
    }

    function updateActionBar(){
      if(!selecting) return;
      clear(actionBar);

      if(isSpecial){
        actionBar.append(
          actionBtn({ label: isEn?"Delete":"حذف", icon:"fas fa-trash-alt", danger:true, sfx:"naker", onClick: ()=> bulkDeleteForever() }),
          actionBtn({ label: isEn?"Reset":"إعادة", icon:"fas fa-rotate-left", onClick: ()=> bulkReset() }),
          actionBtn({ label: isEn?"Share":"مشاركة", icon:"fas fa-share-alt", onClick: ()=> bulkShare() }),
          h("div",{class:"action-btn ghost"}, h("i",{class:"fas fa-minus", ariaHidden:"true"}), " ")
        );
        return;
      }

      const canTrain = selected.size >= 4;
      actionBar.append(
        actionBtn({ label: isEn?"Ignore":"تجاهل", icon:"fas fa-eye-slash", danger:true, sfx:"tagahl", onClick: ()=> bulkIgnore() }),
        actionBtn({ label: isEn?"Training":"تدريب", icon:"fas fa-dumbbell", disabled: !canTrain, disabledTip: isEn?"Pick at least 4":"اختر 4 بطاقات على الأقل", sfx:"play", onClick: ()=> bulkTrain() }),
        actionBtn({ label: isEn?"Add":"إضافة", icon:"fas fa-folder-plus", onClick: ()=> bulkAddToPack() }),
        actionBtn({ label: isEn?"Share":"مشاركة", icon:"fas fa-share-alt", onClick: ()=> bulkShare() }),
      );
    }

    function rebuild(){
      clear(grid);
      const cards = filteredCards();
      for(const c of cards){
        const tile = h("div",{
          class: `word-card btn-active${selected.has(c.id)?" selected":""}`,
          "data-sfx": selecting ? "tap_secondary" : "naker",
          onClick: ()=> handleCardClick(c.id)
        },
          h("span",{class:"word"}, c.a),
          h("span",{class:"lvl"}, isEn?`Level ${c.level}`:`مستوى ${c.level}`),
          h("span",{class:`status ${statusClass(c)}`}, statusLabel(c))
        );
        grid.appendChild(tile);
      }
      updateActionBar();
    }

    function handleCardClick(id){
      if(!selecting){
        openCardDetails(id, ctx);
        return;
      }
      if(selected.has(id)) selected.delete(id);
      else selected.add(id);
      rebuild();
    }

    function toggleSelectionMode(enable){
      selecting = enable;
      selected.clear();
      normalControls.style.display = enable ? "none" : "block";
      selectionControls.style.display = enable ? "flex" : "none";
      actionBar.style.display = enable ? "grid" : "none";
      rebuild();
    }

    function selectAllCards(){
      if(!selecting) return;
      const cards = filteredCards();
      const allSelected = (cards.length > 0) && cards.every(c=> selected.has(c.id));
      for(const c of cards){
        if(allSelected) selected.delete(c.id);
        else selected.add(c.id);
      }
      rebuild();
    }

    searchInput.addEventListener("input", rebuild, {passive:true});

    rebuild();
  }

  function openBulkActions(ctx, selected, refresh){
    if(selected.size === 0){
      toast("حدّد بطاقات أولاً.");
      return;
    }

    const isSpecial = (ctx.packKey === "ignored" || ctx.packKey === "completed");

    let trainBtn = null;
    const canTrain = (!isSpecial) && (selected.size >= 4);

    if(!isSpecial){
      trainBtn = h("button",{
        class: `btn ${canTrain ? "" : "disabled"}`,
        "data-sfx": canTrain ? "laeb" : null,
        onPointerdown: (e)=>{
          if(canTrain) return;
          e.preventDefault();
          e.stopPropagation();
          tip(trainBtn, "اختر 4 بطاقات على الأقل");
        },
        onClick: (e)=>{
          if(canTrain){ bulkTrain(); return; }
          e.preventDefault();
          tip(trainBtn, "اختر 4 بطاقات على الأقل");
        }
      }, "تدريب");
    }

    const body = isSpecial
      ? h("div",{},
          h("div",{class:"subtle"}, "إجراءات"),
          h("div",{class:"hr"}),
          h("div",{class:"row"},
            h("button",{class:"btn danger", "data-sfx":"hathef", onClick: ()=> bulkDeleteForever()}, "حذف نهائي"),
            h("button",{class:"btn", "data-sfx":"naker", onClick: ()=> bulkReset()}, "إعادة"),
          )
          ,h("div",{class:"row", style:"margin-top:10px;"},
            h("button",{class:"btn", "data-sfx":"naker", onPointerup: ()=> { m.close(); bulkShare(); }, onClick: ()=> { m.close(); bulkShare(); }}, "مشاركة")
          )
        )
      : h("div",{},
          h("div",{class:"subtle"}, "إجراءات"),
          h("div",{class:"hr"}),
          h("div",{class:"row"},
            h("button",{class:"btn danger", "data-sfx":"tahther", onClick: ()=> bulkIgnore()}, "تجاهل"),
            trainBtn,
          ),
          h("div",{class:"row", style:"margin-top:10px;"},
            h("button",{class:"btn", "data-sfx":"naker", onClick: ()=> bulkAddToPack()}, "إضافة إلى رزمة"),
            h("button",{class:"btn", "data-sfx":"naker", onPointerup: ()=> { m.close(); bulkShare(); }, onClick: ()=> { m.close(); bulkShare(); }}, "مشاركة"),
            (ctx.packKey?.startsWith?.("custom:") ? h("button",{class:"btn", "data-sfx":"tap_secondary", onClick: ()=> bulkRemoveFromPack(ctx.packKey.split(':')[1])}, "إزالة من الرزمة") : h("div",{}))
          )
        );

    const m = modal("التحديد", body, []);

    function idsSel(){ return Array.from(selected); }

    function bulkIgnore(){
      for(const id of idsSel()){
        const c = state.cards.byId[id];
        if(!c) continue;
        c.ignored = true;
        state.cards.ignored.unshift(id);
        // remove from custom packs
        for(const p of state.library.customPacks){
          p.cardIds = p.cardIds.filter(x=>x!==id);
        }
      }
      saveState(state);
      toast("تم.");
      selected.clear();
      m.close();
      refresh();
    }

    function bulkTrain(){
      const ids = idsSel();
      if(ids.length < 4){ toast("الحد الأدنى 4."); return; }
      // تدريب: تشغيل الألعاب الأربع عشوائياً بدون تقييم، والجوائز تُحسب بعد اللعب
      setTrainingPayload({
        ids,
        origin: "#/library"
      });
      selected.clear();
      m.close();
      refresh();
      nav("#/training");
    }

    function bulkShare(){
      const ids = idsSel();
      if(ids.length === 0){ toast("اختر بطاقة واحدة على الأقل."); return; }
      exportShareDeck({ kind: ctx.packKey ? "pack" : (ctx.dayKey ? "dailyGroup" : "selection"), title, cardIds: ids });
    }


    function bulkAddToPack(){
      const packs = state.library.customPacks;
      if(packs.length === 0){ toast("لا توجد رزم."); return; }
      const sel = new Set();
      const body = h("div",{}, ...packs.map(p=>{
        const btn = h("button",{
          class:"btn",
          "data-sfx":"tap_secondary",
          "aria-pressed": "false",
          onClick: ()=> {
            if(sel.has(p.id)) sel.delete(p.id);
            else sel.add(p.id);
            const isSelected = sel.has(p.id);
            btn.classList.toggle("selected", isSelected);
            btn.setAttribute("aria-pressed", String(isSelected));
          }
        }, p.name);
        return btn;
      }));
      modal("اختيار رزم", body, [
        { label:"حفظ", kind:"primary", sfx:"tap_primary", onClick:(close)=> {
          for(const pid of sel){
            const p = state.library.customPacks.find(x=>x.id===pid);
            if(!p) continue;
            for(const id of idsSel()){
              if(!p.cardIds.includes(id)) p.cardIds.push(id);
            }
          }
          saveState(state);
          close();
          toast("تم.");
          m.close();
          selected.clear();
          refresh();
        }},
      ]);
    }

    function bulkRemoveFromPack(packId){
      const p = state.library.customPacks.find(x=>x.id===packId);
      if(!p) return;
      p.cardIds = p.cardIds.filter(id=> !selected.has(id));
      saveState(state);
      toast("تم.");
      selected.clear();
      m.close();
      refresh();
    }

    function bulkDeleteForever(){
      for(const id of idsSel()){
        // remove from main lists
        delete state.cards.byId[id];
        state.cards.order = state.cards.order.filter(x=>x!==id);
        state.cards.ignored = state.cards.ignored.filter(x=>x!==id);
        state.cards.completed = state.cards.completed.filter(x=>x!==id);

        // remove from daily groups
        for(const g of Object.values(state.cards.dailyGroups)){
          g.cardIds = g.cardIds.filter(x=>x!==id);
        }

        // remove from custom packs
        for(const p of state.library.customPacks){
          p.cardIds = p.cardIds.filter(x=>x!==id);
        }
      }
      saveState(state);
      toast("تم.");
      selected.clear();
      m.close();
      refresh();
    }

    function bulkReset(){
      const iso = todayISO();
      for(const id of idsSel()){
        const c = state.cards.byId[id];
        if(!c) continue;
        c.ignored = false;
        c.completed = false;
        c.level = 0;
        c.lastRating = null;
        c.frozen = false;
        c.lastReviewedIso = null;
        // keep groupIso if exists; ensure group exists
        const gIso = c.groupIso || iso;
        c.groupIso = gIso;
        if(!state.cards.dailyGroups[gIso]){
          state.cards.dailyGroups[gIso] = { iso: gIso, titleDMY: formatDateDMY(gIso), cardIds: [] };
        }
        if(!state.cards.dailyGroups[gIso].cardIds.includes(id)){
          state.cards.dailyGroups[gIso].cardIds.push(id);
        }
      }
      // remove from ignored/completed lists
      state.cards.ignored = state.cards.ignored.filter(x=> !selected.has(x));
      state.cards.completed = state.cards.completed.filter(x=> !selected.has(x));

      saveState(state);
      toast("تم.");
      selected.clear();
      m.close();
      refresh();
    }
  }

  function runTraining(cards){
    // minimal: one combined reward using focus music and same formulas, no attendance
    const avgLevel = Math.round(cards.reduce((s,c)=>s+c.level,0)/Math.max(1,cards.length));
    const base = computeRewardsForGame({
      gameKey:"match",
      pairsOrCards: cards.length,
      successCount: cards.length,
      wrongCount: 0,
      avgSec: 6,
      usedHelp:false,
      levelAvg: avgLevel,
      rankKey: state.rating.rankKey,
      subLevel: state.rating.subLevel,
      priceExtraCard: 100
    });
    // Training tuning:
    // - keep XP/Rating at 50% of Play (as before)
    // - reduce Gold by an additional 50% (i.e., 25% of the base Play formula)
    const mult = 0.5;
    const goldMult = 0.25;
    const scaledXp = Math.round((base.xp || 0) * mult);
    const scaledGold = Math.round((base.gold || 0) * goldMult);
    const scaledRating = Math.round((base.rating || 0) * mult);
    state.economy.xp += scaledXp;
    state.economy.gold += scaledGold;
    state.economy.totalGoldEarned = Math.max(0, Number(state.economy.totalGoldEarned||0) + Number(scaledGold||0));
    applyRatingDelta(state, scaledRating);
    saveState(state);
    toast("تم.");
  }

  function openCardDetails(id, ctx){
    const c = state.cards.byId[id];
    if(!c) return;
    const body = h("div",{},
      h("div",{class:"subtle"}, c.a),
      h("div",{class:"subtle"}, c.b),
      h("div",{class:"subtle"}, c.hint),
      h("div",{class:"hr"}),
      h("div",{class:"subtle"}, `المستوى ${c.level}`),
      h("div",{class:"subtle"}, `آخر تقييم: ${labelRating(c.lastRating)}`),
    );
    modal("تفاصيل", body, [
      { label:"إغلاق", kind:"ghost", sfx:"naker", onClick:(close)=> close() },
      { label:"تجاهل", kind:"danger", sfx:"tahther", onClick:(close)=> { c.ignored = true; state.cards.ignored.unshift(id); saveState(state); close(); nav("#/library"); } },
      { label:"تعديل", kind:"primary", sfx:"naker", onClick:(close)=> { close(); openEdit(c); } },
    ]);
  }

  function openEdit(c){
    const a = h("input",{maxlength:"45", value:c.a});
    const b = h("input",{maxlength:"45", value:c.b});
    const hint = h("input",{maxlength:"60", value:c.hint});
    const body = h("div",{},
      h("div",{class:"field"}, h("label",{}, "النص الأصلي"), a),
      h("div",{class:"field"}, h("label",{}, "الترجمة أو التوضيح"), b),
      h("div",{class:"field"}, h("label",{}, "التلميح"), hint),
    );
    modal("تعديل", body, [
      { label:"إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() },
      { label:"حفظ", kind:"primary", sfx:"tap_primary", onClick:(close)=> {
        c.a = a.value; c.b = b.value; c.hint = hint.value;
        saveState(state);
        close();
        toast("تم.");
        nav("#/library");
      }},
    ]);
  }

  function getPackCardIds(key){
    if(key === "all"){
      return state.cards.order.filter(id=> {
        const c = state.cards.byId[id];
        return c && !c.ignored && !c.completed;
      });
    }
    // Virtual pack: cards due to play today. Uses the same cache as Play so the set matches.
    if(key === "dueToday"){
      const today = todayISO();
      let due = null;

      const cached = state.attendance.lessonCache?.[today];
      const cachedIds = Array.isArray(cached) ? cached : (cached?.cardIds);
      if(Array.isArray(cachedIds) && cachedIds.length){
        due = cachedIds
          .map(id=> state.cards.byId[id])
          .filter(c=> c && !c.ignored && !c.completed && !c.frozen);
        if(due.length === 0) due = null;
      }

      if(!due){
        due = dueCardsForToday(state)
          .filter(c=> c && !c.ignored && !c.completed && !c.frozen);
        state.attendance.lessonCache = state.attendance.lessonCache || {};
        state.attendance.lessonCache[today] = { cardIds: due.map(c=>c.id) };
        saveState(state);
      }
      return due.map(c=>c.id);
    }
    if(key === "easy" || key === "medium" || key === "hard"){
      return state.cards.order.filter(id=>{
        const c = state.cards.byId[id];
        return c && !c.ignored && !c.completed && c.lastRating === key;
      });
    }
    if(key === "ignored") return [...state.cards.ignored];
    if(key === "completed") return [...state.cards.completed];
    if(key.startsWith("custom:")){
      const id = key.split(":")[1];
      const p = state.library.customPacks.find(x=>x.id===id);
      return p ? [...p.cardIds] : [];
    }
    return [];
  }

  function labelRating(r){
    if(r==="easy") return "سهل";
    if(r==="medium") return "متوسط";
    if(r==="hard") return "صعب";
    return "—";
  }
}


function renderShop(){
  const title = (getLang()==="en") ? "Shop" : "المتجر";
  const econ = state.economy || {};

  const goldVal = h("span",{class:"gold-val"}, String(Math.max(0, Math.floor(state.economy.gold||0))));
  const header = h("header",{class:"page-header"},
    h("div",{style:"font-weight:bold;font-size:1.2rem;"}, title),
    h("div",{class:"gold-tag", "aria-label": (getLang()==="en") ? "Gold" : "الذهب"},
      h("i",{class:"fas fa-coins"}),
      goldVal
    )
  );

  const powerGrid = h("div",{class:"content-area shop-grid"});
  const adGrid     = h("div",{class:"content-area shop-grid"});
  const cosGrid   = h("div",{class:"content-area shop-grid"});

  // Power-ups (same logic, new UI)
  powerGrid.append(
    shopTile({
      name: (getLang()==="en") ? "Extra card" : "بطاقة إضافية",
      countText: (getLang()==="en") ? `Today: ${Number(econ.dailyExtraCardsPurchased||0)}/2` : `اليوم: ${Number(econ.dailyExtraCardsPurchased||0)}/2`,
      price: 100,
      icon: "fas fa-id-card",
      sfx:"mal",
      onBuy: ()=> buyExtraCard()
    }),
    shopTile({
      name: (getLang()==="en") ? "Help" : "مساعدة",
      countText: (getLang()==="en") ? `Owned: ${Number(econ.helps||0)}` : `معك: ${Number(econ.helps||0)}`,
      price: 150,
      icon: "fas fa-lightbulb",
      sfx:"mal",
      onBuy: ()=> buyCount("helps", 150)
    }),
    shopTile({
      name: (getLang()==="en") ? "Fuel" : "وقود",
      countText: (getLang()==="en") ? `Owned: ${Number(econ.fuel||0)}` : `معك: ${Number(econ.fuel||0)}`,
      price: 250,
      icon: "fas fa-gas-pump",
      sfx:"mal",
      onBuy: ()=> buyCount("fuel", 250)
    }),
    shopTile({
      name: (getLang()==="en") ? "Skip game" : "تخطي لعبة",
      countText: (getLang()==="en") ? `Owned: ${Number(econ.skips||0)}` : `معك: ${Number(econ.skips||0)}`,
      price: 500,
      icon: "fas fa-forward",
      sfx:"mal",
      onBuy: ()=> buyCount("skips", 500)
    }),
  );


  // Rewarded Ads (Watch & Earn)
  const adsSupported = dmAds.supported();
  const adsReady = dmAds.isReady();

  adGrid.append(
    shopAdTile({
      name: (getLang()==="en") ? "Watch 1 ad: +100 gold" : "شاهد إعلان: +100 ذهب",
      countText: adsSupported ? ((getLang()==="en") ? "Rewarded" : "مكافأة") : ((getLang()==="en") ? "Not available" : "غير متوفر"),
      icon: "fas fa-video",
      disabled: !adsSupported,
      onWatch: ()=> dmAds.show("gold100")
    }),
    shopAdTile({
      name: (getLang()==="en") ? "Watch 1 ad: +1 help" : "شاهد إعلان: +1 مساعدة",
      countText: adsSupported ? ((getLang()==="en") ? `Owned: ${Number(econ.helps||0)}` : `معك: ${Number(econ.helps||0)}`) : ((getLang()==="en") ? "Not available" : "غير متوفر"),
      icon: "fas fa-lightbulb",
      disabled: !adsSupported,
      onWatch: ()=> dmAds.show("help1")
    }),
    shopAdTile({
      name: (getLang()==="en") ? "Watch 2 ads: +1 fuel (daily)" : "شاهد إعلانين: +1 وقود (يوميًا)",
      countText: adsSupported
        ? ((Number(econ.dailyRewardedFuelClaimed||0) >= 1)
            ? ((getLang()==="en") ? "Claimed today" : "تم اليوم")
            : ((getLang()==="en") ? `Progress: ${Number(econ.dailyRewardedFuelProgress||0)}/2` : `التقدم: ${Number(econ.dailyRewardedFuelProgress||0)}/2`))
        : ((getLang()==="en") ? "Not available" : "غير متوفر"),
      icon: "fas fa-gas-pump",
      disabled: (!adsSupported) || (Number(econ.dailyRewardedFuelClaimed||0) >= 1),
      onWatch: ()=> dmAds.show("fuel1")
    }),
    shopAdTile({
      name: (getLang()==="en") ? "Watch 3 ads: +1 skip (daily)" : "شاهد 3 إعلانات: +1 تخطي (يوميًا)",
      countText: adsSupported
        ? ((Number(econ.dailyRewardedSkipClaimed||0) >= 1)
            ? ((getLang()==="en") ? "Claimed today" : "تم اليوم")
            : ((getLang()==="en") ? `Progress: ${Number(econ.dailyRewardedSkipProgress||0)}/3` : `التقدم: ${Number(econ.dailyRewardedSkipProgress||0)}/3`))
        : ((getLang()==="en") ? "Not available" : "غير متوفر"),
      icon: "fas fa-forward",
      disabled: (!adsSupported) || (Number(econ.dailyRewardedSkipClaimed||0) >= 1),
      onWatch: ()=> dmAds.show("skip1")
    }),
  );

  // Cosmetics
  rebuildCosmetics();

  const scroll = h("div",{class:"shop-scroll"},
    h("div",{class:"subtle"}, (getLang()==="en") ? "Power-ups" : "الأدوات التشغيلية"),
    powerGrid,
    h("div",{class:"hr"}),
    h("div",{class:"subtle"}, (getLang()==="en") ? "Watch & Earn" : "إعلانات مقابل جوائز"),
    adGrid,
    h("div",{class:"hr"}),
    h("div",{class:"subtle"}, (getLang()==="en") ? "Cosmetics" : "التخصيص"),
    cosGrid
  );

  const backBtn = h("button",{class:"fixed-back-btn btn-active", "data-sfx":"naker", onClick: ()=> navBack()},
    h("i",{class:"fas fa-arrow-right"}),
    (getLang()==="en") ? "Back" : "رجوع"
  );

  const panel = h("section",{class:"glass gold panel"},
    header,
    scroll,
    backBtn
  );
  app.appendChild(panel);

  function updateGoldUI(){
    goldVal.textContent = String(Math.max(0, Math.floor(state.economy.gold||0)));
  }


  function shopAdTile({name, countText="", icon, disabled=false, onWatch}){
    const tile = h("div",{class:"shop-item btn-active" + (disabled ? " disabled" : ""), role:"button", tabIndex: disabled ? "-1" : "0"}, 
      h("i",{class: icon, "aria-hidden":"true"}),
      h("div",{class:"shop-name"}, name),
      countText ? h("div",{class:"shop-count"}, countText) : null,
      h("div",{class:"price"}, (getLang()==="en") ? "Watch" : "مشاهدة")
    );
    if(disabled) return tile;
    const handler = ()=> {
      // Extra safety: block if ads are missing or in flight
      if(dmAds.isInFlight()){
        toast((getLang()==="en") ? "Please wait…" : "انتظر قليلًا…");
        return;
      }
      onWatch && onWatch();
    };
    tile.addEventListener("click", (e)=>{ e.__dm_sfx_ok = true; handler(); });
    tile.addEventListener("keydown", (e)=>{ if(e.key==="Enter"||e.key===" "){ e.preventDefault(); e.__dm_sfx_ok = true; handler(); }});
    return tile;
  }

  function shopTile({name, countText="", price, icon, sfx="tap_primary", onBuy}){
    const tile = h("div",{class:"shop-item btn-active", "data-sfx": sfx, role:"button", tabIndex:"0"},
      h("i",{class: icon, "aria-hidden":"true"}),
      h("div",{class:"shop-name"}, name),
      countText ? h("div",{class:"shop-count"}, countText) : null,
      h("div",{class:"price"}, String(price))
    );
    const handler = ()=> confirmBuy(name, price, ()=> {
      onBuy();
      saveState(state);
      syncTopbar();
      updateGoldUI();
    });
    tile.addEventListener("click", (e)=>{ e.__dm_sfx_ok = true; handler(); });
    tile.addEventListener("keydown", (e)=>{ if(e.key==="Enter"||e.key===" "){ e.preventDefault(); e.__dm_sfx_ok = true; handler(); }});
    return tile;
  }

  function confirmBuy(name, price, doBuy){
    const body = h("div",{class:"modal-body"},
      h("div",{class:"subtle"}, (getLang()==="en") ? `Buy ${name} for ${price} gold?` : `شراء ${name} مقابل ${price} ذهب؟`)
    );
    modal((getLang()==="en") ? "Confirm" : "تأكيد", body, [
      { label: (getLang()==="en") ? "Cancel" : "إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() },
      { label: (getLang()==="en") ? "Buy" : "شراء", kind:"primary", sfx:"schera", onClick:(close)=> {
        if(state.economy.gold < price){ toast((getLang()==="en") ? "Not enough gold." : "رصيد غير كافٍ."); return false; }
        doBuy();
        close();
        toast((getLang()==="en") ? "Done." : "تم.");
        render(); // refresh any dependent UI
        return true;
      }},
    ]);
  }

  function buyExtraCard(){
    if(state.economy.dailyExtraCardsPurchased >= 2){
      toast((getLang()==="en") ? "Daily limit reached." : "تم بلوغ الحد.");
      return;
    }
    state.economy.gold -= 100;
    state.economy.dailyExtraCardsPurchased += 1;
  }

  function buyCount(key, price){
    state.economy.gold -= price;
    state.economy[key] = (state.economy[key]||0) + 1;
  }

  function rebuildCosmetics(){
    clear(cosGrid);
    const gender = state.profile.gender;

    const bgItems = state.library.backgrounds.items;
    const avItems = state.library.avatars.items;
    const frItems = state.library.frames.items;
    const cbItems = (state.library.cardBacks && state.library.cardBacks.items) ? state.library.cardBacks.items : [];

    const bgOwned = new Set(state.library.backgrounds.owned);
    const avOwned = new Set(state.library.avatars.owned);
    const frOwned = new Set(state.library.frames.owned);
    const cbOwned = new Set((state.library.cardBacks && state.library.cardBacks.owned) ? state.library.cardBacks.owned : []);

    // Non-owned only + do not sell default gender items to the same gender
    const showBg = bgItems.filter(x=> x.gender !== gender && !bgOwned.has(x.id));
    const showAv = avItems.filter(x=> x.gender !== gender && !avOwned.has(x.id));
    const showFr = frItems.filter(x=> !frOwned.has(x.id) && Number(x.price||0) > 0);
    const showCb = cbItems.filter(x=> !cbOwned.has(x.id) && Number(x.price||0) > 0);

    // Frames then avatars then backgrounds, then card backs
    for(const it of showFr){ cosGrid.appendChild(cosmeticTile(it, "frames")); }
    for(const it of showAv){ cosGrid.appendChild(cosmeticTile(it, "avatars")); }
    for(const it of showBg){ cosGrid.appendChild(cosmeticTile(it, "backgrounds")); }
    for(const it of showCb){ cosGrid.appendChild(cosmeticTile(it, "cardBacks")); }

    if(showFr.length + showAv.length + showBg.length + showCb.length === 0){
      cosGrid.appendChild(h("div",{class:"subtle", style:"grid-column: 1 / -1; text-align:center; padding: 10px 0;"}, (getLang()==="en") ? "No items available." : "لا توجد عناصر متاحة."));
    }
  }

  function cosmeticTile(it, kind){
    const name = it.name;
    const price = Number(it.price||0);

    const thumbClass = kind==="frames" ? "shop-thumb is-frame" : (kind==="avatars" ? "shop-thumb is-avatar" : (kind==="backgrounds" ? "shop-thumb is-bg" : "shop-thumb"));
    const img = h("img",{src: it.src, alt: name, loading:"lazy"});
    const visual = h("div",{class: thumbClass}, img);

    const tile = h("div",{class:"shop-item btn-active", "data-sfx":"naker", role:"button", tabIndex:"0"},
      visual,
      h("div",{class:"shop-name"}, name),
      h("div",{class:"price"}, String(price))
    );

    const handler = ()=> confirmBuyCosmetic(tile, kind, it);
    tile.addEventListener("click", (e)=>{ e.__dm_sfx_ok = true; handler(); });
    tile.addEventListener("keydown", (e)=>{ if(e.key==="Enter"||e.key===" "){ e.preventDefault(); e.__dm_sfx_ok = true; handler(); }});
    return tile;
  }

  function confirmBuyCosmetic(tile, kind, it){
    const body = h("div",{class:"modal-body"},
      h("div",{class:"subtle"}, (getLang()==="en") ? `Buy ${it.name} for ${it.price} gold?` : `شراء ${it.name} مقابل ${it.price} ذهب؟`)
    );
    modal((getLang()==="en") ? "Confirm" : "تأكيد", body, [
      { label: (getLang()==="en") ? "Cancel" : "إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() },
      { label: (getLang()==="en") ? "Buy" : "شراء", kind:"primary", sfx:"schera", onClick:(close)=> {
        if(state.economy.gold < it.price){ toast((getLang()==="en") ? "Not enough gold." : "رصيد غير كافٍ."); return false; }
        buyCosmetic(kind, it);
        saveState(state);
        syncTopbar();
        updateGoldUI();
        close();
        toast((getLang()==="en") ? "Done." : "تم.");

        // Fade out then remove from shop after short delay (like original)
        tile.classList.add("owned");
        tile.querySelector(".price").textContent = tr("ممتلك");
        setTimeout(()=>{
          tile.style.transition = "opacity .22s ease, transform .22s ease";
          tile.style.opacity = "0";
          tile.style.transform = "translateY(-6px)";
          setTimeout(()=> tile.remove(), 240);
        }, 800);
        return true;
      }},
    ]);
  }

  function buyCosmetic(kind, it){
    state.economy.gold -= it.price;
    const lib = state.library[kind];
    if(!lib.owned.includes(it.id)) lib.owned.push(it.id);
  }
}


function renderProfile(){
  const p = state.profile;
  const econ = state.economy;
  const r = state.rating;

  // واجهة الملف الشخصي يجب أن تطابق ستايل البروتوتايب (الكود الثاني):
  // Hero + إحصائيات + أزرار تخصيص + زر رجوع ثابت. (لا نعرض محرر نبذة هنا)

  const accPct = accuracyPct();
  const heroStack = h("div",{class:"avatar-stack big", id:"profileAvatarStack", "aria-hidden":"true"});

  // Build avatar stack
  try{
    heroStack.innerHTML = "";
    const {frame, bg, av} = computeAvatarLayers(state);
    const frameImg = new Image(); frameImg.src = frame?.src || ""; frameImg.className = "layer frame";
    const bgImg = new Image(); bgImg.src = bg?.src || ""; bgImg.className = "layer bg";
    const avImg = new Image(); avImg.src = av?.src || ""; avImg.className = "layer av";
    // Layering per spec: frame (bottom) -> background -> avatar (top)
    heroStack.append(bgImg, avImg, frameImg);
  }catch(e){}

  const lvl = computeLevelFromXp(p.xp || 0);

  const header = h("header",{class:"page-header"},
    h("div",{style:"font-weight:bold;font-size:1.2rem;"}, "الملف الشخصي"),
    h("div",{class:"gold-tag", "aria-label":"الذهب"},
      h("i",{class:"fas fa-coins"}),
      h("span",{style:"margin-inline-start:8px;"}, String(Math.max(0, Math.floor(econ.gold||0))))
    )
  );

  const hero = h("div",{class:"profile-hero-section"},
    heroStack,
    h("div",{class:"profile-title-block"},
      h("div",{class:"profile-big-name"}, p.username),
      h("div",{class:"profile-level-pill"}, (getLang()==="en") ? `Level ${lvl.level}` : `مستوى ${lvl.level}`)
    ),
    h("div",{class:"stats-container"},
      h("div",{class:"stat-row"},
        h("span",{}, (getLang()==="en") ? `Rank: ` : "التصنيف: ", h("span",{class:"rank-chip"}, rankLabel(r.rankKey, r.subLevel))),
        h("button",{class:"ladder-btn btn-active", "data-sfx":"naker", onClick: ()=> openRanks()}, (getLang()==="en") ? "Ladder" : "سلم التصنيف")
      ),
      h("div",{class:"stat-row"},
        h("span",{}, (getLang()==="en") ? "Accuracy: " : "الدقة: ", h("span",{style:"font-weight:900;"}, `${accPct}%`))
      ),
      h("div",{class:"accuracy-bar-bg", "aria-hidden":"true"},
        h("div",{class:"accuracy-fill", style:`width:${accPct}%`})
      )
    )
  );

  const custom = h("div",{class:"custom-grid"},
    h("button",{class:"custom-btn btn-active", "data-sfx":"naker", onClick: ()=> openOwnedPicker("avatars")},
      h("i",{class:"fas fa-user-circle", style:"color:var(--accent-color)"}), (getLang()==="en") ? "Avatar" : "الافتار"
    ),
    h("button",{class:"custom-btn btn-active", "data-sfx":"naker", onClick: ()=> openOwnedPicker("backgrounds")},
      h("i",{class:"fas fa-image", style:"color:var(--accent-color)"}), (getLang()==="en") ? "Background" : "الخلفية"
    ),
    h("button",{class:"custom-btn btn-active", "data-sfx":"naker", onClick: ()=> openOwnedPicker("frames")},
      h("i",{class:"fas fa-border-style", style:"color:var(--accent-color)"}), (getLang()==="en") ? "Frame" : "الإطار"
    ),
    h("button",{class:"custom-btn btn-active", "data-sfx":"naker", onClick: ()=> openOwnedPicker("cardBacks")},
      h("i",{class:"fas fa-clone", style:"color:var(--accent-color)"}), (getLang()==="en") ? "Cards" : "البطاقات"
    )
  );

  const scroll = h("div",{class:"profile-scroll"},
    hero,
    custom
  );

  const backBtn = h("button",{class:"fixed-back-btn btn-active", "data-sfx":"naker", onClick: ()=> navBack()},
    h("i",{class:"fas fa-arrow-right"}),
    (getLang()==="en") ? "Back" : "رجوع"
  );

  // نفس بنية صفحة البروتوتايب: Header + محتوى قابل للتمرير + زر رجوع ثابت
  const wrap = h("div",{class:"profile-wrap"}, header, scroll, backBtn);
  app.appendChild(wrap);
  return;

  function accuracyPct(){
    const a = state.rating.accuracySeason;
    if(!a || a.total <= 0) return 0;
    return Math.max(0, Math.min(100, Math.round((a.correct/a.total)*100)));
  }

  function openOwnedPicker(kind){
    const title = kind === "avatars" ? ((getLang()==="en") ? "Avatars" : "الأفاتارات") :
                  kind === "backgrounds" ? ((getLang()==="en") ? "Backgrounds" : "الخلفيات") :
                  kind === "frames" ? ((getLang()==="en") ? "Frames" : "الإطارات") :
                  ((getLang()==="en") ? "Card backs" : "خلفيات البطاقات");
    const lib = state.library[kind];
    const owned = new Set(lib.owned || []);
    const items = (lib.items || []).filter(it => owned.has(it.id));

    const grid = h("div",{class:"cos-grid"});
    const m = modal(title, grid, []);

    for(const it of items){
      grid.appendChild(ownedTile(kind, it, ()=>{ m.close(); }));
    }

    if(items.length === 0){
      grid.appendChild(h("div",{class:"subtle"}, (getLang()==="en") ? "No items." : "لا توجد عناصر."));
    }

    window.lucide?.createIcons?.();
  }

  function ownedTile(kind, it, close){
    const selectedId = state.library[kind].selected;
    const isSelected = (selectedId === it.id);

    const preview = buildCosmeticPreview(kind, it.id);

    const btn = h("button",{class:"btn primary", "data-sfx":"naker", onClick: ()=> {
      if(isSelected){ close(); return; }
      state.library[kind].selected = it.id;
      saveState(state);
      updateTopbarUI();
      if(kind === "cardBacks") applyCardBackSkin();
      close();
    }}, isSelected ? ((getLang()==="en") ? "Active" : "مستخدم") : ((getLang()==="en") ? "Use" : "استخدام"));

    return h("div",{class:"cos-item glass gold"}, preview, h("div",{class:"cos-name"}, it.name), btn);
  }

  function buildCosmeticPreview(kind, id){
    if(kind === "cardBacks"){
      const it = (state.library.cardBacks.items||[]).find(x=>x.id===id);
      const src = it?.src || "";
      return h("div",{class:"cardback-preview", "aria-hidden":"true"},
        h("img",{src, alt: it?.name || "Card back", loading:"lazy"})
      );
    }

    const curFrame = state.library.frames.selected;
    const curBg = state.library.backgrounds.selected;
    const curAv = state.library.avatars.selected;

    const frameId = (kind === "frames") ? id : curFrame;
    const bgId = (kind === "backgrounds") ? id : curBg;
    const avId = (kind === "avatars") ? id : curAv;

    const tmp = {
      library: {
        frames: {...state.library.frames, selected: frameId},
        backgrounds: {...state.library.backgrounds, selected: bgId},
        avatars: {...state.library.avatars, selected: avId}
      },
      profile: state.profile
    };

    const stack = h("div",{class:"avatar-stack big preview", "aria-hidden":"true"});
    try{
      stack.innerHTML="";
      const {frame, bg, av} = computeAvatarLayers(tmp);
      const frameImg = new Image(); frameImg.src = frame?.src || ""; frameImg.className="layer frame";
      const bgImg = new Image(); bgImg.src = bg?.src || ""; bgImg.className="layer bg";
      const avImg = new Image(); avImg.src = av?.src || ""; avImg.className="layer av";
      // Layering per spec: frame (bottom) -> background -> avatar (top)
      stack.append(frameImg, bgImg, avImg);
    }catch(e){}
    return stack;
  }

  function openRanks(){
    const tiers = ["خشبي","حديدي","نحاسي","فضي","ذهبي","بلاتيني","ألماسي","أسطوري","مفكر","حكيم","ملهم"];
    const body = h("div",{class:"card-list"},
      ...tiers.map((name, idx)=> h("div",{class:"glass gold card"},
        h("div",{style:"font-weight:900;"}, name),
        h("div",{class:"subtle"}, (getLang()==="en") ? `Tier ${idx+1}` : `مرتبة ${idx+1}`)
      ))
    );
    modal((getLang()==="en") ? "Ladder" : "سلم التصنيفات", body, []);
  }
}



function renderSettings(){
  const econ = state.economy;
  const isEn = getLang()==="en";

  const music = Number(localStorage.getItem("dm_music") ?? "0.65");
  const sfx = Number(localStorage.getItem("dm_sfx") ?? "0.85");

  const m = h("input",{type:"range", min:"0", max:"1", step:"0.01", value:String(music)});
  const s = h("input",{type:"range", min:"0", max:"1", step:"0.01", value:String(sfx)});

  const header = h("header",{class:"page-header"},
    h("div",{style:"font-weight:bold;font-size:1.2rem;"}, isEn ? "Settings" : "الإعدادات"),
    h("div",{class:"gold-tag", "aria-label": isEn ? "Gold" : "الذهب"},
      h("i",{class:"fas fa-coins"}),
      h("span",{class:"gold-val", style:"margin-inline-start:8px;"}, String(Math.max(0, Math.floor(econ.gold||0))))
    )
  );

  const list = h("div",{class:"settings-list"},
    h("div",{class:"setting-item"},
      h("div",{class:"setting-label"}, h("i",{class:"fas fa-music"}), isEn ? "Music volume" : "مستوى الموسيقى"),
      m
    ),
    h("div",{class:"setting-item"},
      h("div",{class:"setting-label"}, h("i",{class:"fas fa-volume-up"}), isEn ? "Interaction volume" : "مستوى التفاعلات"),
      s
    ),

    // Vibration toggle
    (function(){
      const icon = h("i",{class: vibOn ? "fas fa-toggle-on" : "fas fa-toggle-off"});
      const txt = h("span",{class:"vib-txt"}, vibOn ? (isEn?"On":"شغال") : (isEn?"Off":"متوقف"));
      const btn = h("button",{type:"button", class:`vibrate-pill ${vibOn?"on":"off"} btn-active`, "data-sfx":"tap_secondary", onClick: ()=>{
          vibOn = !vibOn;
          setVibrationEnabled(vibOn);
          btn.classList.toggle("on", vibOn);
          btn.classList.toggle("off", !vibOn);
          icon.className = vibOn ? "fas fa-toggle-on" : "fas fa-toggle-off";
          txt.textContent = vibOn ? (isEn?"On":"شغال") : (isEn?"Off":"متوقف");
          playSfx("tap_secondary");
        }}, icon, txt);
      return h("div",{class:"setting-item"},
        h("div",{class:"setting-row"},
          h("div",{class:"setting-label"}, h("i",{class:"fas fa-mobile-screen-button"}), isEn ? "Tap vibration" : "اهتزاز اللمس"),
          btn
        )
      );
    })(),

    // Dark mode toggle
    (function(){
      const icon = h("i",{class: darkOn ? "fas fa-moon" : "fas fa-sun"});
      const txt = h("span",{}, darkOn ? (isEn?"Dark":"داكن") : (isEn?"Light":"فاتح"));
      const btn = h("button",{type:"button", class:`theme-pill ${darkOn?"on":"off"} btn-active`, "data-sfx":"tap_secondary", onClick: ()=>{
          darkOn = !darkOn;
          applyTheme(darkOn ? "dark" : "light");
          btn.classList.toggle("on", darkOn);
          btn.classList.toggle("off", !darkOn);
          icon.className = darkOn ? "fas fa-moon" : "fas fa-sun";
          txt.textContent = darkOn ? (isEn?"Dark":"داكن") : (isEn?"Light":"فاتح");
          playSfx("tap_secondary");
        }}, icon, txt);
      return h("div",{class:"setting-item"},
        h("div",{class:"setting-row"},
          h("div",{class:"setting-label"}, h("i",{class:"fas fa-circle-half-stroke"}), isEn ? "Theme" : "المظهر"),
          btn
        )
      );
    })(),

    h("button",{class:"btn-wide btn-active neutral", "data-sfx":"naker", onClick: ()=> openLangPicker()},
      h("i",{class:"fas fa-globe"}),
      isEn ? "Change language" : "تغيير اللغة"
    ),

    h("button",{class:"btn-wide btn-active neutral", "data-sfx":"naker", onClick: ()=> exportBackup()},
      h("i",{class:"fas fa-file-export"}),
      isEn ? "Export my data" : "تصدير بيانات الحساب"
    ),

    h("button",{class:"btn-wide btn-active neutral", "data-sfx":"naker", onClick: ()=> importBackup()},
      h("i",{class:"fas fa-file-import"}),
      isEn ? "Import my data" : "استيراد بيانات حساب"
    ),

    h("button",{class:"btn-wide btn-active neutral", "data-sfx":"naker", onClick: ()=> importSharedDeckFile()},
      h("i",{class:"fas fa-file-csv"}),
      isEn ? "Import shared deck" : "استيراد ملف مشاركة"
    ),

    h("button",{class:"btn-wide btn-active danger-outline", "data-sfx":"hathef", onClick: ()=> deleteFlow()},
      h("i",{class:"fas fa-trash-alt"}),
      isEn ? "Delete account" : "حذف الحساب"
    ),

    h("div",{class:"support-email"}, isEn ? "Support: deckmastery0@gmail.com" : "دعم فني: deckmastery0@gmail.com")
  );

  const backBtn = h("button",{class:"fixed-back-btn btn-active", "data-sfx":"naker", onClick: ()=> navBack()},
    h("i",{class:"fas fa-arrow-right"}),
    isEn ? "Back" : "رجوع"
  );

  const wrap = h("div",{class:"settings-wrap"}, header, list, backBtn);
  app.appendChild(wrap);

  function openLangPicker(){
    const body = h("div",{class:"modal-body"},
      h("div",{class:"subtle"}, isEn ? "Choose language:" : "اختر اللغة:"),
      h("div",{class:"row"},
        h("button",{class:"btn", "data-sfx":"naker", onClick: ()=>{ setLang("ar"); applyLang(); location.reload(); }}, "العربية"),
        h("button",{class:"btn", "data-sfx":"naker", onClick: ()=>{ setLang("en"); applyLang(); location.reload(); }}, "English")
      )
    );
    modal(isEn?"Language":"اللغة", body, [
      { label: isEn?"Close":"إغلاق", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() }
    ]);
  }

  m.addEventListener("input", ()=> {
    localStorage.setItem("dm_music", m.value);
    setVolumes({music:Number(m.value), sfx:Number(s.value)});
    playSfx("tap_secondary");
  }, {passive:true});

  s.addEventListener("input", ()=> {
    localStorage.setItem("dm_sfx", s.value);
    setVolumes({music:Number(m.value), sfx:Number(s.value)});
    playSfx("tap_secondary");
  }, {passive:true});

  function exportBackup(){
    downloadFullBackup();
  }


  function importBackup(){
    // Web version used <input type=file>. In Cordova/Android WebView that can fail.
    // Support Cordova pickers when available, while keeping the web fallback.

	    const isEn = getLang()==="en";
	    const isCordova = !!window.cordova;
	    const hasFilePlugin = isCordova && (typeof window.resolveLocalFileSystemURL === "function");

	    const fileChooserObj = (window.fileChooser)
	      || (window.cordova && window.cordova.plugins && (window.cordova.plugins.fileChooser || window.cordova.plugins.filechooser))
	      || null;
	    const hasFileChooser = !!fileChooserObj && (typeof fileChooserObj.open === "function");

	    const chooserObj = (window.chooser)
	      || (window.cordova && window.cordova.plugins && window.cordova.plugins.chooser)
	      || null;
	    const hasChooser = !!chooserObj && (typeof chooserObj.getFile === "function");

	    const filePathObj = (window.FilePath)
	      || (window.cordova && window.cordova.plugins && (window.cordova.plugins.filePath || window.cordova.plugins.filepath || window.cordova.plugins.FilePath))
	      || null;
	    const hasFilePath = !!filePathObj && (typeof filePathObj.resolveNativePath === "function");

    async function applyImportedJson(txt){
      const data = JSON.parse(txt);
      let importedState = null;
      let dmMusic = null;
      let dmSfx = null;
      let dmVibrate = null;
      let dmTheme = null;

      if(data && typeof data === "object" && data.storage && typeof data.storage === "object"){
        if(data.storage.deckmastery_v1) importedState = data.storage.deckmastery_v1;
        if(typeof data.storage.dm_music === "string") dmMusic = data.storage.dm_music;
        if(typeof data.storage.dm_sfx === "string") dmSfx = data.storage.dm_sfx;
        if(typeof data.storage.dm_vibrate === "string") dmVibrate = data.storage.dm_vibrate;
        if(typeof data.storage.dm_theme === "string") dmTheme = data.storage.dm_theme;
      } else {
        importedState = data;
      }

      if(!importedState || typeof importedState !== "object") throw new Error("bad");
      state = migrate(importedState);
      saveState(state);
      if(dmMusic != null) localStorage.setItem("dm_music", dmMusic);
      if(dmSfx != null) localStorage.setItem("dm_sfx", dmSfx);
      if(dmVibrate != null) localStorage.setItem("dm_vibrate", dmVibrate);
      if(dmTheme != null) localStorage.setItem("dm_theme", dmTheme);
      location.reload();
    }

	    // fallback: استخدم <input type=file> حتى داخل Cordova (ينجح في كثير من WebView الحديثة)
	    function openWebFileInput(){
	      const inp = document.createElement("input");
	      inp.type = "file";
	      inp.accept = "application/json,.json";
	      inp.onchange = async ()=> {
	        const file = inp.files?.[0];
	        if(!file) return;
	        try{ await applyImportedJson(await file.text()); }
	        catch(_){ toast(isEn ? "Invalid file." : "ملف غير صالح"); }
	      };
	      inp.click();
	    }

    function normalizeToFileUrl(pathOrUrl){
      const u = String(pathOrUrl||"");
      if(u.startsWith("file://")) return u;
      if(u.startsWith("/")) return "file://" + u;
      return u;
    }

    function readTextFromUrl(url, onOk, onErr){
      try{
        window.resolveLocalFileSystemURL(url, (entry)=>{
          entry.file((file)=>{
            const reader = new FileReader();
            reader.onloadend = ()=> onOk(String(reader.result||""));
            reader.onerror = ()=> onErr(reader.error || new Error("read_failed"));
            reader.readAsText(file);
          }, onErr);
        }, onErr);
      }catch(e){
        onErr(e);
      }
    }

    function cordovaPickAndRead(){
      if(!hasFilePlugin){
        toast(isEn ? "To import in APK, add cordova-plugin-file." : "للاستيراد داخل APK أضف cordova-plugin-file.");
        return;
      }
      if(!hasFileChooser && !hasChooser){
        toast(isEn ? "To import in APK, add a file chooser plugin." : "للاستيراد داخل APK أضف إضافة اختيار الملفات (File Chooser).\n\nمقترح: cordova-plugin-filechooser + cordova-plugin-filepath");
        return;
      }

      const onPicked = (uri)=>{
        if(!uri){ toast(isEn ? "No file selected." : "لم يتم اختيار ملف."); return; }

        const attemptRead = (u)=>{
          const url = normalizeToFileUrl(u);
	          readTextFromUrl(url, async (txt)=>{
            try{ await applyImportedJson(txt); }
            catch(e){ toast(isEn ? "Invalid file." : "ملف غير صالح"); }
	          }, ()=>{
	            toast(isEn ? "Could not read file." : "تعذّر قراءة الملف.");
	            // جرّب الطريقة البديلة
	            try{ openWebFileInput(); }catch(_){ }
	          });
        };

	      const s = String(uri);
	      // 1) جرّب قراءة الـ URI كما هو (بعض البيئات تسمح بـ content:// مباشرة)
	      // 2) إذا فشل وكانت content:// و FilePath متوفر: حوّله لمسار file:// ثم اقرأه.
	      if(s.startsWith("content:") && hasFilePath){
	        // حاول مباشرة أولاً
	        let triedFallback = false;
	        const directUrl = normalizeToFileUrl(uri);
	        readTextFromUrl(directUrl, async (txt)=>{
	          try{ await applyImportedJson(txt); }
	          catch(e){ toast(isEn ? "Invalid file." : "ملف غير صالح"); }
	        }, ()=>{
	          if(triedFallback) return;
	          triedFallback = true;
	          try{
	            filePathObj.resolveNativePath(uri, (p)=> attemptRead(p), ()=> attemptRead(uri));
	          }catch(_){
	            attemptRead(uri);
	          }
	        });
	      } else {
	        attemptRead(uri);
	      }
      };

      const onFail = ()=> toast(isEn ? "No file selected." : "لم يتم اختيار ملف.");

      try{
	        if(hasFileChooser){
	          fileChooserObj.open(onPicked, onFail);
	        } else if(hasChooser){
          // Some chooser plugins use (success, fail) and return { uri: ... }
	          const fn = chooserObj.getFile;
          if(fn.length >= 2){
	            fn.call(chooserObj, (file)=> onPicked(file && (file.uri || file.nativeURL || file.url || file.file || file.path)), onFail);
          } else {
	            fn.call(chooserObj, (file)=> onPicked(file && (file.uri || file.nativeURL || file.url || file.file || file.path)));
          }
        }
      }catch(e){
        toast(isEn ? "Could not open picker." : "تعذّر فتح اختيار الملفات.");
      }
    }

    // Cordova path (APK)
    if(isCordova){
      // Prefer Cordova picker if available; otherwise fall back to web input.
      if(hasFileChooser || hasChooser){
        cordovaPickAndRead();
        return;
      }
    }

	    // Web fallback
	    openWebFileInput();
  }


  function deleteFlow(){
    const b1 = h("div",{}, h("div",{class:"subtle"}, isEn ? "Are you sure?" : "هل أنت متأكد؟"));
    modal(isEn?"Delete":"حذف", b1, [
      { label: isEn?"Cancel":"إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() },
      { label: isEn?"Continue":"متابعة", kind:"danger", sfx:"hathef", onClick:(close)=> { close(); step2(); } },
    ]);
    function step2(){
      const b2 = h("div",{}, h("div",{style:"color:var(--danger);font-weight:900;"}, isEn ? "Warning: this cannot be undone." : "انتبه! بعد الحذف لن تتمكن من استعادة بياناتك."));
      modal(isEn?"Warning":"تحذير", b2, [
        { label: isEn?"Cancel":"إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() },
        { label: isEn?"Continue":"متابعة", kind:"danger", sfx:"hathef", onClick:(close)=> { close(); step3(); } },
      ]);
    }
    function step3(){
      const pass = h("input",{});
      const b3 = h("div",{}, h("div",{class:"field"}, h("label",{}, isEn ? "Password" : "كلمة السر"), pass));
      modal(isEn?"Final confirmation":"تأكيد نهائي", b3, [
        { label: isEn?"Cancel":"إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() },
        { label: isEn?"Delete":"حذف", kind:"danger", sfx:"hathef", onClick:(close)=> {
          // حذف متوافق مع Android/Cordova (SQLite + localStorage)
          deleteStateStorage();
          close();
          location.reload();
        }},
      ]);
    }
  }
}


function renderNotifications(){
  const econ = state.economy;
  const isEn = getLang()==="en";

  const header = h("header",{class:"page-header"},
    h("div",{style:"font-weight:bold;font-size:1.2rem;"}, isEn ? "Notifications" : "الإشعارات"),
    h("div",{class:"gold-tag", "aria-label": isEn ? "Gold" : "الذهب"},
      h("i",{class:"fas fa-coins"}),
      h("span",{class:"gold-val", style:"margin-inline-start:8px;"}, String(Math.max(0, Math.floor(econ.gold||0))))
    )
  );

  // Build items (render state first, then mark read)
  const list = h("div",{class:"settings-list noti-list"});
  if(!state.notifications.items.length){
    list.appendChild(h("div",{class:"setting-item"},
      h("div",{class:"subtle", style:"text-align:center;"}, isEn ? "No notifications yet." : "لا توجد إشعارات بعد.")
    ));
  } else {
    for(const n of state.notifications.items){
      const title = n.title || (isEn ? "Notification" : "تنبيه");
      const body = n.body || "";
      const isNew = !n.readAt;
      const icon = n.claimable ? "fas fa-gift" : (n.type==="level" ? "fas fa-star" : "fas fa-bell");

      const badge = h("div",{class:"kbd"}, isNew ? (isEn ? "NEW" : "جديد") : (isEn ? "Read" : "مقروء"));

      const action = n.claimable
        ? h("button",{
            class:"btn small primary btn-active noti-claim",
            "data-sfx":"tap_primary",
            disabled: !!n.claimed,
            onClick: ()=> claim(n.id)
          }, n.claimed ? (isEn ? "Done" : "تم") : (isEn ? "Claim" : "استلام"))
        : null;

      const metaRow = h("div",{class:"noti-meta"}, badge, action || h("div",{}));

      const item = h("div",{class:"setting-item noti-item"},
        h("div",{class:"setting-label"},
          h("i",{class: icon}),
          h("span",{}, title)
        ),
        body ? h("div",{class:"subtle noti-body"}, body) : null,
        metaRow
      );

      // allow tap to open/claim quickly
      if(n.claimable && !n.claimed){
        item.style.cursor = "pointer";
        item.addEventListener("click", (ev)=>{
          // avoid double trigger if button clicked
          if((ev.target && ev.target.closest && ev.target.closest("button")) ) return;
          claim(n.id);
        });
      }

      list.appendChild(item);
    }
  }

  // mark read after building UI
  const now = Date.now();
  let changed = false;
  for(const n of state.notifications.items){
    if(!n.readAt){ n.readAt = now; changed = true; }
  }
  if(changed){
    saveState(state);
    syncTopbar();
  }

  const backBtn = h("button",{class:"fixed-back-btn btn-active", "data-sfx":"naker", onClick: ()=> navBack()},
    h("i",{class:"fas fa-arrow-right"}),
    isEn ? "Back" : "رجوع"
  );

  const wrap = h("div",{class:"notifications-wrap"}, header, list, backBtn);
  app.appendChild(wrap);

  function claim(id){
    const n = state.notifications.items.find(x=>x.id===id);
    if(!n || n.claimed) return;
    n.claimed = true;
    saveState(state);
    toast(isEn ? "Claimed." : "تم الاستلام.");
    render();
  }
}


function renderStreak(){
  const econ = state.economy;
  const isEn = getLang()==="en";
  const segs = state.streak.segments || [];

  const header = h("header",{class:"page-header"},
    h("div",{style:"font-weight:bold;font-size:1.2rem;"}, isEn ? "Streak" : "الحماس"),
    h("div",{class:"gold-tag", "aria-label": isEn ? "Gold" : "الذهب"},
      h("i",{class:"fas fa-coins"}),
      h("span",{class:"gold-val", style:"margin-inline-start:8px;"}, String(Math.max(0, Math.floor(econ.gold||0))))
    )
  );

  const hero = h("div",{class:"streak-hero-section"},
    h("div",{class:"streak-icon", "aria-hidden":"true"}, h("i",{class:"fas fa-fire"})),
    h("div",{class:"streak-title"}, isEn ? "Your current streak" : "حماسك الحالي"),
    h("div",{class:"streak-number"}, String(state.streak.current||0)),
    h("div",{class:"streak-meta-row"},
      h("div",{class:"kbd"}, (isEn ? "Fuel: " : "الوقود: ") + String(econ.fuel||0)),
      h("div",{class:"subtle"}, isEn ? "Tap a missed day to reconnect." : "اضغط على يوم مفقود لتوصيل الحماس.")
    )
  );

  // build visual days list from segments
  const days = [];
  if(segs.length){
    let num = 1;
    for(const s of segs){
      for(let i=0;i<(s.length||0);i++) days.push({ n:num++, type:"on" });
      const missed = s.missed||[];
      for(let j=0;j<missed.length;j++) days.push({ n:num++, type:"off" });
    }
  }

  const grid = h("div",{class:"streak-days-grid"});
  const last30 = days.slice(-30);
  if(!last30.length){
    grid.appendChild(h("div",{class:"subtle", style:"text-align:center; padding: 10px 0;"}, isEn ? "No streak yet. Play today to start!" : "لا يوجد حماس بعد. العب اليوم لتبدأ!"));
  } else {
    last30.forEach(d=>{
      const cls = `day-btn btn-active ${d.type==="off" ? "off" : "on"}`;
      const sfx = d.type==="off" ? "tap_danger" : "tap_secondary";
      const btn = h("button",{class: cls, "data-sfx": sfx, onClick: ()=> onDay(d)}, String(d.n));
      grid.appendChild(btn);
    });
  }

  const backBtn = h("button",{class:"fixed-back-btn btn-active", "data-sfx":"naker", onClick: ()=> navBack()},
    h("i",{class:"fas fa-arrow-right"}),
    isEn ? "Back" : "رجوع"
  );

  const wrap = h("div",{class:"streak-wrap"}, header, hero, grid, backBtn);
  app.appendChild(wrap);

  function onDay(d){
    if(d.type !== "off") return;
    const last = segs[segs.length-2];
    if(!last || !(last.missed?.length)) return;
    const need = last.missed.length;
    const body = h("div",{},
      h("div",{class:"subtle"}, (isEn ? `Use ${need} fuel to reconnect?` : `استخدام ${need} وقود لتوصيل الحماسة؟`))
    );
    modal(isEn ? "Reconnect" : "توصيل", body, [
      { label: isEn?"Cancel":"إلغاء", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() },
      { label: isEn?"Confirm":"تأكيد", kind:"primary", sfx:"tap_primary", onClick:(close)=> {
        const r = reconnectStreak(state);
        if(!r.ok){ toast(isEn ? "Not enough fuel." : "رصيد غير كافٍ."); return; }
        saveState(state);
        close();
        toast(isEn ? "Done." : "تم.");
        render();
      }},
    ]);
  }
}

function renderTasks(){
  clear(app);
  const isEn = getLang() === "en";
  const view = getTasksView(state, isEn ? "en" : "ar");

  const hasClaimable = (arr)=> Array.isArray(arr) && arr.some(x=> x && x.done && !x.claimed);
  let dotDaily = hasClaimable(view.daily);
  let dotWeekly = hasClaimable(view.weekly);
  let dotAch = hasClaimable(view.achievements);

  let tab = "daily"; // daily | weekly | achievements

  const header = h("div",{class:"page-header"},
    h("div",{},
      h("div",{class:"h1"}, isEn ? "Tasks" : "المهمات"),
      h("div",{class:"subtle"}, isEn ? "Complete tasks in Play and claim rewards here." : "أكمل المهمات عبر زر اللعب، واستلم الجوائز من هنا.")
    ),
    h("button",{class:"btn small btn-active", "data-sfx":"tap_secondary", onClick: ()=> openGlobalInfo()},
      h("i",{class:"fas fa-circle-info"}),
      isEn ? "Info" : "معلومات"
    )
  );

  const tabs = h("div",{class:"task-tabs"},
    tabBtn("daily", isEn?"Daily":"يومية"),
    tabBtn("weekly", isEn?"Weekly":"أسبوعية"),
    tabBtn("achievements", isEn?"Achievements":"إنجازات")
  );

  const list = h("div",{class:"settings-list tasks-list"});
  const backBtn = h("button",{class:"fixed-back-btn btn-active", "data-sfx":"naker", onClick: ()=> navBack()},
    h("i",{class:"fas fa-arrow-right"}),
    isEn ? "Back" : "رجوع"
  );

  const wrap = h("div",{class:"settings-wrap tasks-wrap"}, header, tabs, list, backBtn);
  app.appendChild(wrap);

  renderList();

  function refreshTabDots(){
    dotDaily = hasClaimable(view.daily);
    dotWeekly = hasClaimable(view.weekly);
    dotAch = hasClaimable(view.achievements);
    const d1 = document.getElementById("uiTasksDotDaily");
    const d2 = document.getElementById("uiTasksDotWeekly");
    const d3 = document.getElementById("uiTasksDotAch");
    if(d1) d1.hidden = !dotDaily;
    if(d2) d2.hidden = !dotWeekly;
    if(d3) d3.hidden = !dotAch;
  }

  function tabBtn(key, label){
    const showDot = (key==="daily") ? dotDaily : (key==="weekly") ? dotWeekly : dotAch;
    const dotId = (key==="daily") ? "uiTasksDotDaily" : (key==="weekly") ? "uiTasksDotWeekly" : "uiTasksDotAch";
    const b = h("button",{class:`btn small btn-active ${key===tab?"selected":""}`, "data-sfx":"tap_secondary", onClick: ()=>{ tab = key; updateTabs(); renderList(); }},
      label,
      h("span",{class:"dot", id: dotId, hidden: !showDot, style:"top:6px; left:6px; width:8px; height:8px;"})
    );
    b.dataset.key = key;
    return b;
  }

  function updateTabs(){
    for(const b of tabs.querySelectorAll("button")){
      b.classList.toggle("selected", b.dataset.key === tab);
    }
  }

  function renderList(){
    clear(list);
    refreshTabDots();
    const items = (tab==="daily") ? view.daily : (tab==="weekly") ? view.weekly : view.achievements;

    // section hint
    if(tab === "daily"){
      list.appendChild(h("div",{class:"task-section-hint"}, isEn ? "Resets every day." : "تتجدد يوميًا."));
    } else if(tab === "weekly"){
      list.appendChild(h("div",{class:"task-section-hint"}, isEn ? "Resets every Monday at 00:00." : "تتجدد كل يوم اثنين الساعة 00:00."));
    } else {
      list.appendChild(h("div",{class:"task-section-hint"}, isEn ? "One-time rewards." : "مكافآت تُستلم مرة واحدة فقط."));
    }

    for(const t of items){
      list.appendChild(taskItem(t, tab));
    }
  }

  function taskItem(t, scope){
    const top = h("div",{class:"task-top"},
      h("div",{class:"task-title"}, t.title),
      h("div",{class:"task-actions"},
        h("span",{class:"task-reward"}, t.rewardText),
        h("button",{class:"task-info btn-active", "data-sfx":"tap_secondary", onClick: ()=> openTaskInfo(t)}, "!")
      )
    );

    const pct = Math.max(0, Math.min(1, t.max ? (t.cur / t.max) : 0));
    const bar = h("div",{class:"task-bar"}, h("div",{class:"task-bar-fill", style:`width:${Math.round(pct*100)}%` }));
    const progTxt = h("div",{class:"task-prog"}, `${t.cur}/${t.max}`);

    let right;
    if(t.claimed){
      right = h("span",{class:"task-claimed"}, isEn ? "Claimed" : "تم الاستلام");
    } else if(t.done){
      right = h("button",{class:"btn small primary btn-active", "data-sfx":"tap_primary", onClick: ()=> claim(t, scope)}, isEn ? "Claim" : "استلام");
    } else {
      right = h("span",{class:"task-pending"}, isEn ? "In progress" : "قيد التقدم");
    }

    const bottom = h("div",{class:"task-bottom"}, bar, progTxt, right);

    return h("div",{class:"setting-item task-item"}, top, bottom);
  }

  function openTaskInfo(t){
    const body = h("div",{class:"modal-body"},
      h("div",{class:"subtle"}, t.desc),
      h("div",{class:"hr"}),
      h("div",{class:"subtle"}, (isEn?"Reward":"الجائزة") + ": " + t.rewardText)
    );
    modal(t.title, body, [
      { label: isEn?"Close":"إغلاق", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() }
    ]);
  }

  function openGlobalInfo(){
    const body = h("div",{class:"modal-body"},
      h("div",{class:"subtle"}, isEn
        ? "Daily and weekly tasks only progress through the Play button (training does not count)."
        : "المهمات اليومية والأسبوعية تتقدم فقط عبر زر اللعب (قسم التدريب لا يُحتسب)."
      ),
      h("div",{class:"subtle"}, isEn
        ? "Achievements are one-time milestones."
        : "الإنجازات هي مراحل تُستلم مرة واحدة فقط."
      )
    );
    modal(isEn?"Tasks":"المهمات", body, [
      { label: isEn?"Close":"إغلاق", kind:"ghost", sfx:"tap_secondary", onClick:(close)=> close() }
    ]);
  }

  function claim(t, scope){
    const res = claimTaskReward(state, scope, t.id);
    if(!res?.ok){
      toast(isEn ? "Not ready." : "غير جاهزة بعد.");
      return;
    }
    saveState(state);
    syncTopbar();
    // refresh view & list
    const v2 = getTasksView(state, isEn ? "en" : "ar");
    view.daily = v2.daily; view.weekly = v2.weekly; view.achievements = v2.achievements;
    dotDaily = hasClaimable(view.daily);
    dotWeekly = hasClaimable(view.weekly);
    dotAch = hasClaimable(view.achievements);
    // update tab dots immediately
    const d1 = document.getElementById("uiTasksDotDaily"); if(d1) d1.hidden = !dotDaily;
    const d2 = document.getElementById("uiTasksDotWeekly"); if(d2) d2.hidden = !dotWeekly;
    const d3 = document.getElementById("uiTasksDotAch"); if(d3) d3.hidden = !dotAch;
    renderList();
    toast(isEn ? `Claimed: ${t.rewardText}` : `تم الاستلام: ${t.rewardText}`);
  }
}


function renderMastery(){
  const isEn = getLang()==="en";
  const dir = (document.documentElement?.dir || "rtl").toLowerCase();
  const backIcon = (dir === "ltr") ? "fas fa-arrow-left" : "fas fa-arrow-right";
  // keep mastery state synced with day/week
  try{ ensureMasteryPath(state); }catch(_){}

  const iso = todayISO();
  const mp = state.masteryPath || {};
  const t = mp.today || {};

  // derived step: streak safe if attendance recorded today
  const streakSafe = !!(state.streak && state.streak.lastAttendanceIso === iso);

  // weights
  const steps = [
    {
      key:"dueOpened",
      weight:15,
      title: isEn ? "Review today's cards" : "مراجعة بطاقات اليوم",
      desc: isEn ? "Open the pack of today's play cards and take a quick look." : "افتح رزمة بطاقات اللعب اليوم وراجعها سريعًا.",
      done: !!t.dueOpened,
      actionLabel: isEn ? "Open pack" : "فتح الرزمة",
      action: ()=> nav("#/library"),
    },
    {
      key:"playDone",
      weight:40,
      title: isEn ? "Finish today's Play session" : "إنهاء جلسة اللعب",
      desc: isEn ? "Complete the 4 games for today." : "أكمل الألعاب الأربعة لليوم.",
      done: !!t.playDone,
      actionLabel: isEn ? "Play" : "لعب",
      action: ()=> nav("#/play"),
    },
    {
      key:"ratingDone",
      weight:15,
      title: isEn ? "Save card ratings" : "حفظ تقييم البطاقات",
      desc: isEn ? "Rate today's cards and save once at the end." : "قيّم بطاقات اليوم ثم احفظ مرة واحدة في النهاية.",
      done: !!t.ratingDone,
      actionLabel: isEn ? "Continue" : "متابعة",
      action: ()=> nav("#/play"),
    },
    {
      key:"trainingDone",
      weight:20,
      title: isEn ? "Smart training" : "تدريب ذكي",
      desc: isEn ? "Complete one training session (at least 4 cards)." : "أكمل جلسة تدريب واحدة (4 بطاقات على الأقل).",
      done: !!t.trainingDone,
      actionLabel: isEn ? "Train" : "تدريب",
      action: ()=> {
        // if training payload is missing, send user to packs first
        try{
          const p = getTrainingPayload?.();
          if(!p || !p.ids || p.ids.length < 4) nav("#/library");
          else nav("#/training");
        }catch(_){ nav("#/library"); }
      },
    },
    {
      key:"streakSafe",
      weight:10,
      title: isEn ? "Secure your streak" : "تأمين السلسلة",
      desc: isEn ? "Recording attendance today keeps your streak safe." : "تسجيل حضور اليوم يحافظ على السلسلة.",
      done: streakSafe,
      actionLabel: isEn ? "Streak" : "السلسلة",
      action: ()=> nav("#/streak"),
    },
  ];

  const progress = steps.reduce((sum,s)=> sum + (s.done ? s.weight : 0), 0);
  const isComplete = progress >= 100;

  // Header (reuse profile card style)
  const headerCardStack = h("div",{class:"avatar-stack", ariaHidden:"true"});
  try{
    const bg = getSelectedBg(state);
    const av = getSelectedAvatar(state);
    const frame = getSelectedFrame(state);
    const bgImg = new Image(); bgImg.src = bg?.src || ""; bgImg.className = "layer bg";
    const avImg = new Image(); avImg.src = av?.src || ""; avImg.className = "layer av";
    const frameImg = new Image(); frameImg.src = frame?.src || ""; frameImg.className = "layer frame";
    headerCardStack.append(frameImg, bgImg, avImg);
  }catch{}

  const title = isEn ? "Mastery Path" : "طريق الإتقان";
  const subtitle = isEn ? `Today's progress: ${progress}%` : `تقدّم اليوم: ${progress}%`;

  const header = h("header",{class:"page-header"},
    h("div",{class:"profile-card", "data-sfx":"naker", onClick: ()=> nav("#/profile")},
      headerCardStack,
      h("div",{class:"profile-text"},
        h("div",{class:"profile-name"}, title),
        h("div",{class:"user-meta"}, subtitle)
      )
    ),
    h("div",{class:"gold-tag", "aria-label": isEn?"Gold":"الذهب"},
      h("i",{class:"fas fa-coins", ariaHidden:"true"}),
      h("span",{class:"gold-val"}, String(Math.max(0, Math.floor(state.economy.gold||0))))
    )
  );

  const bar = h("div",{class:"mp-bar"},
    h("div",{class:"mp-bar-fill", style:`width:${Math.max(0,Math.min(100,progress))}%`})
  );

  const primaryBtn = h("button",{class:"btn primary", "data-sfx":"tap_primary", onClick: ()=> {
    // Suggest the most relevant next action
    const next = steps.find(s=> !s.done);
    if(next) next.action();
    else nav("#/home");
  }}, isEn ? (isComplete ? "Back to Home" : "Start") : (isComplete ? "عودة للرئيسية" : "ابدأ الآن"));

  const claimBtn = (!mp.claimed && isComplete) ? h("button",{class:"btn", "data-sfx":"tap_secondary", onClick: ()=> {
      const ok = masteryClaimDailyReward(state);
      if(ok){
        saveState(state);
        syncTopbar();
        toast(isEn ? "Reward claimed!" : "تم استلام المكافأة!");
        render(); // re-render current route
      }else{
        toast(isEn ? "Nothing to claim." : "لا توجد مكافأة.");
      }
    }}, isEn ? "Claim reward (+1 fuel)" : "استلام المكافأة (+1 وقود)") : null;

  const stepsList = h("div",{class:"mp-steps"});
  for(const s of steps){
    const statusIcon = s.done ? "fas fa-check-circle" : "far fa-circle";
    const row = h("div",{class:"mp-step glass gold"},
      h("div",{class:"mp-step-left"},
        h("div",{class:"mp-step-title"},
          h("i",{class: statusIcon, ariaHidden:"true"}),
          h("span",{}, s.title)
        ),
        h("div",{class:"subtle mp-step-desc"}, s.desc)
      ),
      h("div",{class:"mp-step-right"},
        h("div",{class:"mp-step-weight"}, `${s.weight}%`),
        h("button",{class:"btn btn-sm", disabled: s.done ? true : false, "data-sfx":"naker", onClick: ()=> s.action()}, s.done ? (isEn?"Done":"تم") : s.actionLabel)
      )
    );
    stepsList.appendChild(row);
  }

  // Weekly mini grid (Mon..Sun)
  // This file runs as an ES module, so we must not rely on non-imported globals.
  // Compute week start (Monday) locally.
  const weekStart = mondayISO_local(iso);
  const weekDays = [];
  for(let d=0; d<7; d++){
    weekDays.push(addDaysISO(weekStart, d));
  }
  const weekBox = h("div",{class:"glass gold panel"},
    h("div",{class:"h1"}, isEn ? "This week" : "هذا الأسبوع"),
    h("div",{class:"mp-week"},
      ...weekDays.map(dayIso=>{
        const done = !!(mp.week && mp.week.days && mp.week.days[dayIso]);
        const isToday = dayIso === iso;
        const midIcon = done ? "fas fa-check" : "fas fa-minus";
        return h("div",{class:"mp-day " + (done?"done ":"") + (isToday?"today":""), title: dayIso},
          h("div",{class:"mp-day-top"}, isEn ? shortDow(dayIso) : shortDowAr(dayIso)),
          h("div",{class:"mp-day-mid"}, h("i",{class: midIcon, ariaHidden:"true"}))
        );
      })
    ),
    h("div",{class:"subtle", style:"margin-top:10px;"}, isEn ? "Complete the daily path to fill a day." : "أكمل طريق اليوم ليُسجّل هذا اليوم.")
  );

  // Simple milestones
  const totalDone = Number(mp.stats?.totalDaysCompleted || 0) + (isComplete && !mp.week?.days?.[iso] ? 0 : 0);
  const cards = Object.values(state.cards.byId || {});
  const lvl3plus = cards.filter(c=> Number(c.level||0) >= 3).length;
  const mil = h("div",{class:"glass gold panel"},
    h("div",{class:"h1"}, isEn ? "Milestones" : "إنجازات"),
    h("div",{class:"row", style:"gap:10px;flex-wrap:wrap;"},
      h("div",{class:"mp-stat"},
        h("div",{class:"subtle"}, isEn ? "Days completed" : "أيام مكتملة"),
        h("div",{class:"mp-stat-num"}, String(totalDone))
      ),
      h("div",{class:"mp-stat"},
        h("div",{class:"subtle"}, isEn ? "Cards ≥ level 3" : "بطاقات مستوى ≥ 3"),
        h("div",{class:"mp-stat-num"}, String(lvl3plus))
      )
    )
  );

  const backBtn = h("button",{class:"fixed-back-btn btn-active", "data-sfx":"naker", onClick: ()=> navBack()},
    h("i",{class: backIcon, ariaHidden:"true"}),
    isEn?"Back":"رجوع"
  );

  const goalCard = h("div",{class:"glass gold panel"},
    h("div",{class:"row", style:"align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;"},
      h("div",{},
        h("div",{class:"h1", style:"margin:0;"}, isEn ? "Goal of the day" : "هدف اليوم"),
        h("div",{class:"subtle"}, isEn ? "Finish the daily steps for a steady mastery streak." : "أكمل خطوات اليوم لتقدّم ثابت في الإتقان.")
      ),
      h("div",{class:"row", style:"gap:10px;flex-wrap:wrap;justify-content:flex-end;"}, primaryBtn, claimBtn)
    ),
    h("div",{style:"margin-top:12px;"}, bar),
    h("div",{class:"hr"}),
    stepsList
  );

  const body = h("div",{class:"panel", style:"padding: 0;"},
    goalCard,
    weekBox,
    mil
  );

  const scroll = h("div",{class:"packs-scroll", style:"padding-bottom: 18px;"}, body);

  const wrap = h("div",{class:"packs-wrap"},
    header,
    scroll,
    backBtn
  );

  clear(app);
  app.appendChild(wrap);
  window.lucide?.createIcons?.();
}

// small helpers for Mastery UI
function mondayISO_local(iso){
  // Return ISO date (YYYY-MM-DD) for Monday of the week that contains `iso`.
  // Week starts on Monday.
  const d = new Date(String(iso) + "T00:00:00");
  const dow = d.getDay(); // 0=Sun..6=Sat
  const delta = (dow + 6) % 7; // Mon=0, Tue=1, ..., Sun=6
  d.setDate(d.getDate() - delta);
  const y = d.getFullYear();
  const m = String(d.getMonth()+1).padStart(2,"0");
  const day = String(d.getDate()).padStart(2,"0");
  return `${y}-${m}-${day}`;
}
function addDaysISO(iso, addDays){
  const d = new Date(iso + "T00:00:00");
  d.setDate(d.getDate() + Number(addDays||0));
  const y = d.getFullYear();
  const m = String(d.getMonth()+1).padStart(2,"0");
  const day = String(d.getDate()).padStart(2,"0");
  return `${y}-${m}-${day}`;
}
function shortDow(iso){
  const d = new Date(iso + "T00:00:00");
  return ["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][d.getDay()];
}
function shortDowAr(iso){
  const d = new Date(iso + "T00:00:00");
  return ["أحد","اثن","ثلا","أرب","خمي","جمع","سبت"][d.getDay()];
}


function renderLeaderboard(){
  const isEn = getLang()==="en";
  const dir = (document.documentElement?.dir || "rtl").toLowerCase();
  const backIcon = (dir === "ltr") ? "fas fa-arrow-left" : "fas fa-arrow-right";

  const rankName = (()=>{
    const r = RANKS.find(x=>x.key === state?.rating?.rankKey);
    return tr(r?.name || (rankLabel(state.rating.rankKey, state.rating.subLevel) || ""));
  })();
  const points = Math.max(0, Math.floor(state?.rating?.progress || 0));

  const header = h("header",{class:"page-header"},
    h("div",{class:"profile-card btn-active", "data-sfx":"naker", onClick: ()=> nav("#/home")},
      h("div",{class:"login-badge", ariaHidden:"true"}, h("i",{class:"fas fa-trophy", ariaHidden:"true"})),
      h("div",{class:"profile-text"},
        h("div",{class:"profile-name"}, isEn ? "Leaderboard" : "التصنيف"),
        h("div",{class:"user-meta"},
          h("span",{class:"meta-pill"}, (isEn ? "Top 100" : "أفضل 100")),
          h("span",{class:"meta-pill"}, (isEn ? `Season ${state.season?.season||1}` : `الموسم ${state.season?.season||1}`))
        )
      )
    ),
    h("div",{class:"gold-tag", "aria-label": isEn?"Gold":"الذهب"},
      h("i",{class:"fas fa-coins"}),
      h("span",{class:"gold-val"}, String(Math.max(0, Math.floor(state.economy.gold||0))))
    )
  );

  // --- season countdown (compact info row style) ---
  const countdownEl = h("b",{class:"lb-season-count", id:"lbCountdown"}, "-");
  const infoBtn = h("span",{class:"info-link lb-season-info btn-active", "data-sfx":"tap_secondary", onClick: ()=> openLeaderboardInfo()},
    h("span",{class:"lb-info-mark", ariaHidden:"true"}, "!")
  );
  const timerBar = h("div",{class:"card-info-row lb-seasonbar"},
    h("span",{class:"lb-season-left"},
      (isEn ? "Season ends in:" : "المتبقي لنهاية الموسم:"),
      " ",
      countdownEl
    ),
    infoBtn
  );

  const list = h("div",{class:"lb-list"});
  list.appendChild(buildLeaderboardRow({
    n: 1,
    username: state.profile?.username || (isEn?"User":"مستخدم"),
    rankName,
    points,
    interactive: true,
    sfx: "naker",
    onClick: ()=> openVisitorProfile()
  }));

  // placeholder note: only local user exists
  list.appendChild(h("div",{class:"subtle", style:"text-align:center; line-height:1.75; padding: 6px 4px 0;"},
    isEn
      ? "This leaderboard is local (on this device). Only your account appears here."
      : "هذا التصنيف محلي على هذا الجهاز. لذلك سيظهر حسابك فقط هنا."
  ));

  const scroll = h("div",{class:"packs-scroll", style:"padding-bottom: 24px;"}, list);

  // bottom "you" card (visual + sound only)
  const footer = h("div",{class:"lb-footer"},
    h("div",{class:"lb-footer-title subtle"}, isEn ? "You" : "أنت"),
    buildLeaderboardRow({
      n: 1,
      username: state.profile?.username || (isEn?"User":"مستخدم"),
      rankName,
      points,
      interactive: false,
      compact: true,
      sfx: "tap_secondary",
      onClick: ()=>{}
    })
  );

  const backBtn = h("button",{class:"fixed-back-btn btn-active", "data-sfx":"naker", onClick: ()=> navBack()},
    h("i",{class: backIcon, ariaHidden:"true"}),
    isEn ? "Back" : "رجوع"
  );

  const wrap = h("div",{class:"leaderboard-wrap"}, header, timerBar, scroll, footer, backBtn);
  app.appendChild(wrap);

  // start live countdown
  updateCountdown();
  _leaderboardTick = setInterval(updateCountdown, 1000);

  function addDaysISO(iso, days){
    const d = new Date(String(iso)+"T00:00:00");
    d.setDate(d.getDate() + Number(days||0));
    const y = d.getFullYear();
    const m = String(d.getMonth()+1).padStart(2,"0");
    const dd = String(d.getDate()).padStart(2,"0");
    return `${y}-${m}-${dd}`;
  }

  function updateCountdown(){
    try{
      const startIso = state?.season?.startedAt || todayISO();
      const len = Number(state?.season?.lengthDays || 100);
      const endIso = addDaysISO(startIso, len);
      const end = new Date(endIso + "T00:00:00");
      const now = new Date();
      let ms = end - now;
      if(!Number.isFinite(ms) || ms < 0) ms = 0;
      const dayMs = 1000*60*60*24;
      const hrMs = 1000*60*60;
      const days = Math.floor(ms / dayMs);
      const hours = Math.floor((ms % dayMs) / hrMs);
      countdownEl.textContent = isEn ? `${days}d ${hours}h` : `${days} يوم ${hours} ساعة`;
    }catch(_){
      countdownEl.textContent = "-";
    }
  }

  function openLeaderboardInfo(){
    const body = h("div",{},
      h("div",{class:"subtle", style:"white-space:pre-wrap; line-height:1.9;"},
        isEn
          ? "• This leaderboard is local (on this device).\n• The season resets every 100 days.\n• When the season resets, your rank progress is reset and your tier may drop depending on your current tier.\n• Only real accounts are shown, so currently you will see only yourself."
          : "• هذا التصنيف محلي على هذا الجهاز.\n• الموسم يتصفّر كل 100 يوم.\n• عند التصفير يتم تصفير التقدم وقد تنخفض رتبتك حسب رتبتك الحالية.\n• لا يتم عرض أسماء وهمية، لذلك حاليًا سيظهر حسابك فقط."
      )
    );
    modal(isEn ? "Info" : "معلومات", body, [
      { label: isEn?"Close":"إغلاق", kind:"primary", sfx:"tap_secondary", onClick:(close)=> close() }
    ]);
  }

  function buildLeaderboardRow({ n, username, rankName, points, interactive, compact, sfx, onClick }){
    const stack = h("div",{class:"avatar-stack", ariaHidden:"true"});
    try{
      const {frame, bg, av} = computeAvatarLayers(state);
      const bgImg = new Image(); bgImg.src = bg?.src || ""; bgImg.className = "layer bg";
      const avImg = new Image(); avImg.src = av?.src || ""; avImg.className = "layer av";
      const frameImg = new Image(); frameImg.src = frame?.src || ""; frameImg.className = "layer frame";
      stack.append(bgImg, avImg, frameImg);
    }catch(_){ }

    const rankBadge = h("div",{class:"lb-rank", style:"color: var(--gold-color);"}, `#${n}`);

    const row = h("div",{
      class: `lb-entry ${compact ? "lb-entry-compact" : ""} btn-active`,
      "data-sfx": sfx || "tap_secondary",
      onClick: onClick || (()=>{})
    },
      stack,
      h("div",{class:"lb-main"},
        h("div",{class:"lb-name"}, username),
        h("div",{class:"lb-sub"}, `${rankName} (${points})`)
      ),
      rankBadge
    );
    // disable pointer cursor when non-interactive
    if(!interactive) row.style.cursor = "default";
    return row;
  }

  function openVisitorProfile(){
    const isEn = getLang()==="en";
    const heroStack = h("div",{class:"avatar-stack big preview", ariaHidden:"true"});
    try{
      const {frame, bg, av} = computeAvatarLayers(state);
      const frameImg = new Image(); frameImg.src = frame?.src || ""; frameImg.className = "layer frame";
      const bgImg = new Image(); bgImg.src = bg?.src || ""; bgImg.className = "layer bg";
      const avImg = new Image(); avImg.src = av?.src || ""; avImg.className = "layer av";
      heroStack.append(bgImg, avImg, frameImg);
    }catch(_){ }

    const lvl = computeLevelFromXp(state?.economy?.xp || 0);
    const acc = state?.rating?.accuracySeason;
    const accPct = (!acc || acc.total<=0) ? 0 : Math.max(0, Math.min(100, Math.round((acc.correct/acc.total)*100)));

    const body = h("div",{},
      h("div",{style:"display:flex; flex-direction:column; align-items:center; gap:10px;"},
        heroStack,
        h("div",{class:"profile-big-name", style:"font-size:1.25rem;"}, state.profile?.username || (isEn?"User":"مستخدم")),
        h("div",{class:"profile-level-pill"}, isEn ? `Level ${lvl.level}` : `مستوى ${lvl.level}`)
      ),
      h("div",{class:"hr"}),
      h("div",{class:"stats-container"},
        h("div",{class:"stat-row"},
          h("span",{}, isEn?"Rank:":"التصنيف:"),
          h("span",{class:"rank-chip"}, rankName)
        ),
        h("div",{class:"stat-row"},
          h("span",{}, isEn?"Rating points:":"نقاط التصنيف:"),
          h("span",{style:"font-weight:900;"}, String(points))
        ),
        h("div",{class:"stat-row"},
          h("span",{}, isEn?"Accuracy:":"الدقة:"),
          h("span",{style:"font-weight:900;"}, `${accPct}%`)
        ),
        h("div",{class:"accuracy-bar-bg", ariaHidden:"true"},
          h("div",{class:"accuracy-fill", style:`width:${accPct}%`})
        )
      )
    );

    modal(isEn ? "Profile" : "الملف الشخصي", body, [
      { label: isEn?"Close":"إغلاق", kind:"primary", sfx:"tap_secondary", onClick:(close)=> close() }
    ]);
  }
}

function average(arr){
  if(!arr || arr.length===0) return 10;
  return arr.reduce((s,x)=>s+x,0)/arr.length;
}
function shuffle(a){
  for(let i=a.length-1;i>0;i--){
    const j = Math.floor(Math.random()*(i+1));
    [a[i],a[j]] = [a[j],a[i]];
  }
  return a;
}
function countUp(el, target){
  if(!el) return;
  const t = Number(target) || 0;

  // Prefer GSAP when available, but provide a fallback so the UI never stays at 0.
  try{
    if(window.gsap && typeof gsap.to === "function"){
      const d = {v:0};
      gsap.to(d, { v: t, duration: 0.9, ease: "power2.out", onUpdate: ()=> {
        el.textContent = String(Math.floor(d.v));
        if(Math.random() < 0.12) playSfx("count");
      }});
      return;
    }
  }catch{}

  const start = 0;
  const duration = 900;
  const t0 = performance.now();
  function easeOutQuad(p){ return 1 - (1-p)*(1-p); }
  function step(now){
    const p = Math.min(1, (now - t0) / duration);
    const v = Math.floor(start + (t - start) * easeOutQuad(p));
    el.textContent = String(v);
    if(Math.random() < 0.08) playSfx("count");
    if(p < 1) requestAnimationFrame(step);
  }
  requestAnimationFrame(step);
}

// ================================
// مشاركة/استيراد (ملف JSON بسيط)
// ================================
function sanitizeFileName(name){
  return String(name||"مشاركة")
    .replace(/[\\/:*?\"<>|]+/g, "_")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 48) || "مشاركة";
}

function fingerprintCard(a,b,hint){
  return [a,b,hint].map(x=>String(x||"").trim().toLowerCase()).join("|");
}






function uniquePackName(base){
  let name = base;
  let i = 2;
  const exists = (n)=> state.library.customPacks.some(p=>p.name===n);
  while(exists(name)){
    name = `${base} (${i})`;
    i++;
  }
  return name;
}

function ensureCustomPackByName(name){
  let p = state.library.customPacks.find(x=>x.name===name);
  if(!p){
    p = { id: uuid(), name, cardIds: [] };
    state.library.customPacks.unshift(p);
  }
  return p;
}

function importSharedDeckFile(){
  const isEn = getLang()==="en";
  const isCordova = !!window.cordova;
  const hasFilePlugin = isCordova && (typeof window.resolveLocalFileSystemURL === "function");

  const fileChooserObj = (window.fileChooser)
    || (window.cordova && window.cordova.plugins && (window.cordova.plugins.fileChooser || window.cordova.plugins.filechooser))
    || null;
  const hasFileChooser = !!fileChooserObj && (typeof fileChooserObj.open === "function");

  const chooserObj = (window.chooser)
    || (window.cordova && window.cordova.plugins && window.cordova.plugins.chooser)
    || null;
  const hasChooser = !!chooserObj && (typeof chooserObj.getFile === "function");

  const filePathObj = (window.FilePath)
    || (window.cordova && window.cordova.plugins && (window.cordova.plugins.filePath || window.cordova.plugins.filepath || window.cordova.plugins.FilePath))
    || null;
  const hasFilePath = !!filePathObj && (typeof filePathObj.resolveNativePath === "function");

  async function applyImportedJson(txt){
    const data = JSON.parse(txt);
    const res = importSharedDeckPayload(data);
    if(!res.ok){ toast(res.msg || (isEn?"Invalid file.":"ملف غير صالح.")); return; }
    saveState(state);
    toast(isEn ? `Imported ${res.added} cards.` : `تم استيراد ${res.added} بطاقة.`);
    nav("#/library");
  }

  // fallback: <input type=file> حتى داخل Cordova (يصلح عند تعذّر قراءة content://)
  function openWebFileInput(){
    const inp = document.createElement("input");
    inp.type = "file";
    // نقبل الامتدادين: json (قديم) و mastery (جديد)
    inp.accept = ".mastery,application/octet-stream,application/json,.json";
    inp.onchange = async ()=> {
      const file = inp.files?.[0];
      if(!file) return;
      try{ await applyImportedJson(await file.text()); }
      catch(_){ toast(isEn ? "Invalid file." : "ملف غير صالح"); }
    };
    inp.click();
  }

  function normalizeToFileUrl(pathOrUrl){
    const u = String(pathOrUrl||"");
    if(u.startsWith("file://")) return u;
    if(u.startsWith("/")) return "file://" + u;
    return u;
  }

  function readTextFromUrl(url, onOk, onErr){
    try{
      window.resolveLocalFileSystemURL(url, (entry)=>{
        entry.file((file)=>{
          const reader = new FileReader();
          reader.onloadend = ()=> onOk(String(reader.result||""));
          reader.onerror = ()=> onErr(reader.error || new Error("read_failed"));
          reader.readAsText(file);
        }, onErr);
      }, onErr);
    }catch(e){
      onErr(e);
    }
  }

  function cordovaPickAndRead(){
    if(!hasFilePlugin){
      toast(isEn ? "To import in APK, add cordova-plugin-file." : "للاستيراد داخل APK أضف cordova-plugin-file.");
      return;
    }
    if(!hasFileChooser && !hasChooser){
      toast(isEn ? "To import in APK, add a file chooser plugin." : "للاستيراد داخل APK أضف إضافة اختيار الملفات (File Chooser).\n\nمقترح: cordova-plugin-filechooser + cordova-plugin-filepath");
      return;
    }

    const onPicked = (uri)=>{
      if(!uri){ toast(isEn ? "No file selected." : "لم يتم اختيار ملف."); return; }

	      const attemptRead = (u)=>{
        const url = normalizeToFileUrl(u);
        readTextFromUrl(url, async (txt)=>{
          try{ await applyImportedJson(txt); }
          catch(e){ toast(isEn ? "Invalid file." : "ملف غير صالح"); }
	        }, ()=>{ 
	          toast(isEn ? "Could not read file." : "تعذّر قراءة الملف.");
	          try{ openWebFileInput(); }catch(_){ }
	        });
      };

	      const s = String(uri);
	      if(s.startsWith("content:") && hasFilePath){
	        // حاول قراءة content:// مباشرة أولاً، ثم استخدم FilePath كـ fallback
	        let triedFallback = false;
	        const directUrl = normalizeToFileUrl(uri);
	        readTextFromUrl(directUrl, async (txt)=>{
	          try{ await applyImportedJson(txt); }
	          catch(e){ toast(isEn ? "Invalid file." : "ملف غير صالح"); }
	        }, ()=>{
	          if(triedFallback) return;
	          triedFallback = true;
	          try{
	            filePathObj.resolveNativePath(uri, (p)=> attemptRead(p), ()=> attemptRead(uri));
	          }catch(_){
	            attemptRead(uri);
	          }
	        });
	      } else {
	        attemptRead(uri);
	      }
    };

    const onFail = ()=> toast(isEn ? "No file selected." : "لم يتم اختيار ملف.");

    try{
	      if(hasFileChooser){
	        fileChooserObj.open(onPicked, onFail);
	      } else if(hasChooser){
	        const fn = chooserObj.getFile;
        if(fn.length >= 2){
	          fn.call(chooserObj, (file)=> onPicked(file && (file.uri || file.nativeURL || file.url || file.file || file.path)), onFail);
        } else {
	          fn.call(chooserObj, (file)=> onPicked(file && (file.uri || file.nativeURL || file.url || file.file || file.path)));
        }
      }
    }catch(e){
      toast(isEn ? "Could not open picker." : "تعذّر فتح اختيار الملفات.");
    }
  }

  // Cordova path (APK)
  if(isCordova){
    if(hasFileChooser || hasChooser){
      cordovaPickAndRead();
      return;
    }
  }

  // Web fallback
  openWebFileInput();
}


// ===== مشاركة رزم/بطاقات (تنزيل ملف) =====
function makeSharePayload({ kind="selection", title="DeckMastery", cardIds=[] }){
  const cards = [];
  for(const id of cardIds){
    const c = state.cards.byId?.[id];
    if(!c) continue;
    cards.push({ a: c.a ?? "", b: c.b ?? "", hint: c.hint ?? "" });
  }
  return {
    _app: "DeckMastery",
    _type: "share",
    _v: 1,
    kind,
    title,
    exportedAt: new Date().toISOString(),
    cards
  };
}

function downloadSharePayload(payload){
  // Web path only: تنزيل ملف JSON عبر Blob
  const nameBase = sanitizeFileName(payload.title || "DeckMastery");
  // نستخدم امتداد خاص للمشاركة (DeckMastery Share File)
  const fname = `${nameBase}_${backupDateKey()}_Share.mastery`;
  // WhatsApp يتعامل أحياناً مع application/json كنص، لذلك نستخدم octet-stream
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/octet-stream" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = fname;
  document.body.appendChild(a);
  a.click();
  a.remove();
  setTimeout(()=> URL.revokeObjectURL(url), 500);
}


async function exportShareDeck({ kind="selection", title="DeckMastery", cardIds=[] }={}){
  const isEn = getLang()==="en";
  const payload = makeSharePayload({ kind, title, cardIds });
  const isCordova = (typeof window !== "undefined") && !!window.cordova;
  const hasFilePlugin = isCordova && !!(window.cordova.file) && (typeof window.resolveLocalFileSystemURL === "function");
  const social = (window.plugins && window.plugins.socialsharing) || null;
  const hasSocialSharing = !!social && (typeof social.shareWithOptions === "function" || typeof social.share === "function");

  // تأمين مشاركة الملفات على Android/Monaca:
  // بعض البيئات تعطي cdvfile:// فتتجاهله إضافة المشاركة وتتحول المشاركة إلى "نص فقط".
  const resolveShareableUrl = (fileEntry)=> new Promise((resolve)=>{
    try{
      if(!fileEntry) return resolve("");
      const u0 = (fileEntry.nativeURL)
        || (typeof fileEntry.toNativeURL === "function" ? fileEntry.toNativeURL() : "")
        || (typeof fileEntry.toURL === "function" ? fileEntry.toURL() : "")
        || "";
      const u = String(u0||"");
      if(!u) return resolve("");
      if(u.startsWith("cdvfile://") && typeof window.resolveLocalFileSystemURL === "function"){
        window.resolveLocalFileSystemURL(u, (e)=>{
          const nu = String((e && (e.nativeURL || (typeof e.toNativeURL === "function" && e.toNativeURL()) || (typeof e.toURL === "function" && e.toURL()))) || u);
          resolve(nu);
        }, ()=> resolve(u));
        return;
      }
      resolve(u);
    }catch(_){ resolve(""); }
  });

  const toBase64Utf8 = (str)=>{
    try{ return btoa(unescape(encodeURIComponent(String(str||"")))); }
    catch(_){ return ""; }
  };

  const shareFileOrBase64 = async ({fileEntry, fileName, jsonText, message})=>{
    if(!hasSocialSharing) return false;
    const msg = String(message||"");
    const subj = String(fileName||"");
    const fileUrl = await resolveShareableUrl(fileEntry);

    const doShare = (files)=> new Promise((resolve)=>{
      try{
        if(typeof social.shareWithOptions === "function"){
          social.shareWithOptions({ message: msg, subject: subj, files }, ()=> resolve(true), ()=> resolve(false));
          return;
        }
        if(typeof social.share === "function"){
          social.share(msg, subj, (files && files[0]) || null, null, ()=> resolve(true), ()=> resolve(false));
          return;
        }
      }catch(_){ /* ignore */ }
      resolve(false);
    });

    // 1) حاول مشاركة ملف حقيقي
    if(fileUrl){
      try{
        const ok = await doShare([fileUrl]);
        if(ok) return true;
      }catch(_){ /* ignore */ }
    }

    // 2) fallback: ملف base64 (كمرفق)
    const b64 = toBase64Utf8(jsonText);
    if(!b64) return false;
    try{ return await doShare([`base64:${subj}//${b64}`]); }catch(_){ return false; }
  };

  // Web: تنزيل ملف JSON عبر Blob
  if(!isCordova){
    downloadSharePayload(payload);
    modal(isEn ? "Share file downloaded" : "تم تنزيل ملف المشاركة", h("div",{class:"modal-body"},
      h("div",{style:"line-height:1.75"},
        h("div",{}, isEn ? "The share file was downloaded." : "تم تنزيل ملف المشاركة على جهازك."),
        h("div",{style:"margin-top:8px; opacity:.9"}, isEn ? "Send this file to the other person (WhatsApp, Telegram, Email...)." : "شارك هذا الملف مع الشخص الآخر (عبر واتساب أو تلغرام أو البريد الإلكتروني)."),
        h("div",{style:"margin-top:8px; opacity:.9"}, isEn ? "On the other device: Settings → Import share file." : "وعلى الطرف الآخر اختيار «استيراد ملف مشاركة» من الإعدادات لاستقبال البطاقات.")
      )
    ), [
      { label: isEn ? "OK" : "حسنًا", kind:"primary", sfx:"tap_primary", onClick:(close)=> close() }
    ]);
    return;
  }

  // Cordova/Monaca APK
  if(!hasFilePlugin){
    toast(isEn ? "To share in APK, add cordova-plugin-file." : "للمشاركة داخل APK أضف cordova-plugin-file.");
    return;
  }

  const nameBase = sanitizeFileName(payload.title || "DeckMastery");
  const fname = `${nameBase}_${backupDateKey()}_Share.mastery`;
  const jsonText = JSON.stringify(payload, null, 2);

  // مبدأ عام: لا نعتمد على Android/data لأن المستخدم لا يمكنه الوصول له.
  // لذلك ننشئ الملف في cache أولًا ثم نفتح نافذة المشاركة مباشرة (إن وُجدت).
  const candidates = [];
  const addDownloads = ()=>{
    try{
      if(window.cordova.file.externalRootDirectory) candidates.push(window.cordova.file.externalRootDirectory + "Download/");
      if(window.cordova.file.externalStorageDirectory) candidates.push(window.cordova.file.externalStorageDirectory + "Download/");
    }catch(_){ /* ignore */ }
  };

  // إن لم تكن إضافة المشاركة متوفرة، نحاول Downloads أولًا كي يراه المستخدم مباشرة.
  if(!hasSocialSharing) addDownloads();

  // الأفضل للمشاركة على Android (خصوصاً WhatsApp): external cache أولاً ثم cache الداخلي
  candidates.push(
    window.cordova.file.externalCacheDirectory,
    window.cordova.file.cacheDirectory
  );
// كخيار أخير جدًا (قد يكون Android/data) – لكن حتى هنا سنحاول فتح المشاركة لكي لا يحتاج المستخدم للوصول للمسار.
  candidates.push(window.cordova.file.dataDirectory);

  // وإن كانت إضافة المشاركة موجودة، يمكن أيضًا حفظ نسخة داخل Downloads (اختياري) لكن بعد cache.
  if(hasSocialSharing) addDownloads();

  const writeFile = (dir, name, content)=> new Promise((resolve, reject)=>{
    if(!dir) return reject(new Error("no dir"));
    window.resolveLocalFileSystemURL(dir, (dirEntry)=>{
      dirEntry.getFile(name, {create:true, exclusive:false}, (fileEntry)=>{
        fileEntry.createWriter((writer)=>{
          writer.onwriteend = ()=> resolve(fileEntry);
          writer.onerror = (e)=> reject(e);
          // WhatsApp يتعامل أحياناً مع application/json كنص، لذلك نستخدم octet-stream
          const blob = new Blob([content], {type:"application/octet-stream"});
          writer.write(blob);
        }, reject);
      }, reject);
    }, reject);
  });

  try{
    let lastErr = null;
    let fileEntry = null;
    let usedDir = "";
    for(const dir of candidates){
      if(!dir) continue;
      try{ fileEntry = await writeFile(dir, fname, jsonText); usedDir = dir; break; }
      catch(e){ lastErr = e; }
    }
    if(!fileEntry) throw (lastErr || new Error("write failed"));

    const fileUrl = await resolveShareableUrl(fileEntry);
    if(fileUrl) localStorage.setItem("dm_last_share_path", fileUrl);
    localStorage.setItem("dm_last_share_filename", fname);

    const looksLikeDownloads = (()=>{
      const d = String(usedDir||"");
      const u = String(fileUrl||"");
      return /Download\/?$/i.test(d) || /\/Download\//i.test(u) || /\/Downloads\//i.test(u);
    })();

    // المطلوب: المشاركة مباشرة من داخل التطبيق (مثل أي ملف عادي)
    if(hasSocialSharing){
      const ok = await shareFileOrBase64({ fileEntry, fileName: fname, jsonText, message: isEn ? "DeckMastery share file" : "ملف مشاركة DeckMastery" });
      if(ok){
        toast(isEn ? "Share sheet opened." : "تم فتح المشاركة.");
        return;
      }
    }

    // بدون SocialSharing: إذا نجح Downloads نخبر المستخدم مباشرة، وإلا نعرض مسار الملف.
    if(!hasSocialSharing && looksLikeDownloads){
      toast(isEn ? "Share file saved to Downloads." : "تم حفظ ملف المشاركة داخل Downloads.");
      modal(isEn ? "Share file created" : "تم إنشاء ملف المشاركة", h("div",{class:"modal-body"},
        h("div",{style:"line-height:1.75"},
          h("div",{}, isEn ? "The share file was saved to Downloads." : "تم حفظ ملف المشاركة داخل Downloads."),
          (fileUrl ? h("div",{style:"margin-top:8px; opacity:.9; word-break:break-all"}, fileUrl) : h("div",{})),
          h("div",{style:"margin-top:8px; opacity:.9"}, isEn ? "Send this file to the other person." : "شارك هذا الملف مع الشخص الآخر."),
          h("div",{style:"margin-top:8px; opacity:.9"}, isEn ? "On the other device: Settings → Import share file." : "وعلى الطرف الآخر اختيار «استيراد ملف مشاركة» من الإعدادات لاستقبال البطاقات.")
        )
      ), [
        { label: isEn ? "OK" : "حسنًا", kind:"primary", sfx:"tap_primary", onClick:(close)=> close() }
      ]);
      return;
    }

    // احتياطي عام
    modal(isEn ? "Share file created" : "تم إنشاء ملف المشاركة", h("div",{class:"modal-body"},
      h("div",{style:"line-height:1.75"},
        h("div",{}, isEn ? "The share file was created." : "تم إنشاء ملف المشاركة."),
        (fileUrl ? h("div",{style:"margin-top:8px; opacity:.9; word-break:break-all"}, fileUrl) : h("div",{})),
        h("div",{style:"margin-top:8px; opacity:.9"}, isEn
          ? "To share directly in APK, add cordova-plugin-x-socialsharing."
          : "للمشاركة المباشرة داخل APK أضف cordova-plugin-x-socialsharing."),
        h("div",{style:"margin-top:8px; opacity:.9"}, isEn ? "On the other device: Settings → Import share file." : "وعلى الطرف الآخر اختيار «استيراد ملف مشاركة» من الإعدادات لاستقبال البطاقات.")
      )
    ), [
      (hasSocialSharing
        ? { label: isEn ? "Share" : "مشاركة", kind:"primary", sfx:"tap_primary", onClick: async (close)=>{
              close();
              try{ await shareFileOrBase64({ fileEntry, fileName: fname, jsonText, message: isEn ? "DeckMastery share file" : "ملف مشاركة DeckMastery" }); }catch(_){ }
            } }
        : null),
      { label: isEn ? "OK" : "حسنًا", kind: (hasSocialSharing) ? "ghost" : "primary", sfx:"tap_secondary", onClick:(close)=> close() }
    ].filter(Boolean));
  }catch(e){
    toast(isEn ? "Share export failed." : "فشل إنشاء ملف المشاركة.");
  }
}

// دعم للتوافق مع نسخ أقدم كانت تستخدم importShareDeck
function importShareDeck(payload){
  return importSharedDeckPayload(payload);
}

function importSharedDeckPayload(data){
  if(!data || data._app !== "DeckMastery" || data._type !== "share" || !Array.isArray(data.cards)){
    return { ok:false, msg:"هذا ليس ملف مشاركة DeckMastery." };
  }

  const today = todayISO();
  const baseName = (data.kind === "pack")
    ? String(data.title || "مشاركة")
    : (data.kind === "dailyGroup")
      ? `مجموعة مشاركة: ${String(data.title || "")}`.trim()
      : `مشاركة: ${formatDateDMY(today)}`;

  const packName = uniquePackName(baseName || `مشاركة: ${formatDateDMY(today)}`);
  const pack = ensureCustomPackByName(packName);

  // fingerprints
  const fp = new Set();
  for(const id of state.cards.order){
    const c = state.cards.byId[id];
    if(!c) continue;
    fp.add(fingerprintCard(c.a,c.b,c.hint));
  }

  let added = 0;
  for(const x of data.cards){
    const a = String(x?.a ?? "").trim();
    const b = String(x?.b ?? "").trim();
    const hint = String(x?.hint ?? "").trim();
    if(!a || !b || !hint) continue;
    const f = fingerprintCard(a,b,hint);
    if(fp.has(f)) continue;
    const card = addCard(state, {a,b,hint});
    fp.add(f);
    pack.cardIds.push(card.id);
    added++;
  }

  if(added === 0){
    return { ok:true, added:0 };
  }
  return { ok:true, added };
}


// ===============================
// Android: open .mastery files inside the app (Intent VIEW)
// Requires: cordova-plugin-intent-shim, cordova-plugin-file, cordova-plugin-filepath
// ===============================

function dm_extractUriFromIntent(intent){
  if(!intent) return null;
  if(intent.data) return intent.data;
  const ex = intent.extras || {};
  if(ex["android.intent.extra.STREAM"]) return ex["android.intent.extra.STREAM"];
  const clip = intent.clipItems;
  if(Array.isArray(clip) && clip[0]){
    return clip[0].uri || clip[0].data || null;
  }
  return null;
}

function dm_readTextFromAnyUri(uri, onOk, onErr){
  try{
    const u = String(uri || "");
    const isContent = u.startsWith("content://");

    const filePathObj =
      (window.FilePath)
      || (window.cordova && window.cordova.plugins && (window.cordova.plugins.filePath || window.cordova.plugins.filepath || window.cordova.plugins.FilePath))
      || null;

    const readViaResolve = (fileUrl)=>{
      window.resolveLocalFileSystemURL(fileUrl, (entry)=>{
        entry.file((file)=>{
          const reader = new FileReader();
          reader.onloadend = ()=> onOk(String(reader.result || ""));
          reader.onerror = ()=> onErr(reader.error || new Error("read_failed"));
          reader.readAsText(file);
        }, onErr);
      }, onErr);
    };

    if(isContent && filePathObj && typeof filePathObj.resolveNativePath === "function"){
      filePathObj.resolveNativePath(u, (nativePath)=>{
        const fileUrl = nativePath.startsWith("file://") ? nativePath : ("file://" + nativePath);
        readViaResolve(fileUrl);
      }, (e)=> onErr(e || new Error("resolveNativePath_failed")));
      return;
    }

    const fileUrl =
      u.startsWith("file://") ? u :
      (u.startsWith("/") ? ("file://" + u) : u);

    readViaResolve(fileUrl);
  }catch(e){
    onErr(e);
  }
}

function dm_handleOpenMasteryIntent(intent){
  const uri = dm_extractUriFromIntent(intent);
  if(!uri) return;

  // Only handle .mastery files
  if(!/\.mastery(\?|$)/i.test(String(uri))) return;

  dm_readTextFromAnyUri(uri, (txt)=>{
    try{
      const data = JSON.parse(txt);
      const res = importSharedDeckPayload(data);
      if(!res || !res.ok){
        toast(res?.msg || "ملف غير صالح.");
        return;
      }
      saveState(state);
      toast(`تم استيراد ${res.added} بطاقة.`);
      nav("#/library");
    }catch(_){
      toast("تعذّر قراءة الملف.");
    }
  }, ()=>{
    toast("تعذّر فتح الملف.");
  });
}

function dm_setupOpenWith(){
  try{
    if(!window.cordova) return;
    const shim = window.plugins && window.plugins.intentShim;
    if(!shim) return;

    // On app launch
    if(typeof shim.getIntent === "function"){
      shim.getIntent(dm_handleOpenMasteryIntent, function(){});
    }

    // When app already running and receives a new intent
    if(typeof shim.onIntent === "function"){
      shim.onIntent(dm_handleOpenMasteryIntent);
    }

    // Some launchers deliver intent on resume
    document.addEventListener("resume", function(){
      if(typeof shim.getIntent === "function"){
        shim.getIntent(dm_handleOpenMasteryIntent, function(){});
      }
    }, false);
  }catch(_){}
}

// deviceready already fired in Monaca bootstrap.js, so call immediately when possible.
try{
  if(window.cordova) dm_setupOpenWith();
  else document.addEventListener("deviceready", dm_setupOpenWith, false);
}catch(_){}


// Initialize Rewarded Ads once (Cordova only)
try{
  if(window.cordova) dmAds.start();
  else document.addEventListener("deviceready", ()=> dmAds.start(), false);
}catch(_){}
