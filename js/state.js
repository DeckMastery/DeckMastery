import { persistKey, removeKey } from "./native_store.js";
...
// بطاقة: مستوى 0..10 (بعد تحديث جدول الإتقان)
// ملاحظة: المستويات 7 و 9 هي مستويات "انتظار" لا تظهر فيها البطاقة عادةً.
// نُبقي مضاعفات المكافآت متوافقة قدر الإمكان مع المنحنى القديم:
// 8 ≈ (7 القديم) و 10 ≈ (8 القديم)
...
// تظهر البطاقة فقط في هذه المستويات (حسب جدول الإتقان الجديد)
export const SHOW_LEVELS = new Set([1,3,6,8,10]);
...
export function defaultState(){
  ...
  return {
    ...
    economy: {
      gold: 0,
      // Lifetime earned gold (used for achievements). This is NOT the current balance.
      totalGoldEarned: 0,
      xp: 0,
      helps: 0,
      skips: 0,
    ...
    // Lightweight lifetime + per-day counters (for achievements/tasks)
    stats: {
      totalCardsPlayed: 0,        // lifetime cards played in Play (counted once per day)
      cardsPlayedByDay: {},       // iso -> count (to prevent double counting)
    },
    // Tasks (daily/weekly) + achievements (lifetime)
    tasks: {
      daily: { iso, progress:{}, claimed:{}, flags:{}, logged:{} },
      weekly: { weekStartIso: iso, progress:{}, claimed:{}, logged:{} },
      achievements: { claimed:{} },
    },
    masteryPath: {
      dayIso: iso,
      today: { dueOpened:false, playDone:false, ratingDone:false, trainingDone:false },
      claimed: false,
      week: { weekStartIso: iso, days:{}, claimed:false },
      stats: { totalDaysCompleted:0, doneDays:{} },
    },
    notifications: {
...
export function saveState(state){
  // نكتب فوراً إلى localStorage (متزامن) + SQLite (إن وُجد) بالخفاء
  persistKey(LS_KEY, JSON.stringify(state));
}

// حذف بيانات الحساب (متوافق مع Android/Cordova)
export function deleteStateStorage(){
  removeKey(LS_KEY);
}
...
export function migrate(state){
  ...
  merged.stats = { ...d.stats, ...(state.stats||{}) };
  merged.tasks = { ...d.tasks, ...(state.tasks||{}) };
  ...
  if(typeof merged.economy.totalGoldEarned !== "number"){
    // Best-effort backfill: treat current balance as earned baseline.
    merged.economy.totalGoldEarned = Math.max(0, Number(merged.economy.gold||0));
  }

  // stats/tasks fields
  merged.stats = merged.stats || d.stats;
  if(typeof merged.stats.totalCardsPlayed !== "number") merged.stats.totalCardsPlayed = 0;
  merged.stats.cardsPlayedByDay = merged.stats.cardsPlayedByDay || {};

  merged.tasks = merged.tasks || d.tasks;
  merged.tasks.daily = merged.tasks.daily || d.tasks.daily;
  merged.tasks.weekly = merged.tasks.weekly || d.tasks.weekly;
  merged.tasks.achievements = merged.tasks.achievements || d.tasks.achievements;
  merged.tasks.daily.progress = merged.tasks.daily.progress || {};
  merged.tasks.daily.claimed = merged.tasks.daily.claimed || {};
  merged.tasks.daily.flags = merged.tasks.daily.flags || {};
  merged.tasks.daily.logged = merged.tasks.daily.logged || {};
  merged.tasks.weekly.progress = merged.tasks.weekly.progress || {};
  merged.tasks.weekly.claimed = merged.tasks.weekly.claimed || {};
  merged.tasks.weekly.logged = merged.tasks.weekly.logged || {};
  merged.tasks.achievements.claimed = merged.tasks.achievements.claimed || {};
  ...
  merged.attendance.missedPending = (merged.attendance.missedPending || [])
    .map(x=>{
      if(!x) return null;
      if(typeof x === "string") return { iso: x, cardIds: [] };
      const iso = x.iso || x.day || x.date;
      if(!iso) return null;
      const cardIds = Array.isArray(x.cardIds) ? x.cardIds.filter(Boolean) : [];
      return { iso, cardIds };
    })
    .filter(Boolean);
...
export function ensureProfile(state, {username, gender}){
  ...
  state.economy.totalGoldEarned = 900;
  state.economy.xp = 0;
  ...
  // stats/tasks
  state.stats = { totalCardsPlayed: 0, cardsPlayedByDay: {} };
  state.tasks = {
    daily: { iso, progress:{}, claimed:{}, flags:{}, logged:{} },
    weekly: { weekStartIso: iso, progress:{}, claimed:{}, logged:{} },
    achievements: { claimed:{} },
