...
function downloadFullBackup(fileName=defaultBackupFileName()){
  ...
  // إذا كانت إضافة الملفات متوفرة، نحفظ النسخة الاحتياطية كملف داخل مساحة التطبيق.
  ...
  if(hasFilePlugin){
    const dirUrl = window.cordova.file.externalDataDirectory || window.cordova.file.dataDirectory;
    ...
    writeFile(dirUrl, fileName, jsonText)
      .then((fileEntry)=>{
        localStorage.setItem("dm_last_backup_filename", fileName);
        // احتفظ بالمسار للاستفادة لاحقًا (بدون تغيير أي زر آخر الآن)
        const p = (fileEntry && (fileEntry.nativeURL || fileEntry.toURL && fileEntry.toURL())) || "";
        if(p) localStorage.setItem("dm_last_backup_path", p);
        toast(isEn ? "Backup saved to device storage." : "تم حفظ النسخة الاحتياطية داخل ملفات التطبيق.");
      })
      .catch(()=>{
...
function renderLibrary(){
  ...
  function runTraining(cards){
    ...
    // Training tuning (lightweight path):
    // - XP/Rating at 50%
    // - Gold at 25%
    const mult = 0.5;
    const goldMult = 0.25;
...
function importSharedDeckFile(){
  ...
  const hasFilePlugin = typeof window.resolveLocalFileSystemURL === "function";
  const hasFileChooser = window.fileChooser && typeof window.fileChooser.open === "function";
  const hasChooser = window.chooser && typeof window.chooser.getFile === "function";
  const hasFilePath = window.FilePath && typeof window.FilePath.resolveNativePath === "function";
  ...
  function cordovaPickAndRead(){
    if(!hasFilePlugin){
      toast("للاستيراد داخل APK أضف cordova-plugin-file.");
      return;
    }
    if(!hasFileChooser && !hasChooser){
      toast("للاستيراد داخل APK أضف إضافة اختيار الملفات (File Chooser).\n\nمقترح: cordova-plugin-filechooser + cordova-plugin-filepath");
      return;
    }

    const onPicked = (uri)=>{
      if(!uri){ toast("لم يتم اختيار ملف."); return; }
      ...
      if(s.startsWith("content:") && hasFilePath){
        window.FilePath.resolveNativePath(uri, (p)=> attemptRead(p), ()=> attemptRead(uri));
      } else {
        attemptRead(uri);
      }
    };

    const onFail = ()=> toast("لم يتم اختيار ملف.");

    try{
      if(hasFileChooser){
        window.fileChooser.open(onPicked, onFail);
      } else if(hasChooser){
        const fn = window.chooser.getFile;
        if(fn.length >= 2){
          fn.call(window.chooser, (file)=> onPicked(file && (file.uri || file.nativeURL || file.url || file.file || file.path)), onFail);
        } else {
          fn.call(window.chooser, (file)=> onPicked(file && (file.uri || file.nativeURL || file.url || file.file || file.path)));
        }
      }
    }catch(e){
      toast("تعذّر فتح اختيار الملفات.");
    }
  }
